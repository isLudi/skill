...
附件证据快照日期：2026-07-20。群聊为 `青橙数据对接`，附件发布人为郅玲玉；历史检索返回 93 条该发布人的 Excel 附件消息。开课时间链接证据补充于 2026-07-27，发布人为李怡青。运行时必须重新检索，不得把本快照当成“当前最新”。
...
| 青橙行课开课时间 | 李怡青发布的 `om_x100b69478c6c80a0df62926437bfc84`，2026-07-27 19:08 | 活动页 `0728期`，`20260728期`，90 行 | `E:\1900_work\GAOTU\19003_青橙项目部看板维护表格\qing_daoke.xlsx` | `temp_table.dingxi01_qing_daoke` |
...
| 青橙行课开课时间 | `渠道,开课时间,期次,工作日-1,年级,课节数(第几节课)` → `qudao,begin_time,qici,dow,grade,ke_1`；额外列 `工作日` 明确忽略 | `qici`，且必须与活动页名后缀一致 | `qici + qudao + grade + ke_1 + begin_time + dow` |
...
- 开课时间源表只读取活动页；`begin_time` 折叠重复空白，`ke_1` 转为文本，源公式只读取缓存有效值并物化写入，禁止把外部链接公式复制到 `qing_daoke.xlsx`。
- 每个源文件先按登记键做严格唯一性检查。目标表的旧问题只允许保持不变，不允许新增或扩大。
- Excel 中的公式按有效缓存值判断幂等；真正发生切片变化时才重建该切片，并通过 Excel COM 重算后再允许本地 Apply。
- `qing_team_jg.xlsx` 的历史切片可能保留外部链接公式；重建 `result_architecture` 候选表时必须逐字节恢复原工作簿的 `xl/externalLinks/**` 子包，重算前后均验证所有 `r:id` 可解析，禁止用删除外链或公式物化来掩盖包结构损坏。
- 平台上传使用完整本地累计工作簿、`target-mode=reuse`、`import-mode=overwrite`；禁止把单次群文件直接以平台 `append` 模式上传。

## 来源质量门禁

当前安全阈值登记在 `workflow_registry.json`，以注册表为唯一机器执行来源：

| 文件族 | 最大来源年龄 | 来源行数 | 最大逐切片变化率 |
|---|---:|---:|---:|
| 个人期度目标表 | 360 小时 | 50–2000 | 50% |
| 团队期度目标表 | 360 小时 | 10–500 | 50% |
| 团队月度目标表 | 840 小时 | 5–200 | 50% |
| 全员结果数据架构 | 360 小时 | 50–2000 | 35% |
| 期次带班架构 | 360 小时 | 20–800 | 35% |
| 青橙行课开课时间 | 240 小时 | 20–500 | 50% |

逐切片变化优先比较目标中的同一 `qici/month`；新切片使用目标最新切片作为基线。必要列空值率按文件族分别登记，身份、期次和核心业务值默认要求 0 空值；个别组织层级列只允许注册表中明确声明的小比例空值。没有非空目标基线、阈值缺失、来源过期、行数越界、变化超限或空值率超限都会阻断 Plan、Apply 和 Upload。

这些阈值是“异常阻断线”，不是业务事实。若组织扩张、期次结构或课程规模发生已确认变化，先通过代码评审更新注册表和测试，再重新生成 Hash 绑定计划；不得在运行时放宽。
...
- `plan` 只读群消息、下载附件或登记文档到 runtime、生成本地 staged 副本和 Hash 绑定计划；不改 E 盘维护表，不上传平台。
