...
learn_duration AS (
    SELECT
        x.qici AS period_name,
        x.user_id,
        SUM(coalesce(x.live_learn_duration, 0)) AS total_live_learn_duration_seconds
    FROM (
        SELECT
        ...
        WHERE t.dt = date_format(now() - interval '2' hour, '%Y%m%d')
          ...
          AND t.course_second_level_department_name IN ('精品班学部', '市场部')
          AND t.is_need_attend = 1
    ) x
    GROUP BY x.qici, x.user_id
),
profile_base AS (
    SELECT
        b.period_name,
        ...
        coalesce(ld.total_live_learn_duration_seconds, 0) AS total_live_learn_duration_seconds
    FROM lead_base b
    LEFT JOIN learn_duration ld ON ld.period_name = b.period_name AND ld.user_id = b.user_id
),
-- ============================================================
...
profile_union AS (
    SELECT period_name, channel_map, grade_name,
        '不同上课时长成单用户占比（总出勤时长）' AS analysis_type,
        CASE
            WHEN total_live_learn_duration_seconds <= 0 THEN '没上课'
            WHEN total_live_learn_duration_seconds > 0 AND total_live_learn_duration_seconds < 1800 THEN '30min以内'
            WHEN total_live_learn_duration_seconds >= 1800 AND total_live_learn_duration_seconds < 3600 THEN '30-60min'
            WHEN total_live_learn_duration_seconds >= 3600 AND total_live_learn_duration_seconds < 7200 THEN '60-120min'
            WHEN total_live_learn_duration_seconds >= 7200 AND total_live_learn_duration_seconds < 10800 THEN '120-180min'
            ELSE '180min以上'
        END AS bucket_name,
        CASE
            WHEN total_live_learn_duration_seconds <= 0 THEN 1
            WHEN total_live_learn_duration_seconds > 0 AND total_live_learn_duration_seconds < 1800 THEN 2
            WHEN total_live_learn_duration_seconds >= 1800 AND total_live_learn_duration_seconds < 3600 THEN 3
            WHEN total_live_learn_duration_seconds >= 3600 AND total_live_learn_duration_seconds < 7200 THEN 4
            WHEN total_live_learn_duration_seconds >= 7200 AND total_live_learn_duration_seconds < 10800 THEN 5
            ELSE 6
        END AS bucket_sort,
...
WHERE a.period_name > '20260417期'
  AND a.analysis_type = '不同上课时长成单用户占比（总出勤时长）'
