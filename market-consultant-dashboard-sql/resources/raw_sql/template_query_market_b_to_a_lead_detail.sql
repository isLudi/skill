...
case 
  ...
  coalesce(lead_count, 0) as lead_count,
    ...
    coalesce(case when cast(valid_lead_count as varchar) = '1' then friend_lead_count else 0 end, 0) as is_friend_lead,
     coalesce(case when cast(valid_lead_count as varchar) = '1' and friend_lead_count = '0' then 1 else 0 end, 0) as non_friend_lead,
    ...
    coalesce(case when intention_level in ('A', 'B') and t.jieduan in ('深沟','已双沟') and conversion_lead_count = '1' then 1 else 0 end, 0) as AB_zhuanhua 
...
,daoke as (
    select 
        dk.period_name,
	    dk.channel_map,
        dk.employee_email_name,
	    dk.user_id,
	   case when sum(learn_duration_0) > 0 then 1 else 0 end as daoke
    from (
        select distinct 
            t1.period_name,
            ...
            t1.channel_map,
            t1.grade_1,
		    case when coalesce(t2.learn_duration,0)/60 > 60 then 1 else 0 end as learn_duration_0---直播+回放>60min、加上好友、未转化
        from (
            select 
                lead_id,
                user_id,
...
case 
    ...
    -- 对于其他日期，使用原有的周逻辑
        ...
        case when day_of_week(cast(begin_time as date)) = 2 
...
-----------------------整合 
,base as (select  data.*,
 case when is_friend_lead = '1' and conversion_lead_count = '0' and daoke = '1' then 1 else 0 end as B_type,
 case when non_friend_lead = '1' and conversion_lead_count = '0' and daoke = '0' then 1 else 0 end as D_type
 from data 
left join  daoke on daoke.period_name = data.period_name and daoke.channel_map = data.channel_map and daoke.employee_email_name = data.employee_email_name  and daoke.user_id = data.user_id)
select * from base
