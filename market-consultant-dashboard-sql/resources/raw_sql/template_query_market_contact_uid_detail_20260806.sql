...
, employee_email_name -- 顾问
,user_id
,lead_count
,assign_lead_count
,valid_lead_count
,case when first_call_time_diff_hour>=0 and first_call_time_diff_hour<=24 and valid_lead_count>0 then 1 else 0 end  chuda_24h
,case when first_call_time_diff_hour>=0 and first_call_time_diff_hour<=48 and valid_lead_count>0 then 1 else 0 end chuda_48h
,case when first_call_time_diff_hour is not null and valid_lead_count>0 then 1 else 0 end   chuda
,case when first_call_connected_time_diff_hour>=0 and first_call_connected_time_diff_hour<=24 and valid_lead_count>0 then 1 else 0 end goutong_24h
,case when first_call_connected_time_diff_hour>=0 and first_call_connected_time_diff_hour<=48 and valid_lead_count>0 then 1 else 0 end goutong_48h
,case when first_call_connected_time_diff_hour>=0 and first_call_connected_time_diff_hour<=168 and valid_lead_count>0 then 1 else 0 end goutong
,dl.section_assign_time
from 
(
select *,
...
and concat(group_period_year,group_period_term)>'20251001期'
)
SELECT
group_period_name
,virtual_first_department_name
...
,section_assign_time
from t
where  group_period_name in ${group_period_name}
    and virtual_third_department_name = '市场顾问部'
   and channel_map not in ('KOC赠课失败','IP赠课失败')
    and  SUBSTR(section_assign_time, 1, 10) >= ${begin_day}
    and  SUBSTR(section_assign_time, 1, 10) <= ${end_day}
