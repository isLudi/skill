...
virtual_fourth_department_name  depart,
    ...
    user_id,
case 
    when hour(cast(substr(section_assign_time, 1, 19) as timestamp)) >= 23 or hour(cast(substr(section_assign_time, 1, 19) as timestamp)) < 7 then '23-凌晨7'
    when hour(cast(substr(section_assign_time, 1, 19) as timestamp)) between 7 and 9 then '7-10'
    when hour(cast(substr(section_assign_time, 1, 19) as timestamp)) between 10 and 13 then '10-14'
    when hour(cast(substr(section_assign_time, 1, 19) as timestamp)) between 14 and 18 then '14-19'
    when hour(cast(substr(section_assign_time, 1, 19) as timestamp)) between 19 and 22 then '19-23'
    else '其他'
end as assign_day,
 rule_name,
case
