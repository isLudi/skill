...
,hesuanqudao as (
    select
        yijiqudao,
        max(hesuanqudao) as hesuanqudao
    from (
        select
            trim(cast(yijiqudao as varchar)) as yijiqudao,
            trim(cast(hesuanqudao as varchar)) as hesuanqudao
        from temp_table.zhangshiyin01_hesuanqudao
        where yijiqudao is not null
          and hesuanqudao is not null
    ) h
    where yijiqudao <> ''
      and hesuanqudao <> ''
    group by yijiqudao
...
,hq0_match as (
    select
        ...
        hesuanqudao,
        row_number() over (
            partition by _cs_row_id
            order by length(yijiqudao) desc, yijiqudao
        ) as rn
    from (
        select
            c._cs_row_id,
        ...
        from cs_keyed c
        join hesuanqudao h
          on c.rule_name0 is not null
         and strpos(trim(cast(c.rule_name0 as varchar)), h.yijiqudao) > 0
    ) m
)
,hq_match as (
    select
    ...
    from (
        ...
        from cs_keyed c
        join hesuanqudao h
          on c.rule_name is not null
         and strpos(trim(cast(c.rule_name as varchar)), h.yijiqudao) > 0
    ) m
)
,cs as (
    select
        c.*,
        coalesce(h0.hesuanqudao, h0_case.hesuanqudao, c.qudao0_case) as qudao0,
        coalesce(h.hesuanqudao, h_case.hesuanqudao, c.qudao_case) as qudao
    from cs_keyed c
    left join hq0_match h0
      on c._cs_row_id = h0._cs_row_id
     and h0.rn = 1
    left join hq_match h
      on c._cs_row_id = h._cs_row_id
     and h.rn = 1
    left join hesuanqudao h0_case
      on trim(cast(c.qudao0_case as varchar)) = h0_case.yijiqudao
    left join hesuanqudao h_case
      on trim(cast(c.qudao_case as varchar)) = h_case.yijiqudao
)
