...
call_duration_raw AS (
    SELECT
        period_name,
        lead_id,
        user_id,
        max(section_assign_all_call_duration) AS section_assign_all_call_duration
    FROM (
        SELECT DISTINCT
            concat(
    ...
    GROUP BY period_name, lead_id, user_id
),
lead_base AS (
    SELECT
        period_name,
...
profile_base AS (
    SELECT
        ...
        coalesce(cd.section_assign_all_call_duration, 0) AS total_call_duration_seconds
    FROM lead_base b
    LEFT JOIN call_duration_raw cd
      ON cast(cd.period_name AS varchar) = cast(b.period_name AS varchar)
     AND cast(cd.lead_id AS varchar) = cast(b.lead_id AS varchar)
     AND cast(cd.user_id AS varchar) = cast(b.user_id AS varchar)
),
-- ============================================================
...
profile_union AS (
    SELECT period_name, channel_map, grade_name,
        '不同通时成单用户占比（总通时）' AS analysis_type,
        CASE
            WHEN total_call_duration_seconds < 300 THEN '5min以内'
            WHEN total_call_duration_seconds >= 300 AND total_call_duration_seconds < 600 THEN '5min-10min'
            WHEN total_call_duration_seconds >= 600 AND total_call_duration_seconds < 900 THEN '10min-15min'
            WHEN total_call_duration_seconds >= 900 AND total_call_duration_seconds < 1200 THEN '15min-20min'
            ELSE '20min以上'
        END AS bucket_name,
        CASE
            WHEN total_call_duration_seconds < 300 THEN 1
            WHEN total_call_duration_seconds >= 300 AND total_call_duration_seconds < 600 THEN 2
            WHEN total_call_duration_seconds >= 600 AND total_call_duration_seconds < 900 THEN 3
            WHEN total_call_duration_seconds >= 900 AND total_call_duration_seconds < 1200 THEN 4
            ELSE 5
        END AS bucket_sort,
...
WHERE a.period_name > '20260417期'
  AND a.analysis_type = '不同通时成单用户占比（总通时）'
