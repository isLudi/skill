...
select
        b.*,
        case
    when cast(date_parse(replace(concat(group_period_year, group_period_term), '期', ''), '%Y%m%d') as date)
...
end as market_period_name
    from bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df b 
...
src_keyed AS (
    select
        s.*,
        concat(case when s.ad_account_name is null then 'N' else concat('V', cast(length(cast(s.ad_account_name as varchar)) as varchar), ':', cast(s.ad_account_name as varchar)) end, '|', case when s.channel_name_1 is null then 'N' else concat('V', cast(length(cast(s.channel_name_1 as varchar)) as varchar), ':', cast(s.channel_name_1 as varchar)) end, '|', case when s.channel_name_2 is null then 'N' else concat('V', cast(length(cast(s.channel_name_2 as varchar)) as varchar), ':', cast(s.channel_name_2 as varchar)) end, '|', case when s.channel_name_3 is null then 'N' else concat('V', cast(length(cast(s.channel_name_3 as varchar)) as varchar), ':', cast(s.channel_name_3 as varchar)) end, '|', case when s.channel_provider_name is null then 'N' else concat('V', cast(length(cast(s.channel_provider_name as varchar)) as varchar), ':', cast(s.channel_provider_name as varchar)) end, '|', case when s.channel_second_provider_name is null then 'N' else concat('V', cast(length(cast(s.channel_second_provider_name as varchar)) as varchar), ':', cast(s.channel_second_provider_name as varchar)) end, '|', case when s.first_department_name is null then 'N' else concat('V', cast(length(cast(s.first_department_name as varchar)) as varchar), ':', cast(s.first_department_name as varchar)) end, '|', case when s.flow_order_price is null then 'N' else concat('V', cast(length(cast(s.flow_order_price as varchar)) as varchar), ':', cast(s.flow_order_price as varchar)) end, '|', case when s.flow_orders_income_amount is null then 'N' else concat('V', cast(length(cast(s.flow_orders_income_amount as varchar)) as varchar), ':', cast(s.flow_orders_income_amount as varchar)) end, '|', case when s.flow_original_order_activity_price is null then 'N' else concat('V', cast(length(cast(s.flow_original_order_activity_price as varchar)) as varchar), ':', cast(s.flow_original_order_activity_price as varchar)) end, '|', case when s.flow_pool_name is null then 'N' else concat('V', cast(length(cast(s.flow_pool_name as varchar)) as varchar), ':', cast(s.flow_pool_name as varchar)) end, '|', case when s.get_customer_way_name is null then 'N' else concat('V', cast(length(cast(s.get_customer_way_name as varchar)) as varchar), ':', cast(s.get_customer_way_name as varchar)) end, '|', case when s.lead_create_time is null then 'N' else concat('V', cast(length(cast(s.lead_create_time as varchar)) as varchar), ':', cast(s.lead_create_time as varchar)) end, '|', case when s.lead_purchase_intention_level1_category_name is null then 'N' else concat('V', cast(length(cast(s.lead_purchase_intention_level1_category_name as varchar)) as varchar), ':', cast(s.lead_purchase_intention_level1_category_name as varchar)) end, '|', case when s.lead_purchase_intention_level2_category_name is null then 'N' else concat('V', cast(length(cast(s.lead_purchase_intention_level2_category_name as varchar)) as varchar), ':', cast(s.lead_purchase_intention_level2_category_name as varchar)) end, '|', case when s.page_id_name is null then 'N' else concat('V', cast(length(cast(s.page_id_name as varchar)) as varchar), ':', cast(s.page_id_name as varchar)) end, '|', case when s.period_name is null then 'N' else concat('V', cast(length(cast(s.period_name as varchar)) as varchar), ':', cast(s.period_name as varchar)) end, '|', case when s.put_plan_name is null then 'N' else concat('V', cast(length(cast(s.put_plan_name as varchar)) as varchar), ':', cast(s.put_plan_name as varchar)) end, '|', case when s.rule_name is null then 'N' else concat('V', cast(length(cast(s.rule_name as varchar)) as varchar), ':', cast(s.rule_name as varchar)) end, '|', case when s.second_department_name is null then 'N' else concat('V', cast(length(cast(s.second_department_name as varchar)) as varchar), ':', cast(s.second_department_name as varchar)) end, '|', case when s.sku_id_name is null then 'N' else concat('V', cast(length(cast(s.sku_id_name as varchar)) as varchar), ':', cast(s.sku_id_name as varchar)) end, '|', case when s.source_manager_name is null then 'N' else concat('V', cast(length(cast(s.source_manager_name as varchar)) as varchar), ':', cast(s.source_manager_name as varchar)) end, '|', case when s.source_put_plan_name is null then 'N' else concat('V', cast(length(cast(s.source_put_plan_name as varchar)) as varchar), ':', cast(s.source_put_plan_name as varchar)) end, '|', case when s.third_department_name is null then 'N' else concat('V', cast(length(cast(s.third_department_name as varchar)) as varchar), ':', cast(s.third_department_name as varchar)) end, '|', case when s.trace_type_name is null then 'N' else concat('V', cast(length(cast(s.trace_type_name as varchar)) as varchar), ':', cast(s.trace_type_name as varchar)) end, '|', case when s.virtual_fifth_department_name is null then 'N' else concat('V', cast(length(cast(s.virtual_fifth_department_name as varchar)) as varchar), ':', cast(s.virtual_fifth_department_name as varchar)) end, '|', case when s.virtual_fourth_department_name is null then 'N' else concat('V', cast(length(cast(s.virtual_fourth_department_name as varchar)) as varchar), ':', cast(s.virtual_fourth_department_name as varchar)) end, '|', case when s.virtual_second_department_name is null then 'N' else concat('V', cast(length(cast(s.virtual_second_department_name as varchar)) as varchar), ':', cast(s.virtual_second_department_name as varchar)) end) as channel_key
    from src s
),
src_channel_dimensions AS (
    select distinct
        channel_key,
        ad_account_name,
        channel_name_1,
...
when sku_id_name like '%2000题%' and third_department_name='图书营销部' then '西安直播2000题'
when sku_id_name like '%五年高考%' and third_department_name='图书营销部' then '西安直播五年真题'
...
when put_plan_name like '%全国教考%' and third_department_name='图书营销部' then '西安直播全国'
...
when third_department_name = '私域运营部' and source_manager_name in ('陈雷19','崔慧敏01','侯佳林01','郑天琪02','杨彬屹','曹义鹏','王硕阳','于超研','岳一帆02','田起帆','王绍阳','肖佳兴','姚佳03','秦金萍','李冞萤') and (rule_name like '%koc自孵化下引%' or rule_name like '%koc下引%' or page_id_name like '%自营%') then '自孵化KOC下引'
when third_department_name = '私域运营部' and source_manager_name in ('陈雷19','崔慧敏01','侯佳林01','郑天琪02','杨彬屹','曹义鹏','王硕阳','于超研','岳一帆02','田起帆','王绍阳','肖佳兴','姚佳03','秦金萍','李冞萤') then '进校私域合作'
...
when third_department_name = '直播部' and (sku_id_name like '%春春%' or sku_id_name like '%瑞春%' or rule_name like '%春春%' or rule_name like '%瑞春%') and channel_name_2='B站' then '陈瑞春-B站'
...
when third_department_name in ('直播部','新媒体内容运营部') and (sku_id_name like '%春春%' or sku_id_name like '%瑞春%' or rule_name like '%春春%' or rule_name like '%瑞春%') and channel_name_2='视频号' then '陈瑞春-视频号49'
...
when channel_name_1 = '信息流' and channel_name_2 = 'B站' and third_department_name not like '%投放%' and (rule_name like '%0904期-短期班-新高二-市场私域低价单%' or flow_original_order_activity_price='100' or page_id_name like '%1元%') then 'B站信息流-朱汉祺-1元'
...
src_channel_pick AS (
    select
        channel_key,
        min_by(channel, rule_group) as channel
    from src_channel_candidates
...
src_mapped AS (
    select
        s.*,
        coalesce(p.channel, '其他未知流量') as channel_map
    from src_keyed s
    left join src_channel_pick p
      on s.channel_key = p.channel_key
),
data as
(select distinct
market_period_name as period_name,
...
channel_map as channel_map,
    ...
    nvl(lead_count,0) lead_count,--线索数
    nvl(valid_lead_count,0) valid_lead_count,--有效线索数
    nvl(conversion_lead_count,0) conversion_lead_count,--转化人数
    nvl(subject_count,0) subject_count,---转化科目人次
    nvl(same_lead_period_subject_count,0) same_lead_period_subject_count,--当期转化人次
    nvl(lb_subject_count,0)  lb_subject_count,--联报人次
    nvl(same_lead_period_lb_subject_count,0) same_lead_period_lb_subject_count,--当期联报人次
    nvl(order_count,0) order_count,--订单数
    nvl(income_amount,0) income_amount,--收款金额
    nvl(in_pay_period_refund_amount,0) in_pay_period_refund_amount,--当期支付当期退款
    nvl(non_pay_period_refund_amount,0) non_pay_period_refund_amount,--往期支付当期退款
    nvl(jp_cross_department_refund_amount,0) jp_cross_department_refund_amount,--0802期之前其他学部订单退款
    nvl(same_lead_period_order_count,0) same_lead_period_order_count,--当期线索当期转化订单数
    nvl(same_lead_period_conversion_lead_count,0) same_lead_period_conversion_lead_count,--当期线索当期转化线索数
    nvl(same_lead_period_income_amount,0) same_lead_period_income_amount,--当期线索当期收款
    nvl(same_lead_period_refund_amount,0) same_lead_period_refund_amount--当期线索当期同学部退款
from src_mapped
)
select *
from data
where period_name = ${dt}
