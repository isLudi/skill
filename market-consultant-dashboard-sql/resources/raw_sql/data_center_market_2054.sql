with denglu_app as (
    select
        user_number,
        case
            when max(is_app_denglu_h) = 1 or max(is_app_denglu_d) = 1 then 1
            else 0
    ...
    from (
        select
            user_number,
            case
                when event_timestamp >= now() - interval '7' day
                 and appliction_name in ('PC客户端', 'APP', 'PC')
                then 1 else 0
            end as is_app_denglu_d,
            0 as is_app_denglu_h,
            1 as d_present
        from (
            select
                user_number,
                appliction_name,
                try(date_parse(last_event_time, '%Y-%m-%d %H:%i:%s:%f')) as event_timestamp,
                row_number() over (
...
data as (
    select distinct
        ...
        f.channel_name_2,
        f.channel_name_3,
        f.channel_provider_name,
        f.channel_second_provider_name,
        f.deep_communicate_method,
        f.dt,
        f.employee_email_name,
        f.employee_email_prefix,
        f.first_call_time,
        f.first_department_name,
        f.flow_order_price,
        f.flow_orders_income_amount,
        f.flow_original_order_activity_price,
        f.flow_pool_name,
        f.friend_lead_count,
        ...
        f.page_id_name,
        f.period_mapping_first_level_department_name,
        f.period_mapping_second_level_department_name,
        f.period_name,
        f.put_plan_name,
        f.rule_name,
        f.second_department_name,
        f.section_assign_employee_first_level_department_name,
        f.section_assign_employee_second_level_department_name,
        f.section_assign_employee_third_level_department_name,
        f.section_assign_time,
        f.sku_id_name,
        f.source_manager_name,
        f.source_put_plan_name,
...
when rule_name like '%途途私域%' or (rule_name like '%私域%' and first_department_name = 'TT') then '途途私域'
        ...
        from (
            select
                ...
                case
                    ...
                    when cast(assign_time as date) between date '2026-08-13' and date '2026-08-18' then '20260815期'
                    else concat(
                    date_format(
    ...
    where f.dt = format_datetime(now() - interval '2' hour, 'YYYYMMdd')
      ...
      and cast(date_parse(replace(concat(f.group_period_year, f.group_period_term), '期', ''), '%Y%m%d') as date) >= date '2026-05-11'
),
--------------------------5min比例、外呼次数、外呼接通次数
call_c as (
    select
        ...
        inner join (
            select
                ...
                case
                    ...
                    when cast(begin_time as date) between date '2026-08-13' and date '2026-08-18' then '20260815期'
