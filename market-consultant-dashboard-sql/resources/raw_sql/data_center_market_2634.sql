...
case
  ...
  when f.get_customer_way_name in ('平台自播') and f.flow_pool_name in ('赢战广东中考','高途专版图书','状元帮教辅') then '广东图书'
  ...
  when (f.rule_name like '%孟亚飞IP9元%' or f.rule_name like '%孟亚飞ip9元%') and  f.rule_name not like '%孟亚飞IP99元%' then '孟亚飞IP9元'
  when (f.rule_name like '%孟亚飞IP%' or f.rule_name  like '%孟亚飞ip9元%') and f.rule_name not like '%孟亚飞IP9元%' then '孟亚飞IP9元'
  ...
  when lead_purchase_intention_name like '%多学科拓展%' and f.get_customer_way_name = '微信私域' then '训练营'
