...
from (
    ...
    where dt = FORMAT_DATETIME(NOW() - INTERVAL '2' HOUR, 'YYYYMMdd')
		...
		where qici = '20260410期'
)
-- 调课调班（按name和user_id1去重，每个用户保留一条记录）
,gmv_t as (
    select 
        id, order_number, clazz_name, user_id1, trade_status, trade_time,  weekday_name,
...
,gmv_z as (
    select 
        id, order_number, clazz_name, user_id1, trade_status, trade_time, weekday_name, -- 这里加了逗号
        trade_type, email_prefix, name, grade_list, subject,
        qici, school_term_id, teacher_name,
    ...
    group by id, order_number, clazz_name, user_id1, trade_status, trade_time,weekday_name,  -- 这里加了逗号
             trade_type, email_prefix, name, grade_list, subject,
             qici, school_term_id, teacher_name,
             course_first_level_department_name,
             course_second_level_department_name
...
when rr.rule_name like '%百度星耀数学%' or rr.rule_name like '%数学%' or rr.rule_name like '%百度星耀物理%' or rr.rule_name like '%物理%' then '百度星耀' else '未知' 
end as channel_1
from lead_gmv
left join (
	select *,case 
...
when f.flow_pool_name like '%退款%' and f.third_department_name ='图书营销部' then '西安直播-退款订单复用'
...
when f.third_department_name = '私域运营部' and f.source_manager_name in ('陈雷19','崔慧敏01','侯佳林01','郑天琪02','杨彬屹','曹义鹏','王硕阳','于超研','岳一帆02','田起帆','王绍阳','肖佳兴','姚佳03','秦金萍') and (f.rule_name like '%koc自孵化下引%' or f.rule_name like '%koc下引%') then '自孵化KOC下引'
when f.third_department_name = '私域运营部' and f.source_manager_name in ('陈雷19','崔慧敏01','侯佳林01','郑天琪02','杨彬屹','曹义鹏','王硕阳','于超研','岳一帆02','田起帆','王绍阳','肖佳兴','姚佳03','秦金萍') then '进校私域合作'
...
when f.third_department_name in ('直播部','新媒体内容运营部','市场一组','私域运营部') and f.flow_pool_name ='电商退款用户池'  then 'IP退款订单复用'
...
when f.third_department_name = '直播部' and (f.sku_id_name like '%春春%' or f.sku_id_name like '%瑞春%' or f.rule_name like '%春春%' or f.rule_name like '%瑞春%') and f.channel_name_2='百度' then '陈瑞春-百度'
when f.third_department_name = '直播部' and (f.sku_id_name like '%春春%' or f.sku_id_name like '%瑞春%' or f.rule_name like '%春春%' or f.rule_name like '%瑞春%') and f.channel_name_2='抖音' then '陈瑞春-抖音199'
when f.third_department_name = '直播部' and (f.sku_id_name like '%春春%' or f.sku_id_name like '%瑞春%' or f.rule_name like '%春春%' or f.rule_name like '%瑞春%') and f.channel_name_2='视频号' then '陈瑞春-视频号49'
when f.third_department_name = '直播部' and (f.sku_id_name like '%朱博士99%' or f.rule_name like '%朱汉祺99%' or f.rule_name like '%朱博士%' or f.flow_pool_name like '%朱博士%' or f.sku_id_name like '%朱博士%') and f.channel_name_2 = 'B站' then '朱博士-B站'
when f.third_department_name = '直播部' and (f.sku_id_name like '%朱博士99%' or f.rule_name like '%朱汉祺99%' or f.rule_name like '%朱博士%' or f.flow_pool_name like '%朱博士%' or f.sku_id_name like '%朱博士%') and f.channel_name_2 = '百度' then '朱博士-百度'
when f.third_department_name = '直播部' and (f.sku_id_name like '%朱博士99%' or f.rule_name like '%朱汉祺99%' or f.rule_name like '%朱博士%' or f.flow_pool_name like '%朱博士%' or f.sku_id_name like '%朱博士%') and f.channel_name_2 = '抖音' then '朱博士-抖音199'
when f.third_department_name = '直播部' and (f.sku_id_name like '%朱博士99%' or f.rule_name like '%朱汉祺99%' or f.rule_name like '%朱博士%' or f.flow_pool_name like '%朱博士%' or f.sku_id_name like '%朱博士%') and f.channel_name_2 = '视频号' then '朱博士-视频号49'
...
and concat(cast(date_format(date_add('day',4,date_trunc('week',date_add('day',-1,date_parse(replace(concat(f.group_period_year,f.group_period_term),'期',''),'%Y%m%d')))),'%Y%m%d')as varchar),'期') = '20260410期'
),
-------------汇总渠道总线索
leds as (select qici,channel_map_1,virtual_leader_email_name ,sum(valid_lead_count) as v_lead_c from data group by qici,channel_map_1,virtual_leader_email_name)
-------------汇总，左右表数据都要
select 
coalesce(leds.qici, geren.qici) as qici,coalesce(leds.channel_map_1,geren.channel_1) as channel_1,
coalesce(leds.virtual_leader_email_name,geren.jingli) as jingli ,coalesce(leds.v_lead_c,0) as v_lead_c,
sum(nvl(gmv_1,0)) as gmv_1,sum(nvl(gmv_2,0)) as gmv_2,sum(nvl(gmv_3,0)) as gmv_3,sum(nvl(gmv_4,0)) as gmv_4,sum(nvl(gmv_5,0)) as gmv_5 ,sum(nvl(gmv_6,0)) as gmv_6 ,sum(nvl(gmv_7,0)) as gmv_7,sum(nvl(gmv_t,0)) as gmv_t
from leds 
full outer join geren on leds.qici = geren.qici and leds.channel_map_1 = geren.channel_1 and leds.virtual_leader_email_name = geren.jingli
group by coalesce(leds.qici, geren.qici),coalesce(leds.channel_map_1,geren.channel_1),coalesce(leds.virtual_leader_email_name,geren.jingli),coalesce(leds.v_lead_c,0)
