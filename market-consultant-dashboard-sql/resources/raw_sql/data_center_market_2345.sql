...
and (period_mapping_first_level_department_name = 'H业务线' or period_mapping_first_level_department_name is null)
 and (period_mapping_second_level_department_name in ('精品班学部','市场部') or period_mapping_second_level_department_name is null)
)
--------------------------5min比例、外呼次数、外呼接通次数
,call_c as (
    select 
            ...
            when data.channel_map = '曹忆' 
                then case when exists (
                    ...
                    where daoke.user_id = data.user_id 
                        ...
                        and data.channel_map = daoke.channel_map
                        and daoke.ke_1 = '3' 
                        and daoke.is_valid_live_learn = '1'
                ) then 1 else 0 end
            else case when exists (
                    select 1 
                    from daoke 
...
case when date_diff('day', date_parse(zz.assign_day, '%Y-%m-%d'), date_parse(substr(zz.period_name, 1, 4) || '-' || substr(zz.period_name, 5, 2) || '-' || substr(zz.period_name, 7, 2), '%Y-%m-%d')) between -9 and 9 
then substr(zz.assign_day, 6, 5)
else '其他'
end as assign_day_new
from zhuanhua zz
----渠道
