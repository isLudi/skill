...
coalesce(
    ...
    case
        ...
        -- For other dates, keep the original weekly fallback.
        else
            case
            ...
            where learn.dt = date_format(now() - interval '2' hour, '%Y%m%d')
                and learn.hour = date_format(now() - interval '3' hour, '%H')
...
final_dataset as (
    select
        ...
        fre.trade_income as section_refund_rate_den,
         case
             when fre.period_name is null then null
             else dense_rank() over (order by fre.period_name desc)
         end as period_rank
