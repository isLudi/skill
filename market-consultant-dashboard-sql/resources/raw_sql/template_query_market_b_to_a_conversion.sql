...
,call_c as (
    select 
        sub.to_user_number as user_number,
        sub.employee_email_name,
...
from (
select communication_timestamp,from_teacher_email_prefix,to_user_number,call_duration,call_status,call_type,employee_email_name
    from service_dw.dws_service_wechat_call_detail_df
   where  dt = format_datetime(now() - interval '24' hour, 'YYYYMMdd') 
    and employee_first_level_department_name = 'H业务线'
    and employee_second_level_department_name = '市场部'
    and call_duration is not null) sub
    group by 1,2
)
-----------------------整合 
,base as 
...
case when data.first_call_time_diff_hour >= 0 and data.first_call_time_diff_hour <= 24 and data.valid_lead_count > 0 then 1 else 0 end as first_call_in_24h,
	...
	coalesce(call_c.zong_call_ci_1, 0) as zong_call_ci_1,
	coalesce(call_c.call_status_1, 0) as call_status_1,
	coalesce(call_c.call_duration_1, 0) as call_duration_1,
    case when denglu_app.user_number is not null then denglu_app.is_app_denglu else 0 end as is_app_denglu
    from data 
	left join call_c on call_c.user_number = data.user_id and call_c.employee_email_name = data.employee_email_name
...
(select
    ...
    zhuguan, -- 小组长
    employee_email_name,
    count(lead_count) IP_lead_count,
    count(valid_lead_count) can_renew_ds_count_a,
    ...
    sum(is_shengou) shengou_lead,
     ...
     sum(zong_call_ci_1) zong_call_ci,
     sum(call_status_1) call_status_1,
     sum(call_duration_1) call_duration_1,
