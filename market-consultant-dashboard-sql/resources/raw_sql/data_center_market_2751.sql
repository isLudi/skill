...
finance_base as (
    select
        ...
        case
            ...
            when x.trade_dt between date '2026-08-13' and date '2026-08-18' then '20260815期'
            else concat(
            date_format(
                cast(
...
manual_target as (
    select
        cast(qici as varchar) as qici,
        employee_email_name as name,
        max(try_cast(renchan as double)) as manual_receive_target
    from temp_table.dingxi01_pingyou_jg
    where try_cast(renchan as double) > 0
      and cast(qici as varchar) >= '20260403期'
    group by
        cast(qici as varchar),
        employee_email_name
...
conversion_base as (
    select
        ...
        case
            ...
            when x.group_period_date between date '2026-08-13' and date '2026-08-18' then '20260815期'
...
conversion_with_target as (
    select
        ...
        cc.kk_trade_income,
        cc.pre_refund,
        cc.leads_count * coalesce(cd_exact.goal, cd_default.goal, 0) as receive_target
    from conversion_by_channel cc
    left join cost_dim cd_exact
      on cd_exact.qici = cc.qici
     and cd_exact.channel = trim(cc.channel_map)
     and cd_exact.grade = cc.grade_1
    left join cost_dim cd_default
      on cd_default.qici = cc.qici
     and cd_default.channel = trim(cc.channel_map)
     and cd_default.grade = '0'
),
conversion_agg as (
    select
        qici,
...
consultant_period_metric as (
    select
        ...
        coalesce(mt.manual_receive_target, cm.receive_target, 0) as receive_target,
        ...
        case
            when coalesce(mt.manual_receive_target, cm.receive_target, 0) = 0 then cast(0 as double)
            else coalesce(fp.period_pmit, 0) / coalesce(mt.manual_receive_target, cm.receive_target, 0)
        end as target_completion_rate,
