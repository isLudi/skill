    ...
    select distinct
        coalesce(
            case
                ...
                when cast(date_parse(replace(concat(t1.group_period_year, t1.group_period_term), '期', ''), '%Y%m%d') as date) between date '2026-08-07' and date '2026-08-12' then '20260808期'
                when cast(date_parse(replace(concat(t1.group_period_year, t1.group_period_term), '期', ''), '%Y%m%d') as date) between date '2026-08-13' and date '2026-08-18' then '20260815期'
                else null
                end,
            concat(
                date_format(
...
case 
    ...
    when rule_name like '%初一%' then '初一'
        ...
        coalesce(case
            when cast(t.sale_flow_stage_sequence as varchar) in ('450', '470', '700', '850', '950', '955', '1050')
            then 1 else 0
        end, 0) as has_reached_shengou,
        coalesce(case
            when cast(t.sale_flow_stage_sequence as varchar) in ('470', '700', '850', '950', '955', '1050')
            then 1 else 0
        end, 0) as has_reached_shuanggou,
        coalesce(case
            when cast(t.sale_flow_stage_sequence as varchar) in ('450', '470', '700', '850', '950', '955', '1050')
             and t1.conversion_lead_count = 1
            then 1 else 0
        end, 0) as sg_payers,
        coalesce(case
            when cast(t.sale_flow_stage_sequence as varchar) in ('470', '700', '850', '950', '955', '1050')
...
select
    fre.*,
    dense_rank() over (order by fre.period_name desc) as period_rank
from (
    ...
    select
        ...
        frh.call_5m_z,
        frh.daoke_1,
        frh.first_lesson_attendance_num,
        frh.first_lesson_attendance_den,
        frh.deep_effective_attendance_num,
    ...
    select
        ...
        sum(
            case
                when feb.rn_employee_channel_base = 1 and feb.channel_lead_count_all > 0 then 1
                else 0
            end
        ) over (
            partition by feb.period_name, feb.depart, feb.employee_email_name
        ) as channel_mix_cnt_all,
        feb.rn_employee_channel_base as rn_employee_channel_all,
        row_number() over (
            partition by feb.period_name, feb.depart, feb.employee_email_name
            order by feb.channel_map, feb.rule_name, feb.grade_1
        ) as rn_employee_all,
        max(coalesce(feb.podan, 0)) over (
            partition by feb.period_name, feb.depart, feb.employee_email_name
        ) as employee_podan_flag_all,
        row_number() over (
            partition by feb.period_name, feb.depart, feb.employee_email_name
            order by
                case when coalesce(feb.podan, 0) > 0 then 0 else 1 end,
                feb.channel_map,
                feb.rule_name,
                feb.grade_1
...
select
    row_number() over (
        partition by zz.period_name, zz.channel_map, zz.rule_name, zz.grade_1, zz.depart_1, zz.depart, zz.jingli, zz.zhuguan, zz.employee_email_name
        order by org.priority
    ) as org_rn,
    zz.period_name,
    zz.channel_map,
    zz.rule_name,
    ...
    zz.depart_1,
    zz.depart,
    coalesce(org.jingli, zz.jingli) as jingli,
    coalesce(org.xiaozu, zz.zhuguan) as zhuguan,
    ...
    zz.call_5m_z,
    zz.daoke_1,
    zz.first_lesson_attendance_num,
    zz.first_lesson_attendance_den,
    zz.deep_effective_attendance_num,
    ...
    coalesce(org.xiaozu, zz.zhuguan) as xiaozu,
    coalesce(org.jingli, zz.jingli) as jingli_11,
...
select
   ...
   sum(is_f_call) is_f_call,
    ...
    sum(daoke1) daoke_1,
    sum(first_lesson_attendance_num) first_lesson_attendance_num,
    sum(first_lesson_attendance_den) first_lesson_attendance_den,
    sum(deep_effective_attendance_num) deep_effective_attendance_num,
...
case when  conversion_lead_count = '1' and call_c.call_duration_1 >= 2400 then 1 else 0 end as call_40m_z,	
	---------------到课用的渠道！！
	case
            when data.channel_map = '曹忆' then coalesce(daoke_flags.daoke_3, 0)
            else coalesce(daoke_flags.daoke_1, 0)
        end as daoke1,
...
left join (
    ...
    select
        ...
        dk.user_id,
        dk.channel_map,
        max(case when cast(ke.ke_1 as varchar) = '3' and dk.live_learn_duration > 0 then 1 else 0 end) as daoke_3,
        max(case when cast(ke.ke_1 as varchar) = '1' and dk.live_learn_duration > 0 then 1 else 0 end) as daoke_1
...
coalesce(
    case
    when cast(tlearn.begin_time as date) between date '2026-07-14' and date '2026-07-19' then '20260716期'
    when cast(tlearn.begin_time as date) between date '2026-07-20' and date '2026-07-25' then '20260722期'
    when cast(tlearn.begin_time as date) between date '2026-07-26' and date '2026-07-31' then '20260728期'
    when cast(tlearn.begin_time as date) between date '2026-08-01' and date '2026-08-06' then '20260803期'
    when cast(tlearn.begin_time as date) between date '2026-08-07' and date '2026-08-12' then '20260808期'
    when cast(tlearn.begin_time as date) between date '2026-08-13' and date '2026-08-18' then '20260815期'
    else null
    end,
    case
  ...
  on daoke_flags.user_id = data.user_id
 and daoke_flags.employee_email_prefix = data.employee_email_prefix
 and daoke_flags.period_name = data.period_name
 and daoke_flags.channel_map = data.channel_map
left join (
    ...
    group by 1,2,3
) call_c on call_c.user_number = data.user_id and call_c.section_assign_employee_email_prefix = data.employee_email_prefix and call_c.lead_id = data.lead_id
left join (
...
) f_call0 on f_call0.assign_employee_email_name = data.employee_email_name and f_call0.user_id = data.user_id 
left join (
...
left join (
  ...
  on org.employee_email_name = zz.employee_email_name
 and (
