...
from (
    select
	 ...
	 case
			...
			when substr(trade_time, 1, 10) >= '2026-08-13' and substr(trade_time, 1, 10) <= '2026-08-18' then '20260815期'
...
,lead_info as (
    select distinct
        ...
        case
            ...
            when cast(date_parse(replace(concat(t.group_period_year, t.group_period_term), '期', ''), '%Y%m%d') as date) between date '2026-08-13' and date '2026-08-18' then '20260815期'
...
when lf.rule_name like '%语数英%' and lf.third_department_name = '新媒体内容运营部' then '语数英'
 when lf.third_department_name = '直播部' and lf.sku_id_name like '%山东专版%' then '北京直播山东'
  when lf.third_department_name = '直播部' and lf.sku_id_name like '%河南专版%' then '北京直播河南'
...
when lf.source_manager_name in ('马思雨02','袁银') and lf.rule_name like '%集团%' then '集团私域'
...
when (lf.flow_pool_name like '%北大汤哥%' or lf.flow_pool_name like '%海淀名师高阶%' or lf.flow_pool_name like '%海淀高阶%' or lf.flow_pool_name like '%高阶英语887%') and lf.sku_id_name like '%小艺%'  then '郭艺'
...
when lf.third_department_name in ('线上商务部') and lf.source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪','曹蕊07','曲默晗') and lf.flow_pool_name like '%退款%' then 'KOC-退款订单复用'
when lf.third_department_name in ('直播部','新媒体内容运营部','市场一组','私域运营部') and lf.flow_pool_name ='电商退款用户池'  then 'IP退款订单复用'
...
when lf.third_department_name in ('直播部','新媒体内容运营部','市场一组') and (lf.flow_pool_name like '%海淀高阶名师%' or lf.flow_pool_name like '%海淀老师高阶%' or lf.flow_pool_name like '%小艺%' or lf.flow_pool_name like '%老蒋文化%') then '郭艺'
when lf.third_department_name in ('直播部','新媒体内容运营部','市场一组') and lf.flow_pool_name like '%电商退款%' and lf.rule_name like '%郭艺99%' then '郭艺'
...
when lf.third_department_name = '直播部' and (lf.sku_id_name like '%朱博士99%' or lf.rule_name like '%朱汉祺99%' or lf.rule_name like '%朱博士%' or lf.flow_pool_name like '%朱博士%' or lf.sku_id_name like '%朱博士%') and lf.channel_name_2 = '视频号' then '朱博士-视频号49'
...
when lf.channel_name_1 = '信息流' and lf.channel_name_2='B站' and (lf.page_id_name like '%赵星义%') then 'B站信息流-赵星义'
when lf.channel_name_1 = '信息流' and lf.channel_name_2='B站' and (lf.page_id_name like '%亚飞%' or lf.source_put_plan_name like '%亚飞%'   or lf.page_id_name like '%初中-0元%') then 'B站信息流-亚飞'
when (lf.flow_pool_name like '%朱博士%' or lf.flow_pool_name like '%双博士%' or lf.flow_pool_name like '%教育规划%') and lf.third_department_name <> '线上商务部' and lf.period_name not like '%多学科拓展%' and lf.rule_name not like '%张杰%' and lf.sku_id_name not like '%马凯鹏IP%' and lf.third_department_name='直播部' then '朱博士29'
...
when lf.channel_name_1 = '信息流' and lf.channel_name_2 = 'B站' and lf.third_department_name not like '%投放%' and lf.page_id_name like '%肖晗%' then 'B站信息流'
...
when  lf.source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪','曹蕊07','曲默晗') and (lf.sku_id_name like '%帅师%' or lf.rule_name like '%周帅%' or lf.sku_id_name like '%9月升高三%' or lf.sku_id_name like '%新高二%') then 'KOC-周帅数学'
when  lf.source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪','曹蕊07','曲默晗') and (lf.sku_id_name like '%肖晗%' or lf.rule_name like '%肖晗%') then 'KOC-肖晗'
when  lf.source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪','曹蕊07','曲默晗') and lf.period_name not like '%多学科拓展%' and lf.sku_id_name not like '%朱汉祺%' and lf.sku_id_name not like '%朱博士%' and lf.sku_id_name not like '%周帅%' and lf.sku_id_name not like '%29元%' then 'KOC-5元纯课'
when  lf.source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪','曹蕊07','曲默晗') and lf.sku_id_name like '%周帅%' then 'KOC-周帅'
...
when lf.source_manager_name in ('包青青','蔡瑞涵','李文迁','李佳馨44','孙昊17','王洁雅01','王硕北','朱文','贾铭锐','李壮壮04','陈晓菁04','赵艺雅') and lf.channel_name_2 like '%社群%' then '进校社群'
when lf.source_manager_name in ('包青青','蔡瑞涵','李文迁','李佳馨44','孙昊17','王洁雅01','王硕北','朱文','贾铭锐','李壮壮04','陈晓菁04','赵艺雅') and (lf.channel_name_2 like '%直推%' or (lf.put_plan_name like '%良师益友%' or lf.put_plan_name like '%弘诚1元%')) then '进校直推'
when lf.source_manager_name in ('包青青','蔡瑞涵','李文迁','李佳馨44','孙昊17','王洁雅01','王硕北','朱文','贾铭锐','李壮壮04','陈晓菁04','赵艺雅') and lf.channel_name_2 like '%TMK%' and (lf.page_id_name like '%1元%' or lf.channel_provider_name like '%禾顺云%') then '进校TMK1元'
when lf.source_manager_name in ('包青青','蔡瑞涵','李文迁','李佳馨44','孙昊17','王洁雅01','王硕北','朱文','贾铭锐','李壮壮04','陈晓菁04','赵艺雅') and lf.channel_name_2 like '%TMK%' and lf.page_id_name like '%9元%' then '进校TMK9元'
...
when lf.get_customer_way_name in ('进校直推','线下渠道商') then '河南进校'
...
,coalesce(
    ...
    case
        ...
        then '20260808期'
        when regexp_like(rr.group_period_term, '^[0-9]{8}期$')
             and cast(date_parse(substr(rr.group_period_term, 1, 8), '%Y%m%d') as date) between date '2026-08-13' and date '2026-08-18'
        then '20260815期'
        ...
        when regexp_like(rr.group_period_term, '^[0-9]{4}期$')
             and substr(lead_gmv.qici, 1, 4) = '2026'
             and substr(rr.group_period_term, 1, 4) between '0813' and '0818'
        then '20260815期'
        when regexp_like(rr.group_period_term, '^[0-9]{8}期$')
        then date_format(
...
case
    ...
    when cast(date_parse(replace(concat(group_period_year, group_period_term), '期', ''), '%Y%m%d') as date) between date '2026-08-13' and date '2026-08-18' then '20260815期'
...
select
    b.*,
    case
        when b.qici = b.friday_period then 0
        when b.period_gap = 7 then 1
        when b.period_gap between 14 and 21 then 2
        when b.period_gap = -7 then 4
        -- 暑期为 6 天一期，按期次相邻关系识别下期/上期，不能只按自然日 7 天判断
        when (b.friday_period = '20260716期' and b.qici = '20260722期')
          or (b.friday_period = '20260722期' and b.qici = '20260728期')
          or (b.friday_period = '20260728期' and b.qici = '20260803期')
          or (b.friday_period = '20260803期' and b.qici = '20260808期')
          or (b.friday_period = '20260808期' and b.qici = '20260815期')
          or (b.friday_period = '20260815期' and b.qici = '20260821期')
        then 1
        when (b.friday_period = '20260716期' and b.qici = '20260728期')
          or (b.friday_period = '20260722期' and b.qici = '20260803期')
          or (b.friday_period = '20260728期' and b.qici = '20260808期')
          or (b.friday_period = '20260803期' and b.qici = '20260815期')
          or (b.friday_period = '20260808期' and b.qici = '20260821期')
        then 2
        when (b.friday_period = '20260722期' and b.qici = '20260716期')
          or (b.friday_period = '20260728期' and b.qici = '20260722期')
          or (b.friday_period = '20260803期' and b.qici = '20260728期')
          or (b.friday_period = '20260808期' and b.qici = '20260803期')
          or (b.friday_period = '20260815期' and b.qici = '20260808期')
          or (b.friday_period = '20260821期' and b.qici = '20260815期')
        then 4
        else 3
    end as week_diff
from (
