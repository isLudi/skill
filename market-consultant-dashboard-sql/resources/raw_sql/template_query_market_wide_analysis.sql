...
,src AS (
    ...
    where t1.dt = format_datetime(now() - interval '2' hour, 'YYYYMMdd')
      ...
      and (concat(cast(date_format(date_add('day',4,date_trunc('week',date_add('day',-1,date_parse(replace(concat(group_period_year,group_period_term),'期',''),'%Y%m%d')))),'%Y%m%d')as varchar),'期')) >= ${period_name1}
      and (concat(cast(date_format(date_add('day',4,date_trunc('week',date_add('day',-1,date_parse(replace(concat(group_period_year,group_period_term),'期',''),'%Y%m%d')))),'%Y%m%d')as varchar),'期')) < ${period_name2}
)
,src_keyed AS (
    select
        s.*,
...
-------------------业绩订单明细--科目数
,dd as (select
d.qici,d.user_id,d.employee_email_name,count(distinct d.course_subject ) as subject
 from 
	(select *,
        case 
    ...
    where dt = FORMAT_DATETIME(NOW() - INTERVAL '2' HOUR,'YYYYMMdd')
        ...
        and real_price <> 0) d
    where d.qici >=  ${period_name1} and d.qici < ${period_name2}
	group by d.qici,d.user_id,d.employee_email_name	)
-----------------------------整合 
,base as 
(
    select  
...
case when data.first_call_time_diff_hour >= 0 and data.first_call_time_diff_hour <= 24 and data.valid_lead_count > 0 then 1 else 0 end as first_call_in_24h,
	...
	where period_name >= ${period_name1} and period_name < ${period_name2}
)
