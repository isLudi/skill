...
src_keyed as ( select s.*, concat(case when s.period_name is null then 'N' else concat('V', cast(length(cast(s.period_name as varchar)) as varchar), ':', cast(s.period_name as varchar)) end, '|', case when s.rule_name is null then 'N' else concat('V', cast(length(cast(s.rule_name as varchar)) as varchar), ':', cast(s.rule_name as varchar)) end, '|', case when s.flow_pool_name is null then 'N' else concat('V', cast(length(cast(s.flow_pool_name as varchar)) as varchar), ':', cast(s.flow_pool_name as varchar)) end, '|', case when s.sku_id_name is null then 'N' else concat('V', cast(length(cast(s.sku_id_name as varchar)) as varchar), ':', cast(s.sku_id_name as varchar)) end, '|', case when s.ad_account_name is null then 'N' else concat('V', cast(length(cast(s.ad_account_name as varchar)) as varchar), ':', cast(s.ad_account_name as varchar)) end, '|', case when s.source_manager_name is null then 'N' else concat('V', cast(length(cast(s.source_manager_name as varchar)) as varchar), ':', cast(s.source_manager_name as varchar)) end, '|', case when s.channel_name_1 is null then 'N' else concat('V', cast(length(cast(s.channel_name_1 as varchar)) as varchar), ':', cast(s.channel_name_1 as varchar)) end, '|', case when s.channel_name_2 is null then 'N' else concat('V', cast(length(cast(s.channel_name_2 as varchar)) as varchar), ':', cast(s.channel_name_2 as varchar)) end, '|', case when s.channel_name_3 is null then 'N' else concat('V', cast(length(cast(s.channel_name_3 as varchar)) as varchar), ':', cast(s.channel_name_3 as varchar)) end, '|', case when s.put_plan_name is null then 'N' else concat('V', cast(length(cast(s.put_plan_name as varchar)) as varchar), ':', cast(s.put_plan_name as varchar)) end, '|', case when s.flow_original_order_activity_price is null then 'N' else concat('V', cast(length(cast(s.flow_original_order_activity_price as varchar)) as varchar), ':', cast(s.flow_original_order_activity_price as varchar)) end, '|', case when s.flow_orders_income_amount is null then 'N' else concat('V', cast(length(cast(s.flow_orders_income_amount as varchar)) as varchar), ':', cast(s.flow_orders_income_amount as varchar)) end, '|', case when s.flow_order_price is null then 'N' else concat('V', cast(length(cast(s.flow_order_price as varchar)) as varchar), ':', cast(s.flow_order_price as varchar)) end, '|', case when s.channel_provider_name is null then 'N' else concat('V', cast(length(cast(s.channel_provider_name as varchar)) as varchar), ':', cast(s.channel_provider_name as varchar)) end, '|', case when s.channel_second_provider_name is null then 'N' else concat('V', cast(length(cast(s.channel_second_provider_name as varchar)) as varchar), ':', cast(s.channel_second_provider_name as varchar)) end, '|', case when s.page_id_name is null then 'N' else concat('V', cast(length(cast(s.page_id_name as varchar)) as varchar), ':', cast(s.page_id_name as varchar)) end, '|', case when s.source_put_plan_name is null then 'N' else concat('V', cast(length(cast(s.source_put_plan_name as varchar)) as varchar), ':', cast(s.source_put_plan_name as varchar)) end, '|', case when s.get_customer_way_name is null then 'N' else concat('V', cast(length(cast(s.get_customer_way_name as varchar)) as varchar), ':', cast(s.get_customer_way_name as varchar)) end, '|', case when s.first_department_name is null then 'N' else concat('V', cast(length(cast(s.first_department_name as varchar)) as varchar), ':', cast(s.first_department_name as varchar)) end, '|', case when s.second_department_name is null then 'N' else concat('V', cast(length(cast(s.second_department_name as varchar)) as varchar), ':', cast(s.second_department_name as varchar)) end, '|', case when s.third_department_name is null then 'N' else concat('V', cast(length(cast(s.third_department_name as varchar)) as varchar), ':', cast(s.third_department_name as varchar)) end, '|', case when s.virtual_second_department_name is null then 'N' else concat('V', cast(length(cast(s.virtual_second_department_name as varchar)) as varchar), ':', cast(s.virtual_second_department_name as varchar)) end, '|', case when s.virtual_fourth_department_name is null then 'N' else concat('V', cast(length(cast(s.virtual_fourth_department_name as varchar)) as varchar), ':', cast(s.virtual_fourth_department_name as varchar)) end, '|', case when s.virtual_fifth_department_name is null then 'N' else concat('V', cast(length(cast(s.virtual_fifth_department_name as varchar)) as varchar), ':', cast(s.virtual_fifth_department_name as varchar)) end, '|', case when s.trace_type_name is null then 'N' else concat('V', cast(length(cast(s.trace_type_name as varchar)) as varchar), ':', cast(s.trace_type_name as varchar)) end, '|', case when s.lead_purchase_intention_level1_category_name is null then 'N' else concat('V', cast(length(cast(s.lead_purchase_intention_level1_category_name as varchar)) as varchar), ':', cast(s.lead_purchase_intention_level1_category_name as varchar)) end, '|', case when s.lead_purchase_intention_level2_category_name is null then 'N' else concat('V', cast(length(cast(s.lead_purchase_intention_level2_category_name as varchar)) as varchar), ':', cast(s.lead_purchase_intention_level2_category_name as varchar)) end, '|', case when s.lead_create_time is null then 'N' else concat('V', cast(length(cast(s.lead_create_time as varchar)) as varchar), ':', cast(s.lead_create_time as varchar)) end) as channel_key from src s ), channel_candidates as ( select s.channel_key, case
...
end as channel, 1 as rule_group from src_keyed s union all select s.channel_key, case
...
end as channel, 2 as rule_group from src_keyed s union all select s.channel_key, case
...
end as channel, 3 as rule_group from src_keyed s union all select s.channel_key, case
...
end as channel, 4 as rule_group from src_keyed s union all select s.channel_key, case
...
when rule_name like '%途途私域%' or (rule_name like '%私域%' and first_department_name = 'TT') then '途途私域'
...
end as channel, 5 as rule_group from src_keyed s ), channel_pick as ( select channel_key, min_by(channel, rule_group) as channel from channel_candidates where channel is not null group by channel_key ), src_mapped as ( select s.*, coalesce(c.channel, '其他未知流量') as channel_map from src_keyed s left join channel_pick c on s.channel_key = c.channel_key ), data as (
    select
        period_name,
        channel_map,
        case
    ...
    from src_mapped
    where period_name >= ${qici:1} and period_name < ${qici:2}
),
user_base as (
    select
        period_name,
