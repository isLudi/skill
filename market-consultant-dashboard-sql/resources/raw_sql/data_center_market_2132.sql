...
learn_raw as (
    select
        ...
        t.lesson_index_add,
        t.begin_time,
        cast(t.begin_time as date) as begin_date,
...
lead_user_period as (
    select
        ...
        channel_map_1,
        grade_1,
        cast(date_parse(substr(qici, 1, 8), '%Y%m%d') as date) as qici_date
    from data
    group by
        lead_id,
        user_id,
        ...
        channel_map_1,
        grade_1,
        cast(date_parse(substr(qici, 1, 8), '%Y%m%d') as date)
),
daoke as (
    select
        lup.qici,
        lup.employee_email_prefix,
        lup.lead_id,
        lup.user_id,
        lup.channel_map_1,
        lup.grade_1,
    ...
    from lead_user_period lup
    join ke_manual ke
      on lup.qici = ke.qici
     and lup.channel_map_1 = ke.qudao
     and lup.grade_1 = ke.grade
    left join learn_raw lr
      on cast(lup.user_id as varchar) = cast(lr.user_number as varchar)
     and date_format(cast(lr.begin_time as timestamp), '%Y-%m-%d %H:%i:00') = ke.begin_time
     and lr.begin_date >= lup.qici_date - interval '3' day
     and lr.begin_date < lup.qici_date + interval '4' day
),
prc as (
    select distinct
        data.qici,
        ...
        case when sum(case when daoke.ke_1 = '1' and cast(daoke.is_valid_live_learn as varchar) = '1' then 1 else 0 end) > 0 then 1 else 0 end as v_ke_1,
        case when sum(case when daoke.ke_1 = '2' and cast(daoke.is_valid_live_learn as varchar) = '1' then 1 else 0 end) > 0 then 1 else 0 end as v_ke_2,
        case when sum(case when daoke.ke_1 = '3' and cast(daoke.is_valid_live_learn as varchar) = '1' then 1 else 0 end) > 0 then 1 else 0 end as v_ke_3,
        case when sum(case when daoke.ke_1 = '4' and cast(daoke.is_valid_live_learn as varchar) = '1' then 1 else 0 end) > 0 then 1 else 0 end as v_ke_4,
        case when sum(case when daoke.ke_1 = '5' and cast(daoke.is_valid_live_learn as varchar) = '1' then 1 else 0 end) > 0 then 1 else 0 end as v_ke_5,
        case when sum(case when daoke.ke_1 = '6' and cast(daoke.is_valid_live_learn as varchar) = '1' then 1 else 0 end) > 0 then 1 else 0 end as v_ke_6
    from data
    left join daoke
      on data.employee_email_prefix = daoke.employee_email_prefix
     and data.qici = daoke.qici
     and data.lead_id = daoke.lead_id
     and cast(data.user_id as varchar) = cast(daoke.user_id as varchar)
     and data.channel_map_1 = daoke.channel_map_1
     and data.grade_1 = daoke.grade_1
