...
data as (
  ...
  when ( ( period_mapping_second_level_department_name = '本地化大班学部' and rule_name like '浙江-%期-领跑新初三%' and sku_id_name like '%沪浙-%【浙江初三】领跑%' and ( sku_id_name like '%数学英语物理化学%' or sku_id_name like '%数学英语物理生化%' ) ) or rule_name like '%浙江%曹忆科学%' or flow_pool_name like '%浙江科学曹忆%' or flow_pool_name like '%为真-浙江科学曹忆老师%' or (put_plan_name like '%浙江%' and put_plan_name like '%曹忆%' and lower(put_plan_name) like '%ip%') or (source_put_plan_name like '%浙江%' and source_put_plan_name like '%曹忆%' and lower(source_put_plan_name) like '%ip%') or (sku_id_name like '%浙江%' and sku_id_name like '%科学IP%') or sku_id_name like '%浙江初三科学IP%' ) then '浙江科学IP'
  ...
  when section_assign_employee_second_level_department_name = '青橙项目部' and third_department_name = '私域运营部' and channel_name_1 = '市场私域' and channel_name_2 = '抖音' and flow_pool_name = '抖音私域专用流量池' then '抖音私信'
