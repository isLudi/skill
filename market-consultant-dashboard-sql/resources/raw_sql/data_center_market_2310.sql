...
employee_map as (
    ...
    from (
        ...
        where dt = format_datetime(now() - interval '24' hour, 'YYYYMMdd')
          ...
          and second_level_department_name in ('市场部','精品班学部','青橙项目部','菁英班学部','本地化大班学部')
    ) t
    where rn = 1
),
f_call0 as (
...
data_with_process as (
    select
        d.*,
        case
            when d.valid_lead_count = 1 then coalesce(f.call_answer_lead_count, 0)
            else 0
        end as is_f_call
    from data d
    left join f_call0 f
...
select
    ...
    zz.channel_map,
    zz.rule_name,
    zz.lead_purchase_intention_level2_category_name,
    zz.depart_1,
    zz.dept_name,
    zz.depart,
    zz.lead_count,
    zz.can_renew_ds_count_a,
    zz.first_call_in_24h,
    zz.valid_lead_count,
    zz.is_f_call,
    zz.is_friend_lead,
    zz.pay_users,
