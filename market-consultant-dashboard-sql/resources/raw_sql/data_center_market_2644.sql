...
process as (
    select
        case
            when try_cast(substring(pg.qici, 5, 2) as int) between 1 and 6
                then concat(substring(pg.qici, 1, 4), '上半年')
            when try_cast(substring(pg.qici, 5, 2) as int) between 7 and 12
                then concat(substring(pg.qici, 1, 4), '下半年')
            else '其他'
        end as half_year,
        pg.qici,
...
period_agg as (
    select
        ...
        sum(ref) as ref
    from process
    group by half_year, employee_email_name
),
period_profile as (
    select
        half_year,
    ...
    from (
        select
            ...
            xiaozu,
            row_number() over (
                partition by half_year, employee_email_name
                order by qici desc
            ) as rn
        from process
    ) t
...
rank_h as (
    ...
    left join period_profile p
      on p.half_year = a.half_year
     and p.employee_email_name = a.employee_email_name
),
rk_r as (
    select
        *,
        rank() over (partition by half_year order by roi desc) as rank_in_roi
    from rank_h
),
ref_rank as (
    select
...
select
    ...
    total_period_count,
    is_join_test_channel,
    is_test_gmv_rate_over_80,
