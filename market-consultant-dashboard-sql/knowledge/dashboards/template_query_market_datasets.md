...
- 同步策略：raw SQL 保存平台模板取数当前发布版本；治理变更必须原位更新远端、发布并回读一致后，再用 `sync-template-sql` 按线上 SQL SHA-256 原位覆盖 stable canonical 文件并清理日期副本；不用模板 SQL 覆盖数据中心或看板 canonical SQL。
- 口径说明：本清单 SQL 的使用口径统一标记为 `模板取数`。若与数据中心或看板 SQL 同名/同类，默认先按来源区分，不自动互相替代。
- 2026-08-06 0805 版本：8 个既有市场顾问部模板（`5962, 6529, 7689, 7808, 8006, 8101, 8882, 9002`）全部原位 Apply、独立 Publish，并在发布后读回为 `status=2`；模板 id、权限/申请关系和输出字段保持不变，线上 SQL hash 与候选 hash 全部一致。
- 2026-08-07 编译优化版本：`9002`、`8882` 继续使用原模板 id 原位更新并独立发布；两者均保留 170 条 0805 渠道规则的逐条顺序，改为五组连续 CASE + `UNION ALL` + `min_by(rule_group)`，并以无碰撞长度前缀键回连源行。`8882` 原本无参数，本次按模板发布门禁新增 `${qici:1}`/`${qici:2}` 左闭右开期次参数；22 个输出字段、已有申请关系和权限主体不变。
...
## 当前模板清单（0805 规则 / 0807 编译优化）

| 模板名称 | 模板 id | 状态 | 发布后 SQL SHA-256 | raw SQL | SQL 行数/字节 | 模板参数 | 发布后验证 |
|---|---:|---|---|---|---:|---|---|
| 市场部_首call率 | 5962 | published | `1795d0a588701f4e43d5d7e1fef529f108b6e2ca70cc1967d132e07240b78e66` | [`template_query_market_first_call_rate.sql`](../../resources/raw_sql/template_query_market_first_call_rate.sql) | 444 / 51228 | `${group_period_name}` | Apply/Publish/线上回读一致 |
| 市场顾问部_触达_沟通UID明细 | 6529 | published | `65bf87319c2d9acb7c2b7d0274e99e5cb3809ab9f3fa912f5f8a75327099eb26` | [`template_query_market_contact_uid_detail.sql`](../../resources/raw_sql/template_query_market_contact_uid_detail.sql) | 280 / 41316 | `${group_period_name}`, `${begin_day}`, `${end_day}` | Apply/Publish/线上回读一致 |
| 业财用户出单明细 | 7689 | published | `2339f22aa2f903f7712ed25a08f55b6cabebdeae92739650092f5e1bcd46ad70` | [`template_query_market_finance_order_detail.sql`](../../resources/raw_sql/template_query_market_finance_order_detail.sql) | 246 / 40399 | `${dt}` | Apply/Publish/线上回读一致 |
| 市场运营专用_多维全链路分析 | 7808 | published | `109fd2a64409221900e22df7f1555f5bfdef14c629ca9caae2117cafd726e72f` | [`template_query_market_wide_analysis.sql`](../../resources/raw_sql/template_query_market_wide_analysis.sql) | 589 / 57780 | `${period_name1}`, `${period_name2}` | Apply/Publish/线上回读一致 |
| B转A用户明细 | 8006 | published | `3bba3d0f8926ac349c79011f53f27e8c3c45359206ef1810f0b1d0401cd83327` | [`template_query_market_b_to_a_lead_detail.sql`](../../resources/raw_sql/template_query_market_b_to_a_lead_detail.sql) | 358 / 46798 | `${dt}` | Apply/Publish/线上回读一致 |
| 市场_B转A过程_转化数据 | 8101 | published | `e7d48b084ec5f28331e667ba1f333d5fc3bbf08224e4a6b05d7405c89f55f07d` | [`template_query_market_b_to_a_conversion.sql`](../../resources/raw_sql/template_query_market_b_to_a_conversion.sql) | 421 / 49900 | `${period_name}` | Apply/Publish/线上回读一致 |
| AI分析市场顾问部多科用户成单数据 | 8882 | published | `19804a820cb1a6dbd17e6b2fb7dde4747ae722df911fc6f5704c0b910bbf5fc6` | [`template_query_market_multi_subject_order_user.sql`](../../resources/raw_sql/template_query_market_multi_subject_order_user.sql) | 175 / 49506 | `${qici:1}`, `${qici:2}` | query `1535349113`，Result API `success_with_rows`，22 列，约 33.656 秒 |
| AI分析市场顾问部_宽表 | 9002 | published | `8d8391d22112ccc6bb9a46bd24bb92eb520fd0ee34f2206e7b89708baac08c10` | [`template_query_market_wide.sql`](../../resources/raw_sql/template_query_market_wide.sql) | 1 / 79233 | `${qici:1}`, `${qici:2}` | query `1535311064`，Result API `success_with_rows`，122 列，约 109.578 秒 |
...
| 业财用户出单明细 | 7689 | published | 2026-08-03 20:06:04 | 模板取数 | 历史代码已清理；当前入口为 [`template_query_market_finance_order_detail.sql`](../../resources/raw_sql/template_query_market_finance_order_detail.sql) | - | - | `bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df` | `${dt}` | 按运营期次输出市场顾问线索、转化、订单、收退款和渠道明细；`channel_map` 现以 0805 线上发布版本为准。 | 历史查询 `384631` 为 `SUCCESS`；渠道 CASE 更新继续同步模板 7689。 |
