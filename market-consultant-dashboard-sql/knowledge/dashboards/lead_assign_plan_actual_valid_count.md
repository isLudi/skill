...
如果需要将规则侧或实际量侧按最新渠道口径归因，可引用 `resources/raw_sql/market_channel_case_when_0904.sql` 中的渠道 CASE。该 CASE 输出别名为 `qudao`，说明见 `knowledge/sql_patterns/channel_mapping_case_when.md`。
...
- 分配规则表本身没有完整渠道 CASE 依赖字段；若要按最新渠道口径识别“抖音私信”等渠道，应优先在全链路宽表中按 `market_channel_case_when_0904.sql` 派生 `qudao/channel_map`，或确认规则表是否具备 CASE 所需字段。
