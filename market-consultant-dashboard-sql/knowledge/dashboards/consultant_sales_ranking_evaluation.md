...
| 渠道 CASE | 使用 `channel` 字段（来自评优临时表） | 历史 CASE（pre-0524 版本，不含后续 0524/0612/0723 渠道细分，信息流-抖音私信 位于旧位置） |
...
- channel_map CASE 版本仍需确认是否同步最新 `market_channel_case_when_0805.sql`；如与转化看板在同一前端展示，会出现渠道口径不一致。
