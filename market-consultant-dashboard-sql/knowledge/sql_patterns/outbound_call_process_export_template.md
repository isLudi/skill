...
用于从外呼过程数据看板口径导出指定期次范围的数据。该模板来自用户提供的最新外呼过程查询代码，不写入 `resources/raw_sql/`；仅作为可复制、可替换参数的取数模板。

## 2. 模板参数

| 参数 | 含义 | 替换示例 |
|---|---|---|
| `${period_name1}` | 起始期次，包含该期 | `'20260529期'` |
| `${period_name2}` | 结束期次，不包含该期 | `'20260612期'` |

期次范围使用半开区间：`qici >= ${period_name1}` 且 `qici < ${period_name2}`。执行前必须由查询平台或人工替换参数；不要把 `${period_name1}`、`${period_name2}` 原样提交。

## 3. 关键口径

- 主数据驱动表：`bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df`。
- 架构范围：`temp_table.dingxi01_jiagou_db` 同步使用 `${period_name1}` / `${period_name2}` 限制 `qici`。
- 最终导出范围：`prc` CTE 中用 `data.qici >= ${period_name1}` 且 `data.qici < ${period_name2}` 控制。
- 首 call 指标继续使用 `gaotu_crm_offline_statistics.app_mcrm_first_call_task_hf + finance_dw.dim_finance_employee_df`，不得回退到旧过程表。
- 查询平台规则：本模板已使用 `interval` 日期偏移写法，不使用三参数 `date_add('day', n, expr)`。
...
jg_market as (
    ...
    from (
        ...
        where jg.qici >= ${period_name1}
          and jg.qici < ${period_name2}
          and jg.dept_1 = '市场顾问部'
...
prc as (
    ...
    where data.qici >= ${period_name1}
      and data.qici < ${period_name2}
...
- `${period_name1}` 和 `${period_name2}` 需要替换为带引号的期次字符串，例如 `'20260529期'`。
- 如果只导出单期，例如 `20260529期`，建议设置 `${period_name1} = '20260529期'`、`${period_name2} = '20260605期'`。
- 最终输出粒度为 `qici + rule_name + channel_map_1 + grade_1 + department + jingli + xiaozu + employee_email_name + datt`。
- 前端或 Excel 后处理计算比率时，应使用聚合后的分子/分母重新计算，不要对行级比率求和。