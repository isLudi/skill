...
- 0918 版本原始来源：飞书多维表格 `20260918期`（记录 `recvtOopW7sPfH`，Base `O2Rdb1v58ac1VTs6PsrcWtZHnSh`，表 `tblCiP801wWblywT`，视图 `vewrdx1WO6`）
- Skill 最新归档：`resources/raw_sql/market_channel_case_when_0918.sql`
- 0918 与 0904 逐分支融合：以 0918 原始完整顺序为准，结合线上消费者现状审计新增、删除、替换及 first-match 重叠；不按固定行号或机械切片替换
- 0918 融合 CASE：179 个 `WHEN` 分支、123 个结构化解析后的去重渠道输出值；归档文件 SHA-256 `db88ab003aa18607f205a07663305fa05bc67dac415089aac3c62050691a884f`。统计输出值时必须按 CASE 结构解析，不能用会把嵌套 CASE 一并计入的简单 `THEN` 正则。
- 最近共享 CASE 修正日期：2026-09-18；Data Center 定向覆盖继续保留 2026-08-01/02 已确认边界。
- 输出字段：`qudao`

### 0904 → 0918 融合审计

- 0918 为 179 个分支，0904 为 183 个分支；公共规则相对顺序保持不变。唯一扩展是本地化市场规则的负责人名单加入 `喻新娇`。
- 删除 4 条已撤销业务规则：`进校直推/线下渠道商 → 河南进校`、小写 `app`/`M站 → app`、`微信私域 → 集团私域`，以及七个指定流量池且 SKU 命中曹忆的平台自播规则。删除后的宽规则不保留无效排除条件。
- 0904 的 5 个前置宽规则中，随上述删除仅剩 3 个；这 3 个继续显式排除后续 8 条“进校私域合作”细分候选，保持 first-match 归因准确。
- 当前远端共 22 个完整 CASE Data Center 消费者，全部完成 SQL 哈希回读和新的 `SUCCESS`。历史模型 `2533` 已从远端删除，未重建并从 current binding 退役；`2885` 触发后延迟建单，最终运行 `171482346` 成功。22 个当前调度均启用且 `isExpire=false`，无需续期。
- 6 个永久模板（`7689, 7808, 8735, 8882, 8948, 9002`）均保留原 ID 原位发布，发布后真实查询 `401698/401699/401700/401701/401702/401706` 分别返回 `10126/8606/77/155/110/586` 行且全部成功；既有申请与权限关系不变。
- 超长模板采用五组连续 CASE（36/36/36/36/35）、`UNION ALL` 和 `min_by(channel, rule_group)` 保持 first-match；模板 `9002` 保留 28 字段无碰撞键适配，查询 142 秒完成，未出现 504。
...
- `resources/raw_sql/market_channel_case_when_0918.sql` 是通用共享基线；对上述当前 Data Center 模型做整体 CASE 替换时，必须在共享基线上继续保留本定向覆盖，不能因重新粘贴共享片段而丢失。
...
生成或改写市场顾问转化、线索转化到课、外呼过程、分配计划实际有效量或馒头模板等 SQL 时，如果需要“最新渠道 CASE”，优先引用 `resources/raw_sql/market_channel_case_when_0918.sql`，不要直接照抄旧看板中的长 CASE。

### 3.1 模板取数登记消费者与同步授权

- 当前有效的 6 个渠道模板（`7689, 7808, 8735, 8882, 8948, 9002`）均必须与 0918 共享渠道 CASE 保持规则顺序、条件和输出值一致；目标别名仅按模板字段做等价适配。`8735/8948` 两个馒头模板永久属于市场顾问域，后续无需再次声明。
...
- 当前 2026-08-01/02 两条退款批次规则仍属于指定 Data Center 模型的定向覆盖，不是共享 `market_channel_case_when_0918.sql` 的组成部分；只有业务明确将其提升为共享规则或明确要求模板吸收时，才同步进模板，避免把模型定向条件误当作共享 CASE。
...
with base as (
    select
        ...
        t.rule_name,
        -- 粘贴 resources/raw_sql/market_channel_case_when_0918.sql
