...
- 每次更新 `market_channel_case_when_MMDD.sql` 时，必须读取上述有效模板、做规则级差异比较、原位保存并发布同一模板 ID、回读 SQL/参数/最终字段，再发起一次真实模板查询；已有申请关系不得因重建模板而失效。远端已删除模板直接移除 current route，不恢复、不再同步。
...
5. 将 3.1 登记的 6 个有效模板作为强制同步消费者；其中 `7689`、`9002` 还要同步检查暑期运营期次。按原模板 ID 更新、发布、回读、查询验证，并用线上回读 SQL 刷新 stable raw SQL 和模板清单。
