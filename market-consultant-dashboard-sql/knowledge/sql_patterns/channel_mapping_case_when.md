...
- 最近共享 CASE 更新日期：2026-07-26；Data Center 定向覆盖补充日期：2026-08-01
...
- 修正前精确批次 `sum(lead_count)=1444`，其中 `KOC-周帅数学=1121`、`KOC-孟亚飞数学=323`；1442 计入既有退款池/退款计划匹配，另 2 条未命中该物理池条件，但业务确认整批均为退款复用。修正后 query `1514422403` 验证 1444 全部归入 `退款订单复用`，两个错误 KOC 渠道残留为 0；历史 `20260728期` 的周帅 KOC 209 未受影响。
...
- 0728 期退款复用定向覆盖必须先于 `KOC-孟亚飞数学` 和 `KOC-周帅数学` 分支；其四组批次边界必须完整保留，避免误伤其他期次的真实 KOC 线索。
