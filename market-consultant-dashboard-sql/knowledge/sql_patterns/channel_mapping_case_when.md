...
- 0904 版本原始来源：飞书多维表格 `市场顾问部0904最新`（记录 `recvshnqnG0Jl2`，表 `tblCiP801wWblywT`，视图 `vewrdx1WO6`）
- Skill 最新归档：`resources/raw_sql/market_channel_case_when_0904.sql`
- 0904 与 0808 逐分支融合：保留 0904 原始完整顺序，审计新增、删除、替换和公共分支移位；不按固定行号或机械切片替换
- 0904 修正版精确执行 CASE：183 个 `WHEN` 分支，127 个去重渠道输出值；不含文件头异常说明的执行体 SHA-256 `86331bf02acd99268ca8de485f03132b02350bfa4b0f4fb3b299301a989da91b`，含说明的归档文件 SHA-256 `bc858ad4b404ba724bdb23a7f6e3e55d607b6295b512d35d4706f9958f9005ab`。
- 最近共享 CASE 修正日期：2026-09-10；Data Center 定向覆盖继续保留 2026-08-01/02 已确认边界。
- 输出字段：`qudao`

### 0808 → 0904 融合审计

- 0904 相比 0808：183 个唯一分支对 175 个唯一分支，新增 12 个、删除或被替换 4 个、公共 171 个；公共规则发生顺序调整，因此以 0904 全序 first-match 为准。
- 新增/扩展包括 APP/集团私域、浙江图书、2000题/五年真题/全国教考、私域名单、陈瑞春 B站/视频号、B站信息流 1元等映射。完整选择来自飞书 0904 记录，不从 0808 CASE 机械切片。
- 23 个仍在线且使用渠道归因的 Data Center 数据集均已完成 SQL 哈希回读、预览和新 `SUCCESS`；其中重新配置的 `2812/2886/2890` 最新运行分别为 `169247943/169247946/169247954`。
- 6 个永久模板均保持原模板 ID 原位更新并发布，发布后真实查询 `397410..397415` 全部 `SUCCESS`；既有申请与权限关系不变。
- 超长模板采用五组连续 CASE（37/37/37/36/36）和 `min_by(rule_group)` 保持 first-match；数据中心保留完整有序 CASE，并只做分区下推、去重源扫描、投影和无用 CTE/JOIN 清理等已验证优化。

### 2026-09-10 first-match 异常与防回归约束

- 异常：0904 新增/扩展的 5 条宽规则位于 8 条“进校私域合作”细分候选之前。SQL `CASE` 采用 first-match，导致细分候选先被归为“河南进校”或“集团私域”；Bitable 线索级明细仍有该渠道，是因为当时链路版本/顺序不同，不代表 BI 源线索缺失。
- 修复：保留全部 183 条 `WHEN` 和原业务顺序，仅在这 5 条宽规则上显式排除 8 条“进校私域合作”细分候选；不得用无条件调序改变其他重叠规则的 first-match。
- 更新门禁：以后同步新渠道 CASE 时，除分支数、输出数和 Hash 外，必须做宽规则/细分规则重叠审计、公共分支相对顺序审计，并以代表性 lead 逐条比较旧输出、候选输出和预期输出。若宽规则仍位于细分规则之前，必须保留等价排除条件。
- 20260911 验收：修复前同一时点对照 Query `1583081838` 识别出 137 条会从“河南进校/集团私域”回归“进校私域合作”；上线后的最终数量以各生产模型实际刷新分区为准，不把旧截面 137 硬编码为恒定业务量。

### 0805 → 0808 历史融合审计

- 0808 相比 0805：新增 15 个分支、删除 10 个既有分支；160 个公共分支相对顺序不变，同条件输出变化为 0，没有重复条件键。
- 新增/细分包含江苏直播、搜索 1 元、退款订单复用、安徽直播、自孵化 KOC 下引、陈瑞春和朱博士按媒体拆分等规则；删除的是被新边界替代或业务取消的旧规则。本条为 0808 历史审计，完整选择以当时原始顺序为准。
- 27 个完整 CASE 初始目标中，25 个仍在线的数据集均完成保存、SQL 哈希回读和新 `SUCCESS`；远端已删除的数据集 `3153/3202` 已从 current binding 和 canonical SQL 退役，不再同步。
...
- `resources/raw_sql/market_channel_case_when_0904.sql` 是通用共享基线；对上述当前 Data Center 模型做整体 CASE 替换时，必须在共享基线上继续保留本定向覆盖，不能因重新粘贴共享片段而丢失。
...
生成或改写市场顾问转化、线索转化到课、外呼过程、分配计划实际有效量或馒头模板等 SQL 时，如果需要“最新渠道 CASE”，优先引用 `resources/raw_sql/market_channel_case_when_0904.sql`，不要直接照抄旧看板中的长 CASE。

### 3.1 模板取数登记消费者与同步授权

- 当前有效的 6 个渠道模板（`7689, 7808, 8735, 8882, 8948, 9002`）均必须与 0904 共享渠道 CASE 保持规则顺序、条件和输出值一致；目标别名仅按模板字段做等价适配。`8735/8948` 两个馒头模板永久属于市场顾问域，后续无需再次声明。
- 当前发布 SQL 以线上模板回读文件和 `knowledge/dashboards/template_query_market_datasets.md` 登记为准。
- 更新 `market_channel_case_when_MMDD.sql` 时，将上述有效模板纳入影响分析，回读现状并准备规则级差异与待同步清单。本地知识维护不授权远端保存、发布或真实查询；只有对应模板及阶段已获明确授权，才按原模板 ID 执行计划、Apply、独立 Publish 与查询验证。保持已有申请关系，回读 SQL、参数和最终字段；未获授权的消费者标记“待同步”，不把本地完成误报为线上一致。已确认远端删除的模板在获授权的知识维护中移除 current route，不自动恢复。
- 当前 2026-08-01/02 两条退款批次规则仍属于指定 Data Center 模型的定向覆盖，不是共享 `market_channel_case_when_0904.sql` 的组成部分；只有业务明确将其提升为共享规则或明确要求模板吸收时，才同步进模板，避免把模型定向条件误当作共享 CASE。
...
### 2026-09-11 抖音私信历史修复与 2026-09-16 口径恢复
...
当时为修复 20260911 期，退前、退后改为按源投放经理强制取 merge 字段：
...
2026-09-16 的跨期排查发现，上游承载字段会按原始期次切换：20260911 期普通字段为 0、merge 字段有值；20260917 期普通字段有值、merge 字段明确为 0。`dt=20260916, hour=11` 中展示为 20260918 期的抖音私信 81 条记录，普通退前/退后为 78/78，merge 为 0/0。当前生产按恢复要求统一使用普通字段：

```sql
sum(coalesce(lead_count, 0)) as lead_count,
sum(coalesce(valid_lead_count, 0)) as valid_lead_count
```

验收门禁：

- 抖音私信渠道映射仍由渠道 CASE 决定；退前和退后不再用源投放经理选择 merge 字段，也不在指标层追加渠道一级、流量池等收窄条件。
- 同步回归依赖有效线索的加微、外呼、到课指标；验证目标集合和非目标渠道的增量，避免静默扩大影响。
- 普通口径会低估只在 merge 字段有值的历史批次。上游切换规则未确认前，不能将 20260911 期的 merge 验收结果误作当前通用合同。
...
with base as (
    select
        ...
        t.rule_name,
        -- 粘贴 resources/raw_sql/market_channel_case_when_0904.sql
...
5. 检查 3.1 登记消费者的影响；其中 `7689`、`9002` 还需检查暑期运营期次。按各目标、变更和阶段的已有授权执行同步；未授权阶段保留差异和待办。已授权远端变更通过回读与查询验证后，才把真实线上版本同步到获授权维护的 stable raw SQL 和模板清单。
