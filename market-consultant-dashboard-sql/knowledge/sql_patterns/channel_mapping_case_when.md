...
- 0808 版本原始来源：`C:\Users\Ludim\Desktop\新建 文本文档.txt`，读取时来源文件 SHA-256 `654eddfdf3bf0e4d334f0b1e976cb664dc9bc7ee6c0829603a4e3525bd2c6761`
- Skill 最新归档：`resources/raw_sql/market_channel_case_when_0808.sql`
- 0808 与 0805 逐分支融合：保留 0808 原始完整顺序，审计新增、删除、条件/输出变化及公共分支移位；不按固定行号切片替换
- 0808 精确执行 CASE：175 个 `WHEN` 分支，118 个去重后的渠道输出值；CASE 文本 SHA-256 `136accb16cf81580f2a2f97198a91225c174a5ee01b27ca6b9327ceac45df7e5`
- 最近共享 CASE 更新日期：2026-08-08；Data Center 定向覆盖继续保留 2026-08-01/02 已确认边界
- 输出字段：`qudao`

### 0805 → 0808 融合审计

- 0808 相比 0805：新增 15 个分支、删除 10 个既有分支；160 个公共分支相对顺序不变，同条件输出变化为 0，没有重复条件键。
- 新增/细分包含江苏直播、搜索 1 元、退款订单复用、安徽直播、自孵化 KOC 下引、陈瑞春和朱博士按媒体拆分等规则；删除的是被新边界替代或业务取消的旧规则。完整选择以 0808 原始顺序为准，不从旧 CASE 机械切片拼接。
- 27 个完整 CASE 初始目标中，25 个仍在线的数据集均完成保存、SQL 哈希回读和新 `SUCCESS`；远端已删除的数据集 `3153/3202` 已从 current binding 和 canonical SQL 退役，不再同步。
- 超长模板采用五组连续 CASE（5×35）和 `min_by(rule_group)` 保持 first-match；数据中心保留完整有序 CASE，并只做分区下推、去重源扫描、投影和无用 CTE/JOIN 清理等已验证优化。
...
- `resources/raw_sql/market_channel_case_when_0808.sql` 是通用共享基线；对上述当前 Data Center 模型做整体 CASE 替换时，必须在共享基线上继续保留本定向覆盖，不能因重新粘贴共享片段而丢失。
...
生成或改写市场顾问转化、线索转化到课、外呼过程、分配计划实际有效量或馒头模板等 SQL 时，如果需要“最新渠道 CASE”，优先引用 `resources/raw_sql/market_channel_case_when_0808.sql`，不要直接照抄旧看板中的长 CASE。

### 3.1 模板取数强制同步消费者

- 当前有效的 6 个渠道模板（`7689, 7808, 8735, 8882, 8948, 9002`）均必须与 0808 共享渠道 CASE 保持规则顺序、条件和输出值一致；目标别名仅按模板字段做等价适配。`8735/8948` 两个馒头模板永久属于市场顾问域，后续无需再次声明。
...
- 当前 2026-08-01/02 两条退款批次规则仍属于指定 Data Center 模型的定向覆盖，不是共享 `market_channel_case_when_0808.sql` 的组成部分；只有业务明确将其提升为共享规则或明确要求模板吸收时，才同步进模板，避免把模型定向条件误当作共享 CASE。
...
with base as (
    select
        ...
        t.rule_name,
        -- 粘贴 resources/raw_sql/market_channel_case_when_0808.sql
