...
- 使用 `usql-web-query-operator` 的 `fetch-template-sql` 命令从模板取数页面抓取 7 个已发布模板的最新 `sqlDetail`：多科用户成单、分周期转化、转化、过程、到课、员工架构和进量数据。
- 新增 7 份模板取数 raw SQL，完整保存平台模板 SQL 原文，不改写 SQL 语义，不覆盖现有 canonical 看板或数据中心 SQL。
...
## 2026-07-20 P4B 九类窄写能力路由同步

- 同步 operator 已在青橙 P4C 沙箱完成的九类可补偿写能力：字段显示名、局部筛选标签、组件标题、公共筛选标题、Tab 标签、布局、既有公式表达式、公共筛选动态默认项和根背景色。
- 市场顾问 DashboardChangePlan 仍须绑定本域完整 Profile 与精确 Hash；每项只允许稳定 ID 下的单槽位修改，含任一 blocked operation 时整份 Apply 零写入。
- 本次仅同步业务设计路由与门禁，未修改市场顾问看板草稿或发布版本；后续实际批量术语调整仍须重新画像、生成并审阅 ChangePlan 后单独授权 Apply。

## 2026-07-20 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`3039`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-07-20 运营侧新老人转化对比暑期期次修正

- 修正数据中心模型 `3039`（`新老人转化对比`）不完整的 `biz_qici_calendar`：补齐三个单日缺口，并新增 `20260803期`、`20260809期`，使 `2026-07-14` 至 `2026-08-12` 按 5 个标准暑期期次连续覆盖。
- `lead_period` 和 `class_period` 同步使用相同日期边界；未命中业务日历时才回退历史自然周逻辑，避免 `20260717期`、`20260731期` 混入看板期次维度。
- 真实源表探针 query `1487378714` 验证 `calendar_miss=0`、旧映射异常分组 9 行、新映射异常分组 0 行；`2026-07-31` 的 1 条线索由 `20260731期` 正确归入 `20260728期`。
- 生产替换预览 task `1487387588` 成功，保存后 SQL SHA-256 回读为 `7ce4650fe63fa8ee7e88e9f1a89d520a3449b0a3f7ddd2fd96b90a8e6f9bc83d`；立即执行新增同步记录 `159936713`，状态 `SUCCESS`，receipt 标记 `fully_verified=true`。

## 2026-07-20 Skill 只读与知识维护边界收敛

- 市场顾问 Web BI 批量画像默认改为 runtime-only；只有用户明确要求知识维护并显式运行 `profile-all --write-knowledge --confirm-skill-maintenance` 才可写固定的市场顾问知识目标。
- 画像知识目标不再接受命令行覆盖，青橙快照不得写入本 Skill；任一画像失败时整批知识写入被阻断。
- 本次仅更新本地 Skill 路由、README 和生成索引，未修改市场顾问业务口径、远端数据集或看板。

## 2026-07-21 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2688`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-07-21 新人过程转化 `x_qi_count` 重复与临时表上传故障知识化

- 新增 `temp_table.dingxi01_pingyou_jg` 当前表文档和专项 pitfalls，确认模型 `2688` 直接透传 `x_qi_count`：`1`—`4` 是顾问前四个有效新人承接期次，`9` 为排除/第五期及以后；重复根因来自 Excel 手工序号，不是最终 join 放大。
- 记录 Excel 维护公式、保留原有 `9` 的重排规则、蔡铁骑/耿玉琳示例，以及“终版不得新增字段”的粘贴为值流程。
- 记录两类上传故障：多 sheet 首次仅导入 63 行；6 个空 `grade` 行导致线上字段错位和 `qici='1'`。固定终版仅保留 `Sheet4`、14 列全非空、邮箱前缀小写，并以 overwrite 回执和线上唯一性回读作为门禁。
- 记录 2688 暑期日历修复：线索、开课和员工入部门日期角色统一覆盖 `20260716期` 至 `20260809期`；生产 SQL SHA-256 `617cc607e7f584d165042a26a66f1d4ea4875390e07c0f62bb2cb97e54ee89ab`，同步记录 `160164366` 为 `SUCCESS`。
- 同步更新表索引、快速参考、需求路由、指标联动、通用 join 踩坑和暑期期次规则；本次只维护市场顾问知识库，未修改或发布看板。

## 2026-07-23 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2054, 2350, 2421, 2632, 2643, 2644, 2742, 2978, 3153`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-07-23 市场顾问渠道 CASE 0723 版本维护

- 将最新渠道入口从 `resources/raw_sql/market_channel_case_when_0612.sql` 提升为 `resources/raw_sql/market_channel_case_when_0723.sql`；基线仍来自 `D:\Feishu\0612.txt`，本次增量来自用户明确提供的规则。
- 在所有受管渠道 CASE 的最前面新增 `rule_name like '%北京直播江苏%' then '北京直播江苏'`，不写死 `0728期`、年级或未来期次，并确保其先于 `flow_pool_name like '%星义物理%' then '赵星义'` 等宽泛分支命中。
- 在线探针 query `1495639193` 验证最新可见分区 `dt='20260723' and hour='13'`：纯值等于 `北京直播江苏` 命中 0 条；用户指定的 `like '%北京直播江苏%'` 命中 15 条明细、`sum(lead_count)=14`、`sum(valid_lead_count)=14`。既有 query `1495469885` 进一步确认其中 8 条会被旧的星义物理宽泛分支抢先归为 `赵星义`。
- 本地共验收 42 个受管渠道 SQL、46 处可执行期次无关分支，其中包含 30 个当前数据中心模型、5 个独立模板 SQL，另有 1 个模板复用模型 2424 canonical SQL；仅引用上游渠道字段、按规则名拆分或使用独立播报渠道的文件不重复嵌入 CASE。
- 数据中心旧模型 `2625` 在 2026-07-23 全量扫描中已不存在，已从 current model registry 退役并在指标联动文档中保留为历史绑定；未执行线上删除。
- 30 个受影响数据中心模型均完成完整 SQL 替换、预览、保存回读、立即抽数与新 `SUCCESS`：`2054->160497731`、`2132->160497753`、`2253->160497755`、`2293->160497758`、`2310->160497760`、`2344->160497762`、`2345->160497764`、`2349->160497766`、`2350->160497778`、`2353->160497815`、`2423->160497858`、`2424->160497888`、`2461->160497917`、`2533->160497954`、`2623->160497994`、`2634->160498001`、`2683->160498024`、`2688->160498031`、`2751->160498047`、`2774->160498064`、`2809->160498069`、`2812->160498074`、`2836->160498078`、`2842->160498087`、`2883->160498091`、`2885->160363133`、`2886->160505124`、`2890->160505143`、`3039->160505149`、`3153->160505152`。
- `2350/2461/2623/2634` 的既有同步计划已仅续展有效日期至 `2026-10-20`，保留原小时、接收人和 task id；`2885` 因同小时立即执行去重，改以 `2026-07-23 18:13:02` 的下一计划任务 `160363133` 作为新 SQL 抽数成功凭证。最终只读审计再次逐个回读 30 份远端 SQL 哈希、目标分支、有效计划和最新成功记录，结果 `30/30 fully_verified`。
- 5 个有效模板取数均保持原模板 id 原位更新并处于 `published`：`8882`、`8866`、`8796`、`8797`、`8801`；因此已有申请关系继续绑定原模板，无需新建模板或重新申请。
- 模板最新版 SQL 的新查询均成功：`8882->379800 (2523 行)`、`8866->379801 (5237 行)`、`8796->379810 (1566 行)`、`8797->379812 (582 行)`、`8801->379804 (1053 行)`。`8796/8797` 的整月首轮仅因平台 5 分钟限制失败，改用已确认有数据的单期次半开范围 `20260710期 <= qici < 20260711期` 后成功。
- 最终 scoped 同步确认 30 份生产 SQL 与 Skill canonical SQL 哈希一致，并刻意排除 4 个无关评优数据集的外部漂移；反向索引、共享 catalog、唯一版本审计、市场域 integrity、163 个操作边界测试、69 个 SQL/QueryPlan 测试及完整 Text2SQL 栈全部通过。

## 2026-07-23 模板取数退役条目清理

- 从模板取数清单和历史同步记录中移除无现行 raw SQL 绑定的退役模板条目；其余有效模板及数据中心 canonical SQL 保持不变。
- 重建市场域反向索引和共享 Text2SQL catalog，并通过完整知识栈校验。

## 2026-07-26 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2054, 2132, 2253, 2293, 2310, 2344, 2345, 2424, 2461, 2533, 2623, 2634, 2683, 2688, 2751, 2774, 2809, 2812, 2836, 2842, 2883, 2885, 2886, 2890, 3039, 3153`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-07-26 市场顾问渠道 CASE 0726 与宽表维护

- 以 0723 版为基线生成 `resources/raw_sql/market_channel_case_when_0726.sql`：保留“北京直播江苏”和“孟亚飞视频号归入孟亚飞9元”口径，吸收新的区域、KOC、进校规则，并按审阅结果移除 34 个旧规则实例。标准 CASE 共 156 个分支、98 个输出值，SHA-256 为 `ae19deefce5c646b51c85e51e7df7f5449161d3f0c3d36dab07723e05ead443d`。
...
- 按已审阅同步计划原子更新 model_id：`2054, 2132, 2253, 2293, 2310, 2344, 2345, 2533, 2688, 2751, 2774, 2809, 2812, 2836, 2842, 2883, 2885, 2886, 2890, 2978, 3039, 3153`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-08-01 0728 期退款复用渠道归因修正

- 业务确认 `20260803期 + 线上商务部 + source负责人(曲默晗/何木玲) + 0728期SKU(帅师/孟帝)` 是 0728 期退款后的线索复用，不属于 `KOC-周帅数学` 或 `KOC-孟亚飞数学`；旧归因错误来自这批线索先命中 KOC SKU/source 负责人分支。
- 在 22 个受影响模型的渠道 CASE 中前置精确批次规则，统一输出 `退款订单复用`，且不放宽期次、来源部门、source 负责人和 SKU 边界；model `2344` 的 3 处同义 CASE 均已同步，model `2978` 中该批次按新归因退出 KOC-only 播报范围。
- 修正前精确批次 `sum(lead_count)=1444`，旧分布为 `KOC-周帅数学=1121`、`KOC-孟亚飞数学=323`；修正后 query `1514422403` 验证 1444 全部归入 `退款订单复用`，两个错误 KOC 渠道残留为 0。截至 2026-08-01，该次变更未调整 `20260728期` 的周帅 KOC 209；该批次于 2026-08-02 按新的退款池证据另行重分类，见下节。
- 22 个模型均完成完整 SQL 替换、预览、保存哈希回读、立即抽数和 `SUCCESS`：`2054->162138997`、`2132->162139001`、`2253->162139003`、`2293->162139005`、`2310->162139036`、`2344->162145743`、`2345->162145769`、`2533->162145777`、`2688->162145779`、`2751->162145780`、`2774->162146129`、`2809->162145786`、`2812->162145796`、`2836->162145832`、`2842->162145892`、`2883->162145914`、`2885->162007606`、`2886->162146050`、`2890->162146063`、`2978->162146068`、`3039->162146077`、`3153->162146086`。其中 `2774` 在同步周期续期后完成最终写入和首轮成功抽数。
- 线上版本按本地同步计划 `964191c106d16b4c20017f97d4216240876d1bb9782b02a3784720dc1f3981fe` 回写 Skill canonical SQL；渠道规则的长期维护说明已补入 `knowledge/sql_patterns/channel_mapping_case_when.md`。

## 2026-08-02 20260728 期线上商务部退款复用渠道归因修正

- 业务确认 `20260728期 + 线上商务部 + 电商退款用户池 + 0728期退款用户计划 + 内部/流量复用 + source负责人曲默晗 + 0728期帅师SKU` 是退款后的线索复用，不属于 `KOC-周帅数学`。旧归因错误来自通用退款分支未覆盖 `线上商务部`，随后被“曲默晗 + 帅师 SKU”的 KOC 宽泛分支抢先归类。
- 在 22 个实际使用该流量池归因 CASE 的当前模型中，将精确分支置于既有 20260803 退款覆盖和所有 KOC 分支之前，输出 `退款订单复用`；model `2344` 的 3 处同义 CASE 均同步。model `2978` 为支持该精确条件，仅在内部 `lead_base` 补投影 `put_plan_name`、`channel_name_1`，最终输出字段保持不变。
- 上线后 query `1517735633` 验证：退款池 214 个源行/214 个去重线索，`sum(lead_count)=209`、`sum(valid_lead_count)=209`，全部归为 `退款订单复用`；原无年级异常队列中的 1 条 `金榜题火箭班 + 商务/KOL + 何木玲` 线索仍归为 `KOC-周帅数学`，没有被退款覆盖误伤。
...
- 生产异常处置：`2533` 首次预览 400 后原计划重试成功；`2978` 因内部 CTE 缺少两列导致预览 400，补齐内部投影并重新生成哈希绑定计划后成功；`2885` 的时点/taskId 为 `23:00 -> 12:00` 倒序，UI 对整串 taskId 两次返回成功但均未生成执行记录，最终只对已过时点 `15:00` 对应 taskId `61493` 执行一次并读回 `162316734 / SUCCESS`，未修改其同步周期、时点集合或 SQL。
- 线上回读计划 `a863a94f145880b141ed466764eb25ee03f69a02c64dda65a0cd90c1e9d0db3e` 确认 22 个远端 SQL 与 canonical 一致（`raw_sql_changed=0`），随后刷新数据集目录、当前模型绑定哈希和生成 manifest；反向索引、共享 catalog、唯一版本审计、市场域 integrity 与完整 Text2SQL 栈全部通过。

## 2026-08-02 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2421, 2632, 2643, 2644`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-08-02 暑期运营期次 20260809 更名为 20260808

- 业务确认暑期运营周期仍为 `2026-08-07` 至 `2026-08-12`，但正式期次名称应为 `20260808期`；原 `20260809期` 是展示名称错误。本次只改期次名称，不改变日期范围、字段角色、自然周回退、指标、Join 或最终字段。
- 覆盖 31 个当前 Data Center 模型：`2054, 2132, 2253, 2293, 2310, 2344, 2345, 2349, 2353, 2421, 2423, 2424, 2533, 2632, 2643, 2644, 2683, 2688, 2727, 2751, 2809, 2812, 2836, 2842, 2883, 2885, 2886, 2890, 2978, 3039, 3153`。共替换 66 处标签，覆盖线索期、行课期、交易/退款期和规则期次；旧标签残留为 0。
- 写入前线上回读发现 `2421, 2632, 2643, 2644` 有较新 SQL，先用精确计划 `bece39ca86f7072e854634e3f81b2040c2b0b9ebfd7b9f364fd84183bff3b52c` 同步回 canonical，再在最新基线上更名，保留了这些数据集的其他线上修改。
- 31 个模型均完成预览、保存响应、保存后 SQL 哈希回读、立即抽数和新 `SUCCESS`：`2054->162323589`、`2132->162323593`、`2253->162323595`、`2293->162323598`、`2310->162323599`、`2344->162323602`、`2345->162323603`、`2349->162323606`、`2353->162323608`、`2421->162323609`、`2423->162323612`、`2424->162323614`、`2533->162323618`、`2632->162323620`、`2643->162323626`、`2644->162323627`、`2683->162323628`、`2688->162326214`、`2727->162330284`、`2751->162330355`、`2809->162330360`、`2812->162330363`、`2836->162330365`、`2842->162330369`、`2883->162330372`、`2885->162194179`、`2886->162330398`、`2890->162330419`、`2978->162330464`、`3039->162330494`、`3153->162330506`。其中 `2727` 在用户续期后完成，`2885` 的新记录相对其同任务基线 `162194178` 为 `162194179`，开始时间晚于本次触发时间。
- 上线后全量回读计划 `bbbd733cf8ad7c266a6269360dfead2c90f407d0daf0167b79339204d60fd55a` 确认 31 个远端 SQL 与 Skill canonical 一致（`raw_sql_changed=0`）；随后刷新数据集目录、模型绑定哈希和生成 manifest，并执行反向索引、共享 catalog、唯一版本审计、市场域 integrity 与完整 Text2SQL 栈校验。

## 2026-08-02 运营侧个人数据 2293 年级回退修正

- 排查确认 `20260803期` 空年级展示的 1407 条有效线索不是 join 放大：源层有 1466 个物理 lead，其中 `sum(lead_count)=1407`、`sum(valid_lead_count)=1407`；这 1407 条的业财宽表主留痕 `rule_name` 和 `lead_purchase_intention_level2_category_name` 均为空。
- 模型 `2293` 的 `grade_1` 保持原有 `rule_name` 年级关键词优先，并保持非空购买意向二级品类优先；只在二者均为空时回退同一业财宽表已有的 `stats_grade_name`。未新增表或 join，最终字段结构、聚合粒度、渠道归因和指标公式均未修改。
...
- 按已审阅同步计划原子更新 model_id：`3202`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-08-06 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2054, 2132, 2253, 2293, 2310, 2344, 2345, 2424, 2461, 2533, 2623, 2634, 2683, 2688, 2751, 2774, 2809, 2812, 2836, 2842, 2883, 2885, 3039, 3153, 3202`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-08-06 市场顾问 0805 渠道 CASE 融合、数据中心性能优化与模板同步

- 从 `C:\Users\Ludim\Desktop\CASE.txt` 读取 0805 版本，源文件 SHA-256 为 `7ae55aa357ac5f27b23a7f1e82a609c5479ca473667e758667191f42869b3842`。按 0726 基线与 0805 的顺序、新增、删除、同条件输出变化逐分支审计后，生成 `resources/raw_sql/market_channel_case_when_0805.sql`：保留 0805 原始优先级，最终 170 个 `WHEN` 分支、109 个唯一输出值，融合 CASE SHA-256 为 `73bc9bdc26c7719640c6a66352f1675743ae1f7655221e197ab87865068f236e`；未使用固定行号切片删除或机械替换。
- 旧版 `market_channel_case_when_0726.sql` 已重命名为 `market_channel_case_when_0726_legacy.sql`，原文件内容和哈希保持不变，仅从当前 canonical ownership 中退役，避免 0726 与 0805 被版本审计误判为两个当前入口。
...
## 2026-09-10 0904 渠道 CASE first-match 修复

- 修复 0904 渠道映射中 5 条“河南进校/集团私域”宽规则提前截获 8 条“进校私域合作”细分候选的问题；保留 183 个 `WHEN` 与 127 个输出，仅给冲突宽规则增加显式排除。
- 全量覆盖 23 个生产 Data Center、6 个原 ID 永久模板和天宫2 `market2lark.py`；模板完成发布与真实查询，天宫 V7 即时执行实例 `169905915` 为 `SUCCESS`，每 4 小时周期配置未改。
- 在 `resources/raw_sql/market_channel_case_when_0904.sql` 文件头和渠道映射知识中固化异常与防回归约束：后续更新必须检查 first-match 相对顺序、宽/细规则重叠及代表性 lead 逐条归因，不能只校验分支数量。
- 20260911 线索量差异复核：9560/9847/9841 分别来自 `20260908/20`、`20260909/09`、`20260909/08` 三个源小时快照；281 是 134 名顾问正负变化净额（+465、-184），不是负责人映射缺失。按生产组织回退规则，负责人为空且有效线索大于 0 的记录为 0。

## 2026-09-10 模板取数 stable canonical SQL 同步（模板 7689）

- 线上 `published` 模板 `业财用户出单明细`（id `7689`）回读 SQL SHA-256 为 `e124245f589414bdc99e36fe287f8b20d95738691c0fd869dd1b85515baf7407`，只保留稳定入口 `resources/raw_sql/template_query_market_finance_order_detail.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-10 模板取数 stable canonical SQL 同步（模板 7808）

- 线上 `published` 模板 `市场运营专用_多维全链路分析`（id `7808`）回读 SQL SHA-256 为 `d6692d4531099c63836668d4ea43b245076a52ae2249723e99105455aca1a089`，只保留稳定入口 `resources/raw_sql/template_query_market_wide_analysis.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-10 模板取数 stable canonical SQL 同步（模板 8735）

- 线上 `published` 模板 `馒头_订单明细_支付时间`（id `8735`）回读 SQL SHA-256 为 `f9d30fd3a89f02cf5207af225c020b403c52f38485ecaa02e5f9b0498ba4ee4a`，只保留稳定入口 `resources/raw_sql/template_query_market_mantou_order_detail_pay_time.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-10 模板取数 stable canonical SQL 同步（模板 8882）

- 线上 `published` 模板 `AI分析市场顾问部多科用户成单数据`（id `8882`）回读 SQL SHA-256 为 `7977320aeb6b522608ed8b7a2b9ce751ca27b1cbf61b8916368a3db622d0b3ec`，只保留稳定入口 `resources/raw_sql/template_query_market_multi_subject_order_user.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-10 模板取数 stable canonical SQL 同步（模板 8948）

- 线上 `published` 模板 `馒头_订单明细_流水时间`（id `8948`）回读 SQL SHA-256 为 `ec8d40c6d5e2359ff85aefbec27bfef55fb5d05731a6bc0d4eb5b6a4c08bb918`，只保留稳定入口 `resources/raw_sql/template_query_market_mantou_order_detail_trade_time.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-10 模板取数 stable canonical SQL 同步（模板 9002）

- 线上 `published` 模板 `AI分析市场顾问部_宽表`（id `9002`）回读 SQL SHA-256 为 `40d1068f98e8dacd5001f44511b8931773807e851fa250d54846340d5afc0835`，只保留稳定入口 `resources/raw_sql/template_query_market_wide.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-11 抖音私信有效线索口径修复与端到端验收

- 根因不是 0904 渠道 CASE 的 first-match 顺序，而是渠道 CASE 与有效指标 CASE 的目标集合不一致：渠道已按 `source_manager_name = '韩正卿'` 归为“抖音私信”，指标层却额外要求 `channel_name_1 = '市场私域'`；实际目标记录为 `信息流/今日头条`，因此错误回退到普通 `lead_count` / `valid_lead_count` 并在看板显示 0。修复为同一源谓词下退前取 `merge_assign_lead_count`、退后取 `merge_valid_lead_count`。
- 生产门禁 Query `1584564188` 验证 20260911 期目标源行 149、去重 lead 11978/总源行 11981、目标退前/退后 `142/142`、7 条 merge 双零保持无效；非目标渠道退前/退后差异均为 0，订单 48、收入 14146990 不变。四个目标 SQL 未新增 join。
- 四个 Data Center 数据集完成 SQL Hash 回读、立即抽取和新 `SUCCESS`：外呼 2054 `8dc877655394f6a010e411d300b487961718193286bf4b54356ed22a0e16b80c` / `170112794`，到课 2132 `63cf535ad08c2b288b64febc4ac80f53af37a09c54b82133698af7cc4f74e5cb` / `170112796`，转化 2253 `ae879f4e0e29ebccdb5c271ac752a1ff14d87e1ffb9e1afd3d202336afd34808` / `170112799`，运营侧个人 2293 `2a5071a3214848695de469e088da681b6637c93ac4dbac737b230a07ad8d4698` / `170112811`。
- 运营侧看板以明确请求态过滤 `20260911期 + 抖音私信` 验收：分团队转化组件 `unit_3788931121683902464` 退前/退后 `142/142`；专用组件 `unit_3889082426239688709` 退前/退后 `142/142`，最新查询任务 `1584600558,1584600556`。公共期次下拉未预取 20260911 期，官方截图命令仍不能选中该期次；该前端筛选可用性问题不影响组件数值回读结论。
- 天宫2 `market2lark` V9 已保存、提交、发布为版本 `205398`，完整源码 Hash `ae78c13a392253528ea073f332019b270f8d9a38b8f84d183833ffddbd039d4d`，规范化查询 Hash `bbb08d9ab16126d7ac6b4f8bb71d08bf304ef85c232a50c2d2b84ffed865f6cb`；唯一手工执行 `170112837` 为 `SUCCESS`，`triggerSuccessor=false`。周期配置 `53831 / 4h` 未改，17:00 后由既有周期自然产生新实例 `170118934`。
- 飞书 Base `市场顾问部_全渠道线索播报` 表 `tbljWRvaqKTdrCx4` 对同一 V9 快照完成独立全量回读：过滤 `20260911期 + 抖音私信` 得 215 条、`has_more=false`，与任务快照渠道计数一致；退前/退后 `207/207`，8 条双零，记录键与 record_id 重复均为 0。Base 明细范围比看板数据集更宽，因此 `207/207` 不要求等于看板的 `142/142`；两条消费链路均按各自范围恢复且未见 1:N 放大。
- 在 0904 CASE 文件头、渠道映射模式与指标契约中固化防回归要求：渠道和指标必须共享目标集合，同时检查退前、退后及加微/外呼/到课派生指标；平台 `SUCCESS` 之后仍须按明确期次和渠道回读看板及承接 Base。

## 2026-09-17 数据地图字段补充

- 使用数据地图 `tableV2/searchTableList`、`normalColumns`、`partitionColumns` 和 `getDdl` 接口刷新物理表字段信息。
- 覆盖 `knowledge/tables` 中 3 张物理表文档；追加 48 个数据地图字段，回填类型 0 处、字段说明 0 处。
- 复扫结果为字段缺口 0、类型占位 0、说明占位 0。
- 未覆盖 `temp_table.*` 临时表文档；临时表字段仍以本地 Excel、SQL 使用场景和人工维护规则为准。

## 2026-09-17 17:10:41 AI 通话三表探查

- 新增 ASR、通话记录、AI 通话分析结果三张表文档和 `knowledge/joins/ai_call_asr_relationship.md`；字段与类型来自天工数据地图，探查聚合来自 `20260915` 分区的 Query `1591080868`、`1591083960`、`1591084542`、`1591086950`。
- 依用户提供的预期 SQL 核验 `dt + call_id` 和 `call_url = origin_file_url` 两条候选关联。88 条非市场专属诊断通话中，AI 结果没有多匹配，ASR 有 3 条多匹配，直接连接使样本通话时长从 7,887 秒放大到 9,484 秒；ASR 去重规则及市场顾问范围内覆盖仍待确认，不升级为 confirmed contract。
- 表结构同步的本域反向索引已生成；共享 catalog 构建被青橙域已有文档的 source Hash 失效阻断。未修改青橙知识，后续以本域完整性与离线解析检查补充验证；共享 catalog 仍待青橙域单独维护后重建。

## 2026-09-17 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2054, 2132, 2253, 2293`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。
- 本轮精确回读 6 个模型；`2344/2812` 的 SQL 已与远端一致，仅校正 current-model 登记。上述 4 份 SQL 只同步远端当前版本到本地 Skill，未修改远端数据集或触发抽数。
- 先前 AI 通话探查记录中的共享 catalog 阻断已由独立的青橙 source Hash 与生成索引维护解除；本域和完整栈验证均已通过。

## 2026-09-19 数据中心 stable canonical SQL 同步

- 按已审阅同步计划原子更新 model_id：`2054, 2132, 2253, 2293, 2310, 2344, 2345, 2461, 2623, 2683, 2688, 2751, 2774, 2809, 2812, 2836, 2842, 2883, 2885, 2886, 2890, 3039`；每个 model_id 只保留稳定 canonical 路径。
- 写入后已强制重建反向索引和目录，并运行唯一版本审计、域内 integrity 与完整 Text2SQL 栈验证。

## 2026-09-19 模板取数 stable canonical SQL 同步（模板 7689）

- 线上 `published` 模板 `业财用户出单明细`（id `7689`）回读 SQL SHA-256 为 `f8171688d4a74f4827648772eb04a5f723e1503763e7b9e4452b5781566eaff7`，只保留稳定入口 `resources/raw_sql/template_query_market_finance_order_detail.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-19 模板取数 stable canonical SQL 同步（模板 7808）

- 线上 `published` 模板 `市场运营专用_多维全链路分析`（id `7808`）回读 SQL SHA-256 为 `db109cf07779b300fb1dd8791beef2d1120478c15a53288f3d8c7908471fc8fb`，只保留稳定入口 `resources/raw_sql/template_query_market_wide_analysis.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-19 模板取数 stable canonical SQL 同步（模板 8735）

- 线上 `published` 模板 `馒头_订单明细_支付时间`（id `8735`）回读 SQL SHA-256 为 `8e580b5411d6b93a9cc547aace3d62b22caeba73fa9490f03267c20e22cbf0b5`，只保留稳定入口 `resources/raw_sql/template_query_market_mantou_order_detail_pay_time.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-19 模板取数 stable canonical SQL 同步（模板 8882）

- 线上 `published` 模板 `AI分析市场顾问部多科用户成单数据`（id `8882`）回读 SQL SHA-256 为 `a02eb572d77d8d971b5256bb01a24c81a3d78ae4f32da7fc119ed5b9bd45ef31`，只保留稳定入口 `resources/raw_sql/template_query_market_multi_subject_order_user.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-19 模板取数 stable canonical SQL 同步（模板 8948）

- 线上 `published` 模板 `馒头_订单明细_流水时间`（id `8948`）回读 SQL SHA-256 为 `be99aa368462212436947ed20df0962509e0b75f1650012a4cc3a5d01ae94643`，只保留稳定入口 `resources/raw_sql/template_query_market_mantou_order_detail_trade_time.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-19 模板取数 stable canonical SQL 同步（模板 9002）

- 线上 `published` 模板 `AI分析市场顾问部_宽表`（id `9002`）回读 SQL SHA-256 为 `be3da6a346ca64f90128c79b24053073d99d4847062821a03bcf6f765330adf2`，只保留稳定入口 `resources/raw_sql/template_query_market_wide.sql`。
- 本次清理日期后缀历史副本 0 个；历史 SQL 文本不进入知识库，也不作为路由入口。
- 保存后已重建反向索引、共享 catalog、唯一版本审计、域内 integrity 和完整 Text2SQL 栈。

## 2026-09-19 市场顾问部 0918 渠道归因全链路同步

- 从飞书多维表格 `20260918期`（记录 `recvtOopW7sPfH`）读取规则，以 0918 顺序融合线上消费者差异：canonical CASE 为 179 个分支、123 个结构化去重输出，文件 SHA-256 `db88ab003aa18607f205a07663305fa05bc67dac415089aac3c62050691a884f`；新增 `喻新娇` 到本地化市场，删除河南进校、小写 app、微信私域直映射和七池平台自播曹忆 4 条规则，并保留 3 条宽规则对后续 8 条进校私域细分候选的排除保护。
