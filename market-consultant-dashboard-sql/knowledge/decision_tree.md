...
| 模板取数最新代码、AI分析市场顾问部多科/分周期/每天/转化/过程/到课/员工架构/进量模板、业财用户出单明细、馒头订单明细支付时间 | `knowledge/dashboards/template_query_market_datasets.md` | 清单中的对应 raw SQL 文件；业财用户出单明细的渠道维护再读 `knowledge/sql_patterns/channel_mapping_case_when.md`；馒头模板再读 `knowledge/sql_patterns/mantou_order_detail_pay_time_stage_optimization.md` | 使用口径必须说明为“模板取数”；不要默认覆盖或替代数据中心/Web BI canonical SQL；业财模板 7689 是共享渠道 CASE 强制同步消费者；馒头支付时间模板不得恢复重 CTE 四次内联结构 |
...
- 生成新市场顾问渠道归因 SQL：默认使用 `resources/raw_sql/market_channel_case_when_0805.sql`，除非用户明确要求沿用历史 SQL 旧口径。
