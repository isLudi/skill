...
| lead_id（单分区） | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df, gaotu_hl.dim_mkt_h_lead_channel_df | 从市销线索渠道快照补充 `channel_map/channel_group` | 渠道维表必须限定一个 `dt`；`20260812` 分区内 `lead_id` 唯一，不会由维表侧放大，但对 `20260813/hour=13` 市场顾问宽表仅覆盖 24.3341%，且最新期次存在有效线索未命中，当前仅是候选 Join，禁止直接替换共享 CASE |
| source_period_name + department + stat_grade（禁止） | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df, gaotu_hl.dim_mkt_h_lead_channel_df | 尝试不使用 `lead_id` 获取 `channel_map` | 维表 1,136 个键中 760 个多渠道，单键最多 43 个渠道；宽表仅 233 行落入唯一渠道键，直接连原始维表理论放大到约 18.45 亿行，不能使用 |
| department_name + period_name（规则文本选择） | gaotu_hl.ods_mkt_h_channel_rule_df | 选择适用的完整渠道 CASE 文本 | 结果是 `channel_case_when` 字符串，不是逐规则 `channel_map`；不能与事实表等值 Join 后动态执行，且当前 SQL 查询权限未开通 |
...
| user_number + section_assign_employee_email_prefix | service_dw.app_h_crm_lead_employee_workload_detail_hf, bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df | 流量画像外呼工作量关联 | `data_center_market_2683.sql` 中 `call_c` 聚合含 `lead_id`，但 join 到主线索只用 `user_id + employee_email_prefix`；多线索用户需确认是否会放大 |
...
| concat(group_period_year, group_period_term) + period_mapping_second_level_department_name | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df, gaotu_hl.dim_mkt_h_period_map_df | 市场顾问全链路宽表映射系统期名到标准期名 | 必须把宽表 `group_period_year + group_period_term` 拼为 `source_period_name`，并同时用 `period_mapping_second_level_department_name = department`；`group_period_name`、`period_name` 不可替代；映射快照需先验证联合键唯一 |
| derived channel_map + department_name | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df, gaotu_hl.ods_mkt_h_channel_group_df | 市场顾问全链路宽表补充渠道大类 | `channel_map` 必须先由 0808 CASE 派生；渠道组表限定 `department_name='all'`、按 `channel` 预聚合；不能直接用 `channel_name_1/2/3` |
| channel, grade, qici | temp_table.dingxi01_cost | 渠道-年级-期次成本目标 | 应唯一；不唯一会导致看板行数放大 |
| qici + user_id | finance_dw.app_finance_performance_extend_details_hf, bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df | 流量画像成交科目档位关联 | `data_center_market_2683.sql` 的 `dd` 先按 `qici + user_id + employee_email_name` 聚合，最终 join 未带 `employee_email_name`，需确认用户多顾问成交风险 |
...
| period_name + lead_id + user_id | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df | 市场渠道用户画像首 call 通时分桶回连；整体画像数据集 `src` 阶段防止多线索被用户维度折叠 | `section_assign_all_call_duration` 只应单独聚合后用于分桶，不要放入携带业务指标的 `select distinct`；整体画像 SQL 仅保留 `lead_id` 防折叠，最终不输出；当前 SQL 用 `max(section_assign_all_call_duration)`，是否代表最终总通时待人工确认 |
| period_name + user_id | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df, service_dw.dws_service_user_learn_detail_hf | 市场渠道用户画像上课时长分桶回连 | 行课表由 `begin_time` 派生期次后按用户期次汇总 `live_learn_duration`；同一用户同一期多线索共享时长是否合理待人工确认 |
