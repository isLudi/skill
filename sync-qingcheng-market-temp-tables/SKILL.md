...
- 按 [仓库维护验证](../references/maintenance-verification.md) 选择检查。纯文档修改只做相关引用、编码和 Skill 检查，不停用运行中的事件服务。转换、注册表、上传或事件服务执行逻辑变化运行受影响单测和适用离线回放；部署、CLI 升级或恢复生产才进入 `event_service.md` 的停启门禁。已停用的升级服务在门禁完成前保持停用，生产状态变化须在授权范围内。
