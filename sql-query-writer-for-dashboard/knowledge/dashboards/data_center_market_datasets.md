...
- 维护方式：默认仅保存数据中心“数据集详情”接口返回的 `executeSql` 源 SQL，不改写业务逻辑；若数据集已由排查确认修复，则清单指向已验证修正版，并在维护说明中记录。
...
| 3 | `退费_科目_产品` | `menu_set_3804597496486080513` | `2349` | `2339` | `menu_source_817034371567951872` | 通用/SQL数据集/H业务线/市场部/市场顾问部/市场顾问部/退费_科目_产品 | [data_center_market_2349_refund_amount_share_fixed_20260704.sql](../../resources/raw_sql/data_center_market_2349_refund_amount_share_fixed_20260704.sql) | 324 |
...
- `退费_科目_产品` 已改为 2026-07-04 验证后的退款金额占比口径；旧 `refund_total` 负数口径和数据中心同步快照不再作为当前入口。
