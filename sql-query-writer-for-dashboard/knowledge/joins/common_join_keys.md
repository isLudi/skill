...
| period_name + lead_id + user_id | bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df | 市场渠道用户画像首 call 通时分桶回连 | `section_assign_all_call_duration` 只应单独聚合后用于分桶，不要放入携带业务指标的 `select distinct`；当前 SQL 用 `max(section_assign_all_call_duration)`，是否代表最终总通时待人工确认 |
