...
- 来源：`resources/raw_sql/data_center_market_2253.sql`
- 分区：`dt`、`hour`
...
- 来源：`resources/raw_sql/data_center_market_2836.sql`、`resources/raw_sql/data_center_market_2885.sql`、`resources/raw_sql/data_center_market_2883.sql`、`resources/raw_sql/data_center_market_2809.sql`、`resources/raw_sql/data_center_market_2890.sql`。
- 主表：`bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df`。
...
- 整体画像数据集：`data_center_market_2809.sql` 不做外部 join，仅使用全链路主表，按 `period_name + channel_map + grade_name + manager_name` 输出整体线索、有效线索、成交用户、科目档位、收入和订单指标。
- 多维退费率数据集：`data_center_market_2890.sql` 不做外部 join，仅使用全链路主表，按 `period_name + channel_map + grade_name + jingli + zhuguan + employee_email_name` 输出当期/截面 GMV 退费率、人头退费率、1科/2-3科/3科以上退费率所需的分子和分母字段。该数据集不输出行级退费率，透视表必须用分子分母字段重算。
...
- 来源：`resources/raw_sql/data_center_market_2727.sql`
- 主表：`finance_dw.app_finance_performance_extend_details_hf`
...
- 固定期次查询统一从当前通用 SQL `resources/raw_sql/data_center_market_2727.sql` 增加显式 `qici` 过滤生成，不再维护独立期次版本。
...
- `temp_table.dingxi01_pingyou_jg`：当前 `pingyou_jg.xlsx` 对应的标准临时表；模型 `2688` 按 `employee_email_name + qici` 取得新人过程转化的架构、人产和 `x_qi_count`。2026-07-21 已确认该键唯一，且有效 `x_qi_count` 1—4 在同一顾问内不跨期重复；完整维护规则见 `../tables/temp_table.dingxi01_pingyou_jg.md`。
...
- 来源：`resources/raw_sql/data_center_market_2683.sql`；2026-05-15 已按用户提供的 `D:\Feishu\city_channel.txt` 覆盖为省份/城市维度版本。
- 主表：`bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df`
...
- 渠道 CASE 使用 `resources/raw_sql/market_channel_case_when_0723.sql`。
...
- 新入口：`knowledge/dashboards/market_channel_conversion_profile.md` 的“多维退费率数据集”，来源 `resources/raw_sql/data_center_market_2890.sql`。
- 科目/产品/年级退款金额占比当前入口：`resources/raw_sql/data_center_market_2349.sql`。
- 当前来源：`resources/raw_sql/data_center_market_2890.sql`、`resources/raw_sql/data_center_market_2349.sql`、`resources/raw_sql/data_center_market_2353.sql`。
...
- 订单流水处理：2353 退费原因分析使用 `zong_price`；2349 科目/产品/年级金额占比拆为 `gmv_z/gmv_t` 后 `union all`，最终只保留 `name_total_price < 0` 的退款流水并取绝对值作为 `refund_amount`；2890 从全链路主表输出可加和分子分母。
...
- 多科分层：只使用 2890 当前字段 `refund_1_subject_*`、`refund_2_3_subject_*`、`refund_3plus_subject_*` 及对应净收入分母。
