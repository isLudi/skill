...
指标来自 `resources/raw_sql/consultant_sales_department_tenure.sql`。该 SQL 用财务业绩流水计算顾问销售净额，通过员工组织链限制交易时间必须落在顾问位于 `高途-H业务线-市场部-市场顾问部` 的部门任职时间范围内，并输出部门内排名和追赶上一名净收差值。
