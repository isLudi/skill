...
指标来自 `resources/raw_sql/data_center_market_2253.sql` 的 `zhuanhua` CTE 和最终 select。当前状态为“历史看板 SQL 口径”，未与业务指标文档核对。
...
是。需要确认金额单位、抖音私域合并口径、`>= 5` 阈值、成本表维护口径、时间偏移口径、`xiansuo` 是否只作为聚合指标输出，以及渠道映射 CASE 是否仍为最新规则。最新渠道 CASE 维护入口见 `knowledge/sql_patterns/channel_mapping_case_when.md`，归档片段为 `resources/raw_sql/market_channel_case_when_0723.sql`。
