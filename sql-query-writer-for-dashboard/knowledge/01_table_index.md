...
|---|---|---|---|---|---|---|
| bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df | 线索成本转化沟通行课全链路数据 | 线索-渠道-转化全链路明细，小时快照粒度待确认。 | dt, hour | 是 | 已确认 | 字段目录已补全；已记录流量画像 `city_channel` 省市维度和市场渠道用户画像分析三分桶数据集及整体画像数据集，口径需人工校验 |
