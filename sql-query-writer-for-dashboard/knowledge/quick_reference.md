...
| 表 | 常见场景 | USQL状态 | 优先动作 |
|---|---|---|---|
| `bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df` | 线索、渠道、转化、GMV 全链路 | 权限校验器解析错误 | 写 SQL 可参考，API 验证先看权限边界 |
| `finance_dw.app_finance_performance_extend_details_hf` | 财务业绩、收款、退款、评优 | 无表权限 | 标注权限风险，必要时申请权限 |
| `temp_table.dingxi01_jiagou_zx` | 当前在职架构、顾问名单 | 可读 | 限定 `zaizhi` 和部门，按顾问去重 |
| `temp_table.dingxi01_jiagou_db` | 期次架构、小组经理 | 可读 | 同时限定 `qici` 和目标部门 |
| `temp_table.dingxi01_pingyou_jg` | 评优/参评名单/人产 | 可读 | 仅在评优口径使用 |
| `temp_table.dingxi01_cost` | 渠道成本、GMV 目标 | 可读 | join 时处理 `grade='0'` 通配 |
| `temp_table.dingxi01_daoke_1_6_t` | 到课课次映射 | 可读 | 按期次、渠道、年级、开课时间匹配 |
| `service_dw.dwd_crm_assign_private_detail_hf` | 私海阶段、深沟、双沟 | 可读 | 市场顾问场景限定到市场部/市场顾问部 |
| `service_dw.dim_crm_assign_rule_lead_detail_hf` | 分配规则、计划组 | 可读 | 不按不存在的部门字段限权 |
| `gaotu_crm_offline_statistics.app_mcrm_first_call_task_hf` | 首 call 任务 | 无表权限 | 生成可参考，API 验证需权限 |
...
| 外呼过程/首 call | `knowledge/dashboards/outbound_call_process_dashboard.md` | `knowledge/sql_patterns/first_call_task_metric_pattern.md` |
...
| Python 调 USQL/权限失败 | `knowledge/sql_patterns/usql_rest_api_python.md` | `knowledge/sql_patterns/usql_permission_boundaries.md` |
...
- 涉及 API 执行、权限报错或可读性判断时，读 `knowledge/sql_patterns/usql_permission_boundaries.md`。
