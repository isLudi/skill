...
-- 来源：平台数据集最新查询
-- 更新时间：2026-05-24
-- 用途：暑期激励看板按自然月份维度聚合 pmit（收入退款）和转化指标，分部门排名
-- 数据范围：H业务线-市场部-市场顾问部，仅在职架构顾问
-- 注意：channel_map 使用的 CASE 版本不含 0524 新增的 王绍阳/途途私域/周帅-百度数字人 等规则，信息流-抖音私信 位于旧位置（APP之前）
with org_t as (
    select
        dep_path,
        email_prefix,
...
conversion_base as (
    select
        x.group_period_year,
        x.group_period_term,
        x.third_department_name,
        x.flow_pool_name,
        x.sku_id_name,
        x.ad_account_name,
        x.source_manager_name,
        x.channel_name_1,
        x.channel_name_2,
        x.channel_name_3,
        x.put_plan_name,
        x.page_id_name,
        x.source_put_plan_name,
        x.channel_provider_name,
        x.channel_second_provider_name,
        x.virtual_third_department_name,
        x.virtual_fourth_department_name,
        x.virtual_fifth_department_name,
        x.virtual_second_department_name,
        x.first_department_name,
        x.second_department_name,
        x.get_customer_way_name,
        x.lead_purchase_intention_name,
        x.lead_purchase_intention_level1_category_name,
        x.lead_purchase_intention_level2_category_name,
        x.flow_original_order_activity_price,
        x.flow_order_price,
        x.flow_orders_income_amount,
        x.lead_create_time,
        x.rule_name,
        x.virtual_leader_email_name,
        x.virtual_direct_leader_email_name,
        x.employee_email_name,
        x.lead_id,
        x.user_id,
        x.lead_count,
        x.valid_lead_count,
        x.merge_assign_lead_count,
        x.merge_valid_lead_count,
        x.conversion_lead_count,
        x.subject_count,
        x.same_lead_period_subject_count,
        x.lb_subject_count,
        x.same_lead_period_lb_subject_count,
        x.order_count,
        x.income_amount,
        x.in_pay_period_refund_amount,
        x.non_pay_period_refund_amount,
        x.jp_cross_department_refund_amount,
        x.same_lead_period_order_count,
        x.same_lead_period_conversion_lead_count,
        x.same_lead_period_income_amount,
        x.same_lead_period_refund_amount,
        x.group_period_date,
        concat(
            date_format(
                cast(
    ...
    from (
        select
            fl.group_period_year,
            fl.group_period_term,
            fl.third_department_name,
            fl.flow_pool_name,
            fl.sku_id_name,
            fl.ad_account_name,
            fl.source_manager_name,
            fl.channel_name_1,
            fl.channel_name_2,
            fl.channel_name_3,
            fl.put_plan_name,
            fl.page_id_name,
            fl.source_put_plan_name,
            fl.channel_provider_name,
            fl.channel_second_provider_name,
            fl.virtual_third_department_name,
            fl.virtual_fourth_department_name,
            fl.virtual_fifth_department_name,
            fl.virtual_second_department_name,
            fl.first_department_name,
            fl.second_department_name,
            fl.get_customer_way_name,
            fl.lead_purchase_intention_name,
            fl.lead_purchase_intention_level1_category_name,
            fl.lead_purchase_intention_level2_category_name,
            fl.flow_original_order_activity_price,
            fl.flow_order_price,
            fl.flow_orders_income_amount,
            fl.lead_create_time,
            fl.rule_name,
            fl.virtual_leader_email_name,
            fl.virtual_direct_leader_email_name,
            fl.employee_email_name,
            fl.lead_id,
            fl.user_id,
            fl.lead_count,
            fl.valid_lead_count,
            fl.merge_assign_lead_count,
            fl.merge_valid_lead_count,
            fl.conversion_lead_count,
            fl.subject_count,
            fl.same_lead_period_subject_count,
            fl.lb_subject_count,
            fl.same_lead_period_lb_subject_count,
            fl.order_count,
            fl.income_amount,
            fl.in_pay_period_refund_amount,
            fl.non_pay_period_refund_amount,
            fl.jp_cross_department_refund_amount,
            fl.same_lead_period_order_count,
            fl.same_lead_period_conversion_lead_count,
            fl.same_lead_period_income_amount,
            fl.same_lead_period_refund_amount,
            case
                when regexp_like(
...
conversion_data as (
    select distinct
        ...
        t1.lead_id,
        user_id,
        -- channel_map CASE: 注意此版本不含 0524 新增的 王绍阳/途途私域/周帅-百度数字人 规则
...
when (flow_pool_name like '%朱博士%' or flow_pool_name like '%双博士%' or flow_pool_name like '%教育规划%') and third_department_name <> '线上商务部' and period_name_calc not like '%多学科拓展%' and rule_name not like '%张杰%' and sku_id_name not like '%马凯鹏IP%' and third_department_name='直播部' then '朱博士29'
when put_plan_name like '%朱博士说教育%' and period_name_calc not like '%多学科拓展%' and flow_pool_name not like '%高分讲堂%' and  flow_pool_name not like '%总裁%' and third_department_name='直播部' then '朱博士29'
when flow_pool_name like '%朱博士讲英语%' and sku_id_name like '%马凯鹏IP%' and third_department_name='直播部' then '马凯鹏29'
when (flow_pool_name like '%朱博士讲英语%' or flow_pool_name like '%朱博士英语%' or flow_pool_name like '%朱汉祺说英语%' or flow_pool_name like '%朱博士说英语%' or flow_pool_name like '%教育规划%' or flow_pool_name like '%双博士英语规划%' or flow_pool_name like '%朱博士满分英语%' or flow_pool_name like '%英语教父朱博士%' or (flow_pool_name like '%中考决胜天团%' and lead_purchase_intention_level1_category_name = '规划系统')) and third_department_name = '直播部' and period_name_calc not like '%多学科拓展%' and sku_id_name not like '%马凯鹏IP%' and third_department_name='直播部' then '朱博士29'
when (flow_pool_name like '%汤哥%' or flow_pool_name like '%汤老师%') and period_name_calc not like '%多学科拓展%' and third_department_name in ('直播部','新媒体内容运营部')  then '汤老师'
when (flow_pool_name like '%中考百日冲刺%') and period_name_calc not like '%多学科拓展%' and third_department_name='直播部'  then '曹忆9.9纯课'
when (flow_pool_name like '%马总%' or  flow_pool_name = '减法化学马老师' or flow_pool_name like '%总裁讲化学%' or flow_pool_name like '%高分讲堂%') and period_name_calc not like '%多学科拓展%' and sku_id_name  like '%99%' and third_department_name='直播部'  then '马凯鹏99'
when (flow_pool_name like '%马总%' or  flow_pool_name = '减法化学马老师' or flow_pool_name like '%总裁讲化学%' or flow_pool_name like '%高分讲堂%') and period_name_calc not like '%多学科拓展%' and sku_id_name not like '%99%' and third_department_name='直播部'  then '马凯鹏29'
when (flow_pool_name like '%北大杰哥%' or flow_pool_name like '%张小杰%') and period_name_calc not like '%多学科拓展%' and third_department_name='直播部' then '张杰'
...
when flow_pool_name like '%峥峥%' and period_name_calc not like '%多学科拓展%' and third_department_name='直播部'  then '何峥峥'
when flow_pool_name like '%汐子%' and period_name_calc not like '%多学科拓展%' and sku_id_name not like '%亚飞%' and third_department_name='直播部'  then '王汐子'
when flow_pool_name like '%汐子%' and period_name_calc not like '%多学科拓展%' and sku_id_name  like '%亚飞%' and third_department_name='直播部' and rule_name like '%99%'  then '孟亚飞99-1组'
when flow_pool_name like '%汐子%' and period_name_calc not like '%多学科拓展%' and sku_id_name  like '%亚飞%' and third_department_name='直播部' then '孟亚飞9元'
when (flow_pool_name like '%曹忆%' or flow_pool_name like '%dudu%' or flow_pool_name like '%中考决胜天团%' or flow_pool_name like '%具象思维%' or flow_pool_name like '%在逃发面馒头%' or flow_pool_name like '%库洛米%' and lead_purchase_intention_level1_category_name <> '规划系统') and period_name_calc not like '%多学科拓展%' and third_department_name in ('直播部','新媒体内容运营部') then '曹忆'
...
when  source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪') and period_name_calc not like '%多学科拓展%' and sku_id_name not like '%朱汉祺%' and sku_id_name not like '%朱博士%' and sku_id_name not like '%周帅%' and sku_id_name not like '%29元%' then 'KOC-5元纯课'
when  source_manager_name in ('孙晗01','方俊结01','刘亦鹏02','何木玲','杨梓月','张可意03','任颖迪') and sku_id_name like '%周帅%' then 'KOC-周帅'
--when (channel_name_2 like '%KOL%' and source_manager_name in ('崔文轩','孙培尧')) or (channel_name_2 like '%抖音%' and source_manager_name in ('徐绮鹤')) and period_name_calc not like '%多学科拓展%' then '自孵化KOC'
...
when period_name_calc like '%多学科拓展%' and third_department_name like '%私域运营%' then '市场私域入群'
...
target_completion_ranked as (
    select
        ...
        end as target_completion_need_rate_to_previous
...
combined as (
    select
        ...
        coalesce(cm.target_completion_rate, 0) as target_completion_rate,
        coalesce(tcr.target_completion_period_dept_rank_no, 0) as target_completion_period_dept_rank_no,
        coalesce(tcr.target_completion_need_rate_to_previous, 0) as target_completion_need_rate_to_previous,
    ...
    left join conversion_metric_base cm
      on p.natural_month = cm.natural_month
     and p.name = cm.name
    left join target_completion_ranked tcr
      on p.natural_month = tcr.natural_month
     and p.dept = tcr.dept
     and p.name = tcr.name
)
select
    ...
    period_dept_rank_scope,
    period_dept_rank_no,
    period_dept_need_pmit_to_previous,
    target_completion_rate,
    target_completion_period_dept_rank_no,
    target_completion_need_rate_to_previous,
    leads_count,
    can_renew_ds_count_a,
    s_lead,
    podan,
