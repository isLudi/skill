...
data as (
  ...
  case when (
        (
            ...
            and sku_id_name like '%沪浙-%【浙江初三】领跑%'
            and (
                sku_id_name like '%数学英语物理化学%'
                or sku_id_name like '%数学英语物理生化%'
            )
        )
        or rule_name like '%浙江%曹忆科学%'
        or flow_pool_name like '%浙江科学曹忆%'
        or flow_pool_name like '%为真-浙江科学曹忆老师%'
        or (put_plan_name like '%浙江%' and put_plan_name like '%曹忆%' and lower(put_plan_name) like '%ip%')
        or (source_put_plan_name like '%浙江%' and source_put_plan_name like '%曹忆%' and lower(source_put_plan_name) like '%ip%')
        or (sku_id_name like '%浙江%' and sku_id_name like '%科学IP%')
        or sku_id_name like '%浙江初三科学IP%'
    ) then '浙江科学IP'
