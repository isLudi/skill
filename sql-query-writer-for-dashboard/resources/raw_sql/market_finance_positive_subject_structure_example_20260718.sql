...
lead_mapped as (
    select
        ...
        case
            ...
            else coalesce(lead_purchase_intention_level2_category_name, '未知')
        end as grade_name,
        manager_name,
        user_id,
        lead_count,
    ...
    from lead_src
    where period_name >= '20260605期'
      and period_name <= '20260703期'
),
lead_user_base as (
    select
        period_name,
    ...
    from lead_mapped
    where channel_map in ('KOC-5元纯课', '自孵化KOC-5元纯课', 'KOC-孟亚飞数学', 'KOC-周帅数学')
    group by
        period_name,
        channel_map,
        grade_name,
...
positive_users as (
    select distinct
        ...
        lub.user_id,
        lub.lead_count,
        lub.valid_lead_count,
        lub.regular_course_user_count as source_regular_course_user_count,
        lub.pay_subject_person_count as source_pay_subject_person_count,
        lub.regular_course_order_count as source_regular_course_order_count,
        lub.trade_income as source_trade_income,
        lub.net_income as source_net_income
    from lead_user_base lub
    inner join finance_user_subject fs
        on fs.period_name = lub.period_name
       and fs.user_id = lub.user_id
),
positive_user_subject as (
    select
        ...
        pu.user_id,
        fs.subject,
        fs.subject_order_count,
        fs.subject_amount,
        pu.lead_count,
        pu.valid_lead_count,
        pu.source_regular_course_user_count,
        pu.source_pay_subject_person_count,
        pu.source_regular_course_order_count,
        pu.source_trade_income,
        pu.source_net_income
...
subject_dim as (
    select distinct subject from positive_user_subject
),
group_dim as (
    select distinct period_name, channel_map, grade_name, manager_name from positive_users
),
group_subject_dim as (
    select gd.period_name, gd.channel_map, gd.grade_name, gd.manager_name, sd.subject
    from group_dim gd
...
subject_agg as (
    select
        ...
        manager_name,
        subject,
        count(distinct user_id) as subject_user_count_num,
        count(distinct cast(user_id as varchar)) as subject_person_count_num,
        sum(subject_order_count) as subject_order_count,
        sum(subject_amount) as subject_amount_num,
        sum(lead_count) as matched_user_lead_count,
        sum(valid_lead_count) as matched_user_valid_lead_count,
        sum(source_regular_course_user_count) as source_regular_course_user_count,
        sum(source_pay_subject_person_count) as source_pay_subject_person_count,
        sum(source_regular_course_order_count) as source_regular_course_order_count,
        sum(source_trade_income) as source_trade_income,
        sum(source_net_income) as source_net_income
    from positive_user_subject
    group by period_name, channel_map, grade_name, manager_name, subject
),
total_agg as (
    select
        period_name,
    ...
    from positive_user_subject
    group by period_name, channel_map, grade_name, manager_name
),
final_result as (
    select
        gsd.period_name,
        ...
        gsd.manager_name,
        gsd.subject,
        coalesce(sa.subject_user_count_num, 0) as subject_user_count_num,
        coalesce(ta.total_positive_user_den, 0) as total_positive_user_den,
        coalesce(sa.subject_person_count_num, 0) as subject_person_count_num,
        coalesce(ta.total_subject_person_den, 0) as total_subject_person_den,
        coalesce(sa.subject_amount_num, 0) as subject_amount_num,
        coalesce(ta.total_subject_amount_den, 0) as total_subject_amount_den,
        coalesce(sa.subject_order_count, 0) as subject_order_count,
        coalesce(sa.matched_user_lead_count, 0) as matched_user_lead_count,
        coalesce(sa.matched_user_valid_lead_count, 0) as matched_user_valid_lead_count,
        coalesce(sa.source_regular_course_user_count, 0) as source_regular_course_user_count,
        coalesce(sa.source_pay_subject_person_count, 0) as source_pay_subject_person_count,
        coalesce(sa.source_regular_course_order_count, 0) as source_regular_course_order_count,
        coalesce(sa.source_trade_income, 0) as source_trade_income,
        coalesce(sa.source_net_income, 0) as source_net_income
    from group_subject_dim gsd
    left join subject_agg sa
        on sa.period_name = gsd.period_name
       and sa.channel_map = gsd.channel_map
       and sa.grade_name = gsd.grade_name
       and sa.manager_name = gsd.manager_name
       and sa.subject = gsd.subject
    left join total_agg ta
        on ta.period_name = gsd.period_name
       and ta.channel_map = gsd.channel_map
       and ta.grade_name = gsd.grade_name
       and ta.manager_name = gsd.manager_name
)
select
    period_name,
    channel_map,
    ...
    manager_name,
    subject,
    subject_user_count_num,
    total_positive_user_den,
    subject_person_count_num,
    total_subject_person_den,
    subject_amount_num,
    total_subject_amount_den,
    subject_order_count,
    matched_user_lead_count,
    matched_user_valid_lead_count,
    source_regular_course_user_count,
    source_pay_subject_person_count,
    source_regular_course_order_count,
    source_trade_income,
    source_net_income
from final_result
order by period_name, channel_map, grade_name, manager_name, subject
limit 1000;
