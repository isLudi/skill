...
and t.is_del = '0'
left join temp_table.dingxi01_plan_id pl on pl.group_id = f.group_id
left join temp_table.dingxi01_jinliang_goal jg
on jg.channel_2 = split_part(f.rule_name, '-', 3)
and jg.grade = split_part(f.rule_name, '-', 4)
and jg.qici = split_part(f.rule_name, '-', 1)
...
from fp_base
left join vd on vd.rule_name = fp_base.rule_name and vd.employee_email_name = fp_base.employee_email_name
left join (select *,substr(qici, -5) as qici_0 from temp_table.dingxi01_jiagou_db) db on db.employee_email_name = fp_base.employee_email_name and db.qici_0 = fp_base.group_period_name
)
select
plan_id,
group_id,
