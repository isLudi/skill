...
when channel_name_1 = '信息流' and channel_name_2 = 'B站' and third_department_name not like '%投放%' and  (sku_id_name like '%陈瑞春%' or rule_name like '%陈瑞春%') and (ad_account_name  like '%语文%' or source_put_plan_name like '%自然流%') then 'B站信息流-陈瑞春'
...
when source_manager_name in ('包青青','蔡瑞涵','李文迁','李佳馨44','孙昊17','王洁雅01','王硕北','朱文','贾铭锐','李壮壮04') and channel_name_3 = '社群' then '商务低价'
...
when source_manager_name in ('耿文超','晋翠翠','赵语诗','王慧敏13','于新茹','梁超01','刘晓20','王玉120','吕佳乐01') and put_plan_name like '%社群%' then '创新商务'
...
when source_manager_name in ('宋莹莹','辛世如') and channel_name_1 = '直播获客' then '信息流-虚拟号挂车'
...
(
  ...
  data.*,
        case ---------------------触达时间打标签
            when data.valid_lead_count = 0 or data.first_call_time_diff_hour is null then '00_未获取'
            when data.first_call_time_diff_hour >= 0 and data.first_call_time_diff_hour <= 10/60 then '01_10分钟内'
            when data.first_call_time_diff_hour > 10/60 and data.first_call_time_diff_hour <= 30/60 then '02_30分钟内'
            when data.first_call_time_diff_hour > 30/60 and data.first_call_time_diff_hour <= 1 then '03_1小时内'
            when data.first_call_time_diff_hour > 1 and data.first_call_time_diff_hour <= 4 then '04_4小时内'
            when data.first_call_time_diff_hour > 4 and data.first_call_time_diff_hour <= 8 then '05_8小时内'
            when data.first_call_time_diff_hour > 8 and data.first_call_time_diff_hour <= 16 then '06_16小时内'
            when data.first_call_time_diff_hour > 16 and data.first_call_time_diff_hour <= 24 then '07_24小时内'
            when data.first_call_time_diff_hour > 24 and data.first_call_time_diff_hour <= 48 then '08_48小时内'
            when data.first_call_time_diff_hour > 48 then '09_超过48小时'
            else '00_未获取'
        end as first_call_time_range,	
case when data.first_call_time_diff_hour >= 0 and data.first_call_time_diff_hour <= 24 and data.valid_lead_count > 0 then 1 else 0 end as first_call_in_24h,
	...
	where period_name > '20260306期' and  channel_map = '抖音私信'
)
---------------------------转化
,zhuanhua as
(select
...
order by zz.first_call_time_range
