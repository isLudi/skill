...
-- 调课调班：按 name 和 user_id1 去重，每个用户保留一条记录
gmv_t as (
    select
        id,
        order_number,
...
rd_0 as (
    select
        ...
        grade_list,
        date(cast(trade_time as timestamp)) as trade_date,
        cast(year(cast(trade_time as timestamp)) as varchar) || '年' as years,
        'Q' || cast(quarter(cast(trade_time as timestamp)) as varchar) as quarter,
        cast(month(cast(trade_time as timestamp)) as varchar) || '月' as month,
    ...
    from rd
    left join temp_table.dingxi01_jiagou_zx zz on zz.employee_email_name = rd.name
