...
finance_trade_raw as (
    ...
    where f.dt = format_datetime(now() - interval '2' hour, 'YYYYMMdd')
      ...
      and f.employee_second_level_department_name = '精品班学部'
      and f.employee_third_level_department_name = '学习顾问部'
...
detail_base as (
    select
        ...
        case when l.lead_section_second_department_name = '精品班学部' then 1 else 0 end as is_lead_jpb_scope,
        row_number() over (
...
select
    ...
    is_lead_jpb_scope as "线索是否属于精品班范围",
    case
        when coalesce(is_stats_conversion_order, 0) = 1
         and is_lead_jpb_scope = 1
