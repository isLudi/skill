...
- Select `usql-web-query-operator` for SQL execution/download, stored or permanent templates, large-result download, dashboard profiling/change, Data Center read/sync/replace/create, Tiangong2 read-only inspection/log retrieval, and explicit planning/publication of one already-saved owned Tiangong2 task. It executes approved workflows but does not generate business SQL or infer metric definitions. Business Skill names, knowledge roots, dashboard folders, and Data Center sync targets resolve through its domain-adapter registry, never command-local paths. Tiangong2 uses its exact credential section, isolated state, exact read allowlists, owned-task scope, and redacted runtime artifacts. Only Workflow S may publish; it never saves, edits, submits, runs, reruns, or changes task configuration.
...
### S. Tiangong2 Owned-task Execution Logs and Governed Publish

`fetch-execution-log` requires exact project/direct “数据开发” folder/menu/name/execution and authenticated owner, binds Data-development to Nezha period/instance/stages through fixed GET/JSON/form reads, follows bounded monotonic log pagination, and writes only redacted runtime artifacts. `plan-task-publish` binds identity/scope/owner plus saved-source, task-metadata, version-state, and latest-published-source Hashes; identical published source is blocked. `publish-task` requires the exact Plan Hash and `--confirm-publish`, rechecks all drift, and uses a single-use client whose sole mutation is one `dataDevelop/publishTask` request for the planned menu. It never retries ambiguity and succeeds only after version-state and published-source Hash readback; otherwise its failed Receipt requires manual attention. It never saves/edits/submits/runs/debugs/reruns, changes scheduling/resources/configuration, leaves the folder, or operates another owner's task.
