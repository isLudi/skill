...
**Scope:** Playwright web automation for SQL execution, optional read-only QueryPlan/hash validation, result downloads, Template Query workflows, Data Center SQL read/local-sync plus independently authorized replacement and creation production chains, BI dashboard scanning/profiling, and governed P3/P4B dashboard changes. Data Center reading/local knowledge writes do not authorize remote replacement or creation; each production Apply requires its own exact plan hash, explicit confirmation, preview, save readback, and a new synchronization `SUCCESS` receipt. Dashboard operations are read-only by default except the five registry-verified P4B draft operations. Dashboard dataset rebinding, component/dashboard create-delete, component filters, permission, folder, and all other dashboard mutations remain blocked. Does NOT generate SQL.
