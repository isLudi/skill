...
- Select `usql-web-query-operator` for SQL execution/download, stored or permanent templates, large-result download, dashboard profiling/change, Data Center read/sync/replace/create, Tiangong2 read-only inspection/log retrieval, and explicit planning of a query-only save, version submission, publication, or one-time execution for an owned Tiangong2 task. It executes approved workflows but does not generate business SQL or infer metric definitions. Business Skill names, knowledge roots, dashboard folders, and Data Center sync targets resolve through its domain-adapter registry, never command-local paths. Tiangong2 uses its exact credential section, isolated state, exact read allowlists, owned-task scope, redacted runtime artifacts, and Workflow S's separate Hash/confirmation gates. It never edits Python outside the unique `query_sql`, changes the company default block, debugs/reruns, triggers downstream tasks, or changes task configuration.
...
### S. Tiangong2 Owned-task Query Update, Submit, Publish, Execute Once, and Logs

History/log reads require exact project/folder/menu/name, owner, and execution/stage binding; fixed reads use bounded pagination and redacted runtime artifacts.

`plan-task-query-update` binds one owned Python task, reviewed SQL, unique `query_sql` and company-default block, projected source, resource state, and metadata by Hash without storing code/secrets. Apply requires exact Hash plus `--confirm-save-query`, proves all other Python byte-identical, permits one `savePython`, and mandates source/query/default-block/resource readback. No-resource `null` may travel as `0` only when no project resource exists; never borrow one.

Submit planning binds saved source, metadata, versions, and note; apply requires exact Hash plus `--confirm-submit`, permits one `taskConfirm`, never saves, and reads back stable source/state. Without distinct submit state keep `fully_verified=false` until publish. Publish is independently planned, Hash-confirmed, single-use, blocks identical source, and requires version/source readback.

Execution planning requires current source to match latest published source and binds period/baseline executions. Apply requires exact Hash plus `--confirm-execute`, permits one `executeOnce` with downstream disabled and empty params/stages, and requires one new exec ID; terminal logs stay read-only. All writes are separate, drift-blocked, single-attempt, and never debug/rerun, reconfigure, leave scope, or touch another owner’s task.
