...
**Scope:** Playwright web automation for SQL execution, optional read-only QueryPlan/hash validation before execution, direct xlsx result downloads, Template Query stored-SQL fetch and large-result download, BI dashboard scanning/profiling, and the governed P3/P4B dashboard chain for components, layout, formulas, filters, and root theme: `profile -> DashboardDesignSpec -> diff -> dry-run -> draft apply -> separate publish confirmation`. Dashboard operations are read-only by default. P3A may profile, design, diff, and dry-run all change classes. P4B draft Apply may execute only five registry-verified operations: one existing stable field display-name update, existing-node `x/y/w/h`, one existing component-local non-shared formula expression with unchanged dependencies, stable-ID public-filter dynamic default, and dashboard-root background color. Dataset rebinding, create/delete, component-filter, permission, folder, and all other mutations remain blocked. Does NOT generate SQL.
