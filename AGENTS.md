...
1. **usql-web-query-operator** - Build and validate `DashboardBuildSpec`, then run read-only `plan-dashboard-build`. Resolve every existing dataset to exact model/subject/field identities and bind current QueryPlan, DashboardDatasetSpec, contract-source, folder, schema, and field-binding hashes.
