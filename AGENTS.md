...
- Select `usql-web-query-operator` for SQL execution/download, templates, dashboard and Data Center operations, plus Tiangong2 inspection/logs and Workflow S maintenance of one owned task. It executes approved workflows but does not generate business SQL or infer metrics. Domain paths resolve only through its adapter registry. Tiangong2 uses isolated credentials/state, fixed endpoint allowlists, redacted artifacts and Hash-bound authorization; it never crosses owner/folder scope, changes credentials/defaults/resources/configuration, reruns an exec, triggers downstream tasks, or replaces arbitrary full source.
...
### R. Tiangong2 Data-development Read-only Exploration

Operator `tiangong2_task.py list-projects` verifies the exact `# tiangong2 Web Query (Playwright) credentials` identity with a dedicated state file → `explore --project-id <id> --folder <exact-name>` recursively reads the selected “数据开发” folders, current source, version metadata/code when requested, schedule config, resource bindings, and project quality inventory through a fixed read-only endpoint allowlist → it redacts suspected secret literals before writing source snapshots, inventory, analysis, summary, and manifest only under the Tiangong2 runtime directory. It must not reuse the USQL/Data Map account or state, call a generic endpoint, run/debug/save/submit/publish/create/update/delete/move/rename any remote object, or write findings into a business Skill without a separate explicit knowledge-maintenance request.

### S. Tiangong2 Owned-task Maintenance and Debug

Reads bind exact task/owner/execution/stage; pagination is bounded and artifacts redacted. Tiers are R (read), P (one Plan Hash plus phase confirmation), M (one confirmed time/budget-bound session), and X (forbidden). M binds one owned task, protected baselines, allowed operations, expiry and execution cap. It removes repeated prompts only: every phase still needs a fresh Hash, drift check, one write and readback. Expiry, exhausted budget or uncertain mutation stops M. X covers cross-scope work, credential/default/resource/config changes, downstream triggers, exec reruns and arbitrary source replacement.

`plan-task-query-update`/Apply may change only the unique `query_sql`; other Python stays byte-identical, resource `null` maps to `0` only when truly absent, and readback covers source/query/default/resource. A diagnosed non-SQL defect may use only bounded unique non-secret exact replacements; projection must preserve query/default/resource and parse successfully. Patch plans retain hashes/lengths, not source or secrets; Apply sends at most one `savePython` and performs protected-source readback. No full-source replacement command exists.

Submit, publish and execute stay separate Hash-bound P/M phases. Submit never saves; publish blocks identical source and reads back version/source; execute requires current=latest-published, sends one downstream-disabled `executeOnce`, and requires one new exec ID. More debugging needs new evidence, repair/version/Plan and M budget; never rerun or reconfigure.
