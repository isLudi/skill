...
- User wants to plan, capture evidence for, apply, resume, or separately publish a from-zero self-service BI dashboard build
- User wants to read or locally synchronize Data Center dataset SQL, explicitly replace/save/refresh an existing Data Center SQL dataset, or explicitly create a named SQL dataset in a specified folder and complete its first extraction

**Scope:** Playwright web automation for SQL execution, optional read-only QueryPlan/hash validation, result downloads, Template Query workflows, Data Center SQL read/local-sync plus independently authorized replacement and creation production chains, BI dashboard scanning/profiling, governed P3/P4B dashboard changes, and the separate P4C no-auto-delete dashboard creation Saga. Data Center and dashboard build chains retain independent Plan hashes, confirmations, and Receipts. Dashboard operations are read-only by default except explicit Registry-verified operations; P4C dashboard shell, subject-level formula, card/pivot/bar/pie, multiple creation-time local/global filters, dashboardHtml assembly, all-new-metric aliases, evidence-backed styles, two-slot pivot containers, and restricted rich text are production allowlisted only through the exact-hash `taitan_dashboard_build_v1` Saga. Existing-component dataset rebinding/filter mutation/style expansion, delete, permission, folder creation/move, cloning fallback, and automatic publish remain blocked. Does NOT generate SQL.
...
- User wants to listen to or consume Feishu/Lark real-time events, Approval status-change events, webhook-like event streams, or bounded bot event subscriptions -> `lark-event`
...
1. **usql-web-query-operator** - Read the current dashboard draft into a complete normalized `DashboardProfile` with stable component, selected-field, dataset/model, formula, layout, and filter identities plus a canonical profile hash. Every configured `card/u_pivot/u_bar/u_pie` reference must resolve. Preserve partial profiles for diagnosis, but never pass an incomplete profile to Design, Apply, or Publish.
...
#### Workflow E5: Governed From-Zero Dashboard Build
> "Create a new self-service BI dashboard in this existing folder"（在既有文件夹从零创建自助 BI 看板）

1. **usql-web-query-operator** - Build and validate `DashboardBuildSpec`, then run read-only `plan-dashboard-build`. Resolve every existing dataset to exact model/subject/field identities and bind current QueryPlan, DashboardDatasetSpec, contract-source, folder, schema, and field-binding hashes. Advanced specs may declare every measure instance's display name, multiple creation-time local/global filters, evidence-backed styles, exactly-two-slot pivot containers, and restricted rich text.
2. If a dataset must be created, use Workflow O with its own exact Data Center plan hash and confirmation. Only after a new verified `SUCCESS` and readable BI field tree may the dashboard build be replanned to `ready`. Dashboard artifacts never authorize Data Center Apply.
3. P4C evidence may be captured only with `capture-dashboard-build-evidence --confirm-sandbox-write --headed` in the exact approved sandbox dashboard or a clearly named existing test/sandbox folder. Evidence is redacted and never promotes Registry entries automatically.
4. `apply-dashboard-build` requires the exact ready BuildPlan hash, explicit production-write confirmation, and every required creation capability to be Registry `verified/allowlisted` with full readback and `creation_saga_no_auto_delete`. It creates only a new unpublished dashboard and never modifies existing production dashboards. Metric aliases are all-or-none across the build; nested slot pivots cannot carry local filters; raw styles, arbitrary HTML/CSS, and arbitrary tab nesting stay blocked.
5. On failure, do not delete created dashboards, formulas, units, relations, or datasets. Emit a failed BuildReceipt with created/reused/orphaned resources and `manual_cleanup_required`; resume only exact-match logical resources.
6. `publish-dashboard-build` is a separate command consuming the successful BuildReceipt and requiring `--confirm-publish` plus a non-empty version description. Without a formal published-version read API, record `publish_requested_unverified` and `fully_verified=false`.
...
#### Workflow O: Data Center Dataset Creation and First Extraction
> "Create dataset *** under folder *** and complete extraction"（在指定文件夹新建数据集并成功抽数）

1. Resolve the business domain and obtain the reviewed concrete SQL file; business SQL semantics remain owned by the selected business SQL Skill.
2. **usql-web-query-operator** - Run `plan-data-center-dataset-creation` to resolve one exact domain-bound folder, verify the target name is unique, and bind SQL Hash, data-source identity, and schedule. This step is remote read-only.
3. Review the exact plan Hash. Only after explicit production-write authorization run `apply-data-center-dataset-creation --expected-plan-sha256 <hash> --confirm-production-write`.
4. Apply must create a draft from the exact folder, set name/data source/full SQL, preview successfully, configure synchronization before save, save and read back the new `menu_set_*` identity/SQL/schedule, trigger “立即执行”, and wait for a new `SUCCESS` record. Save success alone is incomplete.
...
5. **Boundary enforcement**:
   ...
   - QueryPlan execution gate: when a plan is supplied to `usql-web-query-operator`, it must be executable, have no unresolved slots, and match the exact submitted SQL SHA-256. A QueryPlan does not authorize downloads, dashboard writes, template writes, Data Center dataset replacement/creation, or publication. Likewise, `DashboardDatasetSpec`, `DashboardDesignSpec`, `DashboardChangePlan`, `DashboardBuildSpec`, `DashboardBuildPlan`, `DataCenterSqlReplacementPlan`, and `DataCenterDatasetCreationPlan` are descriptive/review artifacts and do not authorize Apply or publication
   ...
   - P4C dashboard creation governance: keep creation separate from DashboardChangePlan. Use `DashboardBuildSpec -> DashboardBuildPlan -> DashboardBuildReceipt -> DashboardBuildPublishReceipt`, exact upstream and schema hashes, a pre-existing uniquely resolved folder/name, and Registry `creation_saga_no_auto_delete` operations. The production P4C allowlist contains thirteen operations: dashboard shell, subject formula, four data-component types, public filter, dashboardHtml, all-new-metric rename, evidence-backed new-component style, two-slot Tab creation/slot assembly, and rich-text creation. Multiple local filters are permitted only inside the same new-root-unit creation operation. `capture-dashboard-build-evidence` is restricted to the exact approved sandbox dashboard or clearly named test/sandbox folder, always requires visible explicit confirmation, saves no payload values or credentials, and never auto-promotes capabilities. Apply must stop before browser launch while any required operation lacks real evidence, a production adapter, full readback, failure/idempotency tests, and an allowlisted Registry entry. Never auto-delete Saga resources or claim rollback; never use template cloning or a pre-created empty board as a production fallback; never combine Apply and Publish
   - Playwright boundary: do not use generic Playwright directly for `SQL取数` execution, Data Center SQL reading/replacement/creation/refresh, BI dashboard scanning or profiling, Taitan edit-page metric or formula profiling, authenticated SQL-platform downloads, or login-state management; route those through `usql-web-query-operator`
