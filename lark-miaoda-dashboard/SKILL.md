...
精简 Base 时同时核验当前天宫正式源码与本地读表依赖。`market2lark` 已移除经理/主管汇总键表的同步、校验和专用代码；当前写入仍需要原始表全部 45 个接收字段，原始表视图 `vewO4yxS3O` 因上游源链接固定引用而保留。经理表 `tblvBLOSRUggxsBF`、主管表 `tblQp7kAaLd6rkIh` 已不是正常推送的读写依赖，本次未删除；后续清理须先核验新版本自然周期写入成功，并取得相应删除授权。完整依赖、明确批准版本的一次性文件编号衔接与定时任务运维见 [config/SCHEDULED_PUSH.md](config/SCHEDULED_PUSH.md)。
