...
Codex-Lark-Market-KOC-GroupPush 的进量报告读取同一 Base 中的 市场顾问进量原始数据 与 市场顾问线索原始数据。筛选进量表最新期次，渠道名必须同时包含 KOC（不区分大小写）和 自孵化；采用动态AND规则，不设精确渠道白名单，未来名称变化或新增渠道只要仍满足两个关键词就自动纳入，最终按年级汇总。线索留存率、人均带班、量级完成度以及异常流量占比都必须先汇总原始分子和分母再相除，禁止对 Base 中已有的比例或人均字段求和。异常明细须同时精确匹配期次、渠道和年级。图片年级严格按初三、高一、高二、高三排列（缺失年级直接跳过），最后追加用原始加总分子和分母重算的总计行；线索留存率和量级完成度使用既有市场顾问报表配色的进度条。文字只点名量级完成度最低的一个年级且不附完成度数值，随后空一行，再按图片年级顺序逐一列出各年级异常流量占比；最低值并列时按上述年级顺序取第一个。

在用户审阅前只允许运行无发送能力的 volume_report_preview.py 入口。审批门禁依次为：本地预览审阅、一次群内测试发送、最终确认后接入正式调度。前一门禁不授权后一阶段；配置为 volume_report.stage=preview_only 时不得发送，也不得修改 taskschd.msc。正式任务仅保留每日 13:20、17:20 两个触发器：13:20 周一至周四发送过程、周五至周日发送转化；17:20 每天发送进量，不恢复21:20。进量发送前必须分别复核进量表和线索表的Base版本，幂等键须区分常规报告与进量报告。

商务 KOC 数学群复用上述进量计算和样式，但范围必须隔离：13:21 的过程/转化报告继续把 `KOC-周帅数学`、`KOC-孟亚飞数学` 分开生成和推送；17:21 的进量报告才合并渠道名包含 KOC（NFKC 归一化后不区分大小写）且不包含 `自孵化` 的全部渠道。两张表应用同一动态 AND-NOT 规则，不允许用精确渠道白名单代替。该任务已通过本地预览、一次群内测试和最终调度上线三个独立门禁，正式配置仅保留 `13:21`、`17:21` 及显式的 regular/volume 时点路由，不再保留 21 点触发器。
