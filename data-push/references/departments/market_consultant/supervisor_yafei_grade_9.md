...
本渠道只负责 `B站信息流-亚飞` 的 `初三` 数据，使用主管明细模板，并固定投递到群 `oc_3c652da1589558f0b0585d2bdb2f8cb9`（核验名称 `💪【亚飞IP- B站&直播】初三`）。群名仅用于漂移核验，不用于搜索替代 `chat_id`。
...
- 每个渠道分别@初三范围内精确最低主管，所有并列者都提醒；仅使用唯一解析且已验证属于目标群的 `open_id`。
- 本地预览以已认证 user 身份只读核验群名和成员；正式消息固定由 `sender.identity=bot` 的“管家”发送，两个身份不得互相替代。

## 周期与调度

周一至周四自动播报过程数据，周五至周日自动播报转化结果。它加入 13:20、17:20、21:20 三轮全局播报，因前五个任务已占用 `:20` 至 `:24`，本任务固定为第 6 顺位并在每轮 `:25` 启动，失败时每 2 分钟重试至 `:50`。Windows 任务名为 `Codex-Lark-Supervisor-Yafei-Grade9-Push`。
