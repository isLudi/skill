...
本渠道只负责 `KOC-孟亚飞数学` 与 `自孵化KOC-5元纯课` 的 `初三` 数据，使用主管明细模板，并固定投递到群 `oc_601bde838d4fbda7ac09840a58228f9c`（核验名称 `💗0911期【KOC初中】沟通群`）。两个渠道分别生成图片、文案和主管提醒，不合并汇总；群名仅用于漂移核验，不用于搜索替代 `chat_id`。
