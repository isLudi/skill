...
def prepare_report(args: argparse.Namespace, definition, *, ports: ReportPorts | None = None, services=feishu) -> dict[str, Any]:
    ...
    report_module = supervisor_report if definition["source"]["report_profile"] == "supervisor-detail" else grade_report
    ...
    records = ports.read_records(raw_coords, args, fields, audit=raw_audit,
        filter_json={"logic": "and", "conditions": [["渠道", "==", source_channel], ["期次", "==", period]]})
    server_returned_count = len(records)
    records = [row for row in records if str(value(row, "渠道")) == source_channel
    ...
    if report_module is supervisor_report:
        report_options["included_grades"] = tuple(
            definition["report"].get("included_grades", supervisor_report.HIGH_SCHOOL_GRADES)
        )
    ...
    skip_delivery = report_module is supervisor_report and report["empty_after_minimum_filter"]
    ...
    if not disabled and report["reminder_names"]:
        expected_target = "supervisor" if report_module is supervisor_report else "manager"
        if ("none" if args.no_mentions else args.mention_target) != expected_target:
            raise ValueError("分年级最低提醒的@对象与报表配置不一致")
        queries = sorted(set(report_module.manager_queries(report["reminder_names"]).values()))
        try:
            payload = ports.search_users(queries, args)
            mention_info = report_module.resolve_manager_candidates(report["reminder_names"], payload)
    ...
    if args.with_image and not skip_delivery:
        ...
        prefix = "supervisor-preview-" if report_module is supervisor_report else "grade-preview-"
    ...
    return {"coords": raw_coords, "source_mode": "lead-detail", "report_profile": profile, "channel": channel,
            ...
            "skip_reason": "no_supervisor_rows_meet_minimum_post_leads" if skip_delivery else "",
            "raw_count": len(records), "raw_read_audit": raw_audit, "grade_report": report,
            "supervisor_report": report if profile == "supervisor-detail" else None, "image_geometry": geometry,
            ...
            "markdown": markdown, "idempotency_key": ("supervisor-" if profile == "supervisor-detail" else "grade-") + digest,
