...
from .channels import business_koc_math, self_incubated_koc_5
...
def validate_definition(definition):
    ...
    if definition["source"]["report_profile"] != "grade-compact":
        raise ValueError("This adapter only supports the reviewed grade/manager report")
    ...
    for target in definition["targets"]:
        if definition["channel_id"] == "business_koc_math" and target["chat_id"] != policy.CHAT_ID:
...
def report_arguments(definition, target, *, channel=None, report_type="auto", state_dir=None, slot=None):
    ...
    return Namespace(channel=channel, chat_id=target["chat_id"], chat_name=target["display_name"],
        ...
        report_profile=source["report_profile"], source_mode="lead-detail", mention_target="manager", no_mentions=False,
