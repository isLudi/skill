...
$pushConfigPath = Join-Path (Split-Path $PSScriptRoot -Parent) 'config\supervisor_koc_douyin_sync_scheduled_push.json'
$pushConfig = & 'D:\anaconda3\python.exe' $pushScheduler --config $pushConfigPath --show-config | ConvertFrom-Json
if ($LASTEXITCODE -ne 0) { throw 'Could not resolve the supervisor KOC and Douyin schedule configuration.' }
...
$pushTaskName = 'Codex-Lark-Supervisor-KOC-Douyin-Push'
$pushLauncher = Join-Path $PSScriptRoot 'run_supervisor_koc_douyin_sync_scheduled_push.ps1'
...
$pushDescription = 'data-push staggered local broadcast; starts :22, retries every 2 minutes through :50. Live detail exists only while running; use view_live_push_status.ps1. Process Mon-Thu; process and conversion Fri-Sun; empty channels skipped.'
