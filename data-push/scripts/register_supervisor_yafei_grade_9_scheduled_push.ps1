...
$pushConfigPath = Join-Path (Split-Path $PSScriptRoot -Parent) 'config\supervisor_yafei_grade_9_scheduled_push.json'
$pushConfig = & 'D:\anaconda3\python.exe' $pushScheduler --config $pushConfigPath --show-config | ConvertFrom-Json
if ($LASTEXITCODE -ne 0) { throw 'Could not resolve the supervisor Yafei grade-9 schedule configuration.' }
...
$pushTaskName = 'Codex-Lark-Supervisor-Yafei-Grade9-Push'
$pushLauncher = Join-Path $PSScriptRoot 'run_supervisor_yafei_grade_9_scheduled_push.ps1'
...
$pushDescription = 'data-push staggered local broadcast; starts :25, retries every 2 minutes through :50. Grade 9 and Yafei KOC/Bilibili only. Process Mon-Thu; conversion Fri-Sun; empty channel skipped.'
