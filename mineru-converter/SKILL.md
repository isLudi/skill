...
Use when the user needs full-fidelity output: preserved images, accurate tables, LaTeX formulas, or non-Markdown formats.
...
Before using `extract`, load the token from the env file:

```powershell
# Load MINERU_TOKEN from env file (required for extract mode)
Get-Content "E:\2000_work\GAOTU\20002_市场顾问部看板维护表格\usql_api.env" | ForEach-Object {
    if ($_ -match '^MINERU_TOKEN=(.+)$') { $env:MINERU_TOKEN = $Matches[1] }
}
```

Token is stored in `E:\2000_work\GAOTU\20002_市场顾问部看板维护表格\usql_api.env`. Valid until **2026-09-09**. If `extract` fails with auth error, the token may have expired.
...
- **Auth error on extract:** Token stored in `E:\2000_work\GAOTU\20002_市场顾问部看板维护表格\usql_api.env` — load with PowerShell command above. Token expires 2026-09-09.
