    ...
    select
        ...
        case
            ...
            when rule_name like '%SEC首期掉海%'
              or rule_name like '%首期掉海%' then 'SEC首期掉海'
            when rule_name like '%SEC未加好友%'
              or rule_name like '%未加好友%'
    ...
    select
        ...
        case
            when r.record_source = 'tmk_prelead'
              and r.tmk_channel_detail = '公域学霸' then '公域'
            when r.record_source = 'tmk_prelead'
            ...
            when r.rule_name like '%赠失-星义%'
              ...
              or r.rule_name like '%赠失-春春%'
              or r.rule_name like '%赠失-郭艺%'
              or r.rule_name like '%赠失-亚飞%'
              or r.rule_name like '%青橙IP%' then concat('IP', chr(36192), chr(35838), chr(22833), chr(36133))
            when r.rule_name like '%私域本地化%'
              or r.rule_name like '%河南本地化%'
              or r.rule_name like '%青橙本地化%' then '本地化'
            when r.rule_name like '%私域会话%'
            ...
            when r.rule_name like '%武汉图书%'
              or r.rule_name like '%西安图书%' then '图书'
            when r.rule_name like '%亚飞IP%' then '主讲IP'
            when r.rule_name like '%SEC未加好友%'
              or r.rule_name like '%SEC首期掉海%'
              or r.rule_name like '%SEC招生退费%'
              or r.rule_name like '%招生退费%' then '订单复用'
            when r.rule_name like '%顾问未加好友%'
              or r.rule_name like '%青橙公海%' then '公海'
            when r.rule_name like '%抖音私信%' then '抖音私信'
            when r.rule_name like '%进校9元%'
              or r.rule_name like '%进校%' then '进校9元'
        ...
        case
            ...
            when r.rule_name like '%赠失-春春%' then '春春IP'
            when r.rule_name like '%赠失-郭艺%' then '郭艺IP'
            when r.rule_name like '%赠失-亚飞%' then '亚飞IP'
            when r.rule_name like '%青橙IP%' then '亚飞IP'
            ...
            when r.rule_name like '%河南本地化%'
              or r.rule_name like '%青橙本地化%' then '河南本地化'
            when r.rule_name like '%亚飞IP%' then '亚飞IP'
            when r.rule_name like '%SEC未加好友%' then 'SEC未加好友'
            when r.rule_name like '%SEC首期掉海%' then 'SEC首期掉海'
            when r.rule_name like '%SEC招生退费%'
              or r.rule_name like '%招生退费%' then 'SEC招生退费'
            when r.rule_name like '%顾问未加好友%'
