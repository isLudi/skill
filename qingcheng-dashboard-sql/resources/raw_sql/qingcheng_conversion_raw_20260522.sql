...
concat(cast(date_format(date_add('day',4,date_trunc('week',date_add('day',-1,date_parse(replace(concat(group_period_year,group_period_term),'期',''),'%Y%m%d')))),'%Y%m%d')as varchar),'期') qici_lead-----------线索期次
,section_assign_time---分配时间
from bdg_ba.dm_crm_lead_cost_gmv_communication_learn_full_link_df  
where  dt=format_datetime(NOW()-interval '2' hour,'YYYYMMdd') and hour=format_datetime(NOW()-interval '3' hour,'HH')
and section_assign_employee_first_level_department_name = 'H业务线'
and section_assign_employee_second_level_department_name = '青橙项目部'
...
,bb_dedup as (
    select *,
     row_number() over(partition by employee_email_name, qici, channel_map_1, channel_map_2 order by v_lead desc, grade_1) as rn
    from bb
)
----------------------------合并
,mm as (select 
...
from bb_dedup bb1  
full outer join ud on ud.name = bb1.employee_email_name and ud.qici = bb1.qici and ud.qudao = bb1.channel_map_2
where bb1.rn = 1 or bb1.rn is null
)
-------------------------例子成本
select distinct
mm.*,
case when channel_map_2 = '亚飞IP' then 120 when channel_map_2 = '武汉图书' then 5 else 0 
end as cost_lead,
case when channel_map_2 like '%私域%' then '私域'
     when channel_map_2 like '%IP%' then 'IP'
     when channel_map_2 like '%公域%' then '公域'
