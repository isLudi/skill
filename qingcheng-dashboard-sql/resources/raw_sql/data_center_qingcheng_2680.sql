    ...
    select cast(
        case when
            ...
            and (select count(*) from (
                select qici, xuebu, xiaozu
                from temp_table.dingxi01_qing_team_g_qi
                group by qici, xuebu, xiaozu having count(*) > 1
            ) duplicate_goal) = 0
...
select
    coalesce(qg.qici, r.qici) as qici,
...
where quality.valid_key = 1
  and (qg.qici is not null or r.qici >= '20260918期')
group by coalesce(qg.qici, r.qici),
