...
from (
    select
	    ...
	    case when course_subject like '%英语%' or course_subject like '%英文%' then '英语'
	     ...
	     as subject,
        coalesce(trade_cal.qici, concat(date_format(date_trunc('week', cast(trade_time as timestamp) - interval '1' day) + interval '4' day, '%Y%m%d'), '期')) as qici,
        leader_employee_email_name,teacher_name,
    ...
    left join biz_qici_calendar trade_cal
        on cast(cast(trade_time as timestamp) as date) >= trade_cal.period_start_date
       and cast(cast(trade_time as timestamp) as date) <= trade_cal.period_end_date
    where dt = format_datetime(now() - interval '2' hour, 'YYYYMMdd')
      and hour = format_datetime(now() - interval '2' hour, 'HH')
...
,dd as (
    ...
    left join team_hist th
       ...
       and th.qici = coalesce(hist_cal.qici, concat(
            date_format(
...
,ord as (
	SELECT
		...
		coalesce(refund_cal.qici, concat(date_format(date_trunc('week', cast(full_refund_timestamp as timestamp) - interval '1' day) + interval '4' day, '%Y%m%d'), '期')) as qici_re,
          CASE
        ...
        on cast(cast(full_refund_timestamp as timestamp) as date) >= refund_cal.period_start_date
       and cast(cast(full_refund_timestamp as timestamp) as date) <= refund_cal.period_end_date
        WHERE dt = format_datetime(now() - interval '24' hour, 'YYYYMMdd')
...
from (
   select
	    qg.qici,qg.xuebu,qg.xiaozu as xiaozu1,qg.dazu,cast(qg.emye_c as decimal) as emye_c,cast(qg.goal as decimal) as goal,
    ...
    left join renchan rc on qg.xiaozu = rc.leader_employee_email_name and qg.qici = rc.qici
group by qg.qici,qg.xuebu,qg.xiaozu,qg.dazu,leader_employee_email_name,qg.emye_c,qg.goal)
