select
    c.qici,
    coalesce(c.dept_2, '-') as dept_2,
    coalesce(c.xiaozu, '-') as xiaozu,
    coalesce(c.dazu, '-') as dazu,
    coalesce(c.jingli, '-') as jingli,
    coalesce(c.channel_1, '未知') as channel_1,
    coalesce(c.channel_map_2, '未知') as channel_map_2,
    coalesce(c.grade_1, '未知') as grade_1,
    coalesce(c.employee_email_name, '-') as employee_email_name,
    coalesce(c.virtual_direct_leader_email_name, '-') as virtual_direct_leader_email_name,
    coalesce(c.v_lead, 0) as lead_count,
    coalesce(c.income, 0) as income_gmv,
    coalesce(c.refund, 0) as refund_amount,
    coalesce(c.refund_user, 0) as refund_headcount,
    coalesce(c.pay_user, 0) as pay_headcount,
    coalesce(c.promit, 0) as net_income,
    coalesce(c.p_income, 0) as current_period_income,
    coalesce(c.pay_sub, 0) as pay_sub,
    coalesce(c.p_pay_sub, 0) as p_pay_sub,
    coalesce(c.refund_1_subject_gmv, 0) as refund_1_subject_gmv,
    coalesce(c.income_1_subject_gmv, 0) as income_1_subject_gmv,
    coalesce(c.refund_2_subject_gmv, 0) as refund_2_subject_gmv,
    coalesce(c.income_2_subject_gmv, 0) as income_2_subject_gmv,
    coalesce(c.refund_3_subject_gmv, 0) as refund_3_subject_gmv,
    coalesce(c.income_3_subject_gmv, 0) as income_3_subject_gmv,
    coalesce(c.refund_4_subject_gmv, 0) as refund_4_subject_gmv,
    coalesce(c.income_4_subject_gmv, 0) as income_4_subject_gmv,
    coalesce(c.refund_5_subject_gmv, 0) as refund_5_subject_gmv,
    coalesce(c.income_5_subject_gmv, 0) as income_5_subject_gmv,
    coalesce(c.refund_6_subject_gmv, 0) as refund_6_subject_gmv,
    coalesce(c.income_6_subject_gmv, 0) as income_6_subject_gmv,
    c.qici as filter_qici,
    coalesce(c.xiaozu, '-') as filter_xiaozu,
    coalesce(c.dazu, '-') as filter_dazu,
    coalesce(c.channel_1, '未知') as filter_channel_1,
    coalesce(c.channel_map_2, '未知') as filter_channel_2
from (
...
select
    ...
    case when ub.pay_sub = 2 then ub.income else 0 end as income_2_subject_gmv,
    case when ub.pay_sub = 2 then ub.refund else 0 end as refund_2_subject_gmv,
    case when ub.pay_sub = 3 then ub.income else 0 end as income_3_subject_gmv,
    case when ub.pay_sub = 3 then ub.refund else 0 end as refund_3_subject_gmv,
    case when ub.pay_sub = 4 then ub.income else 0 end as income_4_subject_gmv,
    case when ub.pay_sub = 4 then ub.refund else 0 end as refund_4_subject_gmv,
    case when ub.pay_sub = 5 then ub.income else 0 end as income_5_subject_gmv,
    case when ub.pay_sub = 5 then ub.refund else 0 end as refund_5_subject_gmv,
    case when ub.pay_sub = 6 then ub.income else 0 end as income_6_subject_gmv,
    case when ub.pay_sub = 6 then ub.refund else 0 end as refund_6_subject_gmv
from udd_base ub
)
-- 汇总订单至顾问
,ud as (
...
sum(income_1_subject_gmv) as income_1_subject_gmv,
sum(refund_1_subject_gmv) as refund_1_subject_gmv,
sum(income_2_subject_gmv) as income_2_subject_gmv,
sum(refund_2_subject_gmv) as refund_2_subject_gmv,
sum(income_3_subject_gmv) as income_3_subject_gmv,
sum(refund_3_subject_gmv) as refund_3_subject_gmv,
sum(income_4_subject_gmv) as income_4_subject_gmv,
sum(refund_4_subject_gmv) as refund_4_subject_gmv,
sum(income_5_subject_gmv) as income_5_subject_gmv,
sum(refund_5_subject_gmv) as refund_5_subject_gmv,
sum(income_6_subject_gmv) as income_6_subject_gmv,
sum(refund_6_subject_gmv) as refund_6_subject_gmv,
...
select
    p.qici,
    case
        when p.prelead_rule_name like '%抖音正价退费%' then '抖音复用'
        else '未知'
    end as channel_map_1,
    case
        when p.prelead_rule_name like '%抖音正价退费%' then '抖音正价退费'
        else '未知'
    end as channel_map_2,
    p.grade_1,
    p.virtual_direct_leader_email_name,
...
coalesce(ud.income_1_subject_gmv, 0) as income_1_subject_gmv,
coalesce(ud.refund_1_subject_gmv, 0) as refund_1_subject_gmv,
coalesce(ud.income_2_subject_gmv, 0) as income_2_subject_gmv,
coalesce(ud.refund_2_subject_gmv, 0) as refund_2_subject_gmv,
coalesce(ud.income_3_subject_gmv, 0) as income_3_subject_gmv,
coalesce(ud.refund_3_subject_gmv, 0) as refund_3_subject_gmv,
coalesce(ud.income_4_subject_gmv, 0) as income_4_subject_gmv,
coalesce(ud.refund_4_subject_gmv, 0) as refund_4_subject_gmv,
coalesce(ud.income_5_subject_gmv, 0) as income_5_subject_gmv,
coalesce(ud.refund_5_subject_gmv, 0) as refund_5_subject_gmv,
coalesce(ud.income_6_subject_gmv, 0) as income_6_subject_gmv,
coalesce(ud.refund_6_subject_gmv, 0) as refund_6_subject_gmv,
...
mm.income_1_subject_gmv,
mm.refund_1_subject_gmv,
mm.income_2_subject_gmv,
mm.refund_2_subject_gmv,
mm.income_3_subject_gmv,
mm.refund_3_subject_gmv,
mm.income_4_subject_gmv,
mm.refund_4_subject_gmv,
mm.income_5_subject_gmv,
mm.refund_5_subject_gmv,
mm.income_6_subject_gmv,
mm.refund_6_subject_gmv,
mm.refund_user,
mm.podan,
mm.sc,
...
where substr(c.qici, 1, 8) between '20260710' and '20260728'
order by c.qici, c.channel_1, c.channel_map_2, c.grade_1, c.employee_email_name
