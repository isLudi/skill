...
from (
    values
        ...
        ('20260809期', '0809期', '20260814期', '0814期', date '2026-08-07', date '2026-08-12')
...
select
    g.*,
    case
        when regexp_like(coalesce(g.qici0, ''), '^[0-9]{4}期$') then
            concat(
                date_format(
                    date_trunc(
                        'week',
                        try(
                            date_parse(
                                concat(
                                    cast(
                                        case
                                            when cast(substr(g.qici0, 1, 2) as integer)
                                                 - month(cast(g.trade_timestamp as timestamp)) >= 6
                                            then year(cast(g.trade_timestamp as timestamp)) - 1
                                            when month(cast(g.trade_timestamp as timestamp))
                                                 - cast(substr(g.qici0, 1, 2) as integer) >= 6
                                            then year(cast(g.trade_timestamp as timestamp)) + 1
                                            else year(cast(g.trade_timestamp as timestamp))
                                        end as varchar
                                    ),
                                    substr(g.qici0, 1, 4)
                                ),
...
select
    t.*,
    case
        when date_parse(substr(t.rule_friday_period, 1, 8), '%Y%m%d')
             = date_parse(substr(t.trade_friday_period, 1, 8), '%Y%m%d') then 0
        when date_diff(
            'day',
            date_parse(substr(t.rule_friday_period, 1, 8), '%Y%m%d'),
            date_parse(substr(t.trade_friday_period, 1, 8), '%Y%m%d')
        ) = 7 then 1
        when date_diff(
            'day',
            date_parse(substr(t.rule_friday_period, 1, 8), '%Y%m%d'),
            date_parse(substr(t.trade_friday_period, 1, 8), '%Y%m%d')
        ) between 14 and 21 then 2
        when date_diff(
            'day',
            date_parse(substr(t.rule_friday_period, 1, 8), '%Y%m%d'),
            date_parse(substr(t.trade_friday_period, 1, 8), '%Y%m%d')
...
select
    b.qici,
    b.channel_map_1 as channel_1,
    b.qudao as channel_2,
    jg.dazu,
    jg.leader_employee_email_name,
    b.grade_0 as grade_list,
    b.name,
    sum(case when b.week_diff = 0 then b.promit_amount else 0 end) as gmv_7,
    sum(case when b.week_diff = 1 then b.promit_amount else 0 end) as gmv_14,
    sum(case when b.week_diff = 2 then b.promit_amount else 0 end) as gmv_30,
    sum(case when b.week_diff = 3 then b.promit_amount else 0 end) as gmv_n30,
    sum(case when b.week_diff = 4 then b.promit_amount else 0 end) as gmv_7_h,
    sum(b.promit_amount) as gmv_total,
    -sum(case when b.week_diff = 0 then b.refund_amount else 0 end) as refund_7,
    -sum(case when b.week_diff = 1 then b.refund_amount else 0 end) as refund_14,
    -sum(case when b.week_diff = 2 then b.refund_amount else 0 end) as refund_30,
    -sum(case when b.week_diff = 3 then b.refund_amount else 0 end) as refund_n30,
    -sum(case when b.week_diff = 4 then b.refund_amount else 0 end) as refund_7_p,
    -sum(b.refund_amount) as refund_total
from bucketed b
left join team_org jg
    on b.name = jg.employee_email_name
   and b.qici = jg.qici
where coalesce(b.promit_amount, 0) <> 0
   or coalesce(b.refund_amount, 0) <> 0
group by
    b.qici,
    b.channel_map_1,
    b.qudao,
