...
from gmv
where substr(qici, 1, 8) between '20260710' and '20260728'
  and refund_amount > 0
group by qici, coalesce(channel_map_1, '未知'), coalesce(qudao, '未知'), coalesce(grade_0, '未知'), name, cast(uid as varchar), order_number
)
...
select order_number, refund_reason, sum(reason_source_amount) as reason_source_amount
from refund_reason_txn
group by order_number, refund_reason
)
,refund_reason_order_total as (
select order_number, sum(reason_source_amount) as reason_source_total, count(*) as reason_count
from refund_reason_by_order
group by order_number
)
,order_reason_alloc as (
select
    go.qici, go.channel_1, go.channel_map_2, go.grade_list, go.name, go.uid, go.order_number,
...
from gmv_order go
left join refund_reason_by_order rr on go.order_number = rr.order_number
left join refund_reason_order_total rt on go.order_number = rt.order_number
)
,user_reason_detail as (
select
    qici, channel_1, channel_map_2, grade_list, name, uid, refund_reason,
    sum(refund_amount) as refund_amount,
    count(distinct order_number) as refund_order_count
from order_reason_alloc
group by qici, channel_1, channel_map_2, grade_list, name, uid, refund_reason
)
,user_reason_with_total as (
select ur.*,
    sum(ur.refund_amount) over (
        partition by ur.qici, ur.channel_1, ur.channel_map_2, ur.grade_list, ur.name, ur.uid
    ) as user_total_refund_amount
from user_reason_detail ur
)
,reason_detail_with_org as (
select
    ur.qici, ur.name, ur.channel_1, ur.channel_map_2,
    ...
    ur.grade_list, ur.uid,
    case when ur.user_total_refund_amount > 500 and ur.uid is not null then concat(
        ur.qici, '|', ur.channel_1, '|', ur.channel_map_2, '|', ur.grade_list, '|', ur.name, '|', ur.uid
    ) else null end as refund_head_key,
    ur.refund_reason, ur.refund_amount, ur.refund_order_count, ur.user_total_refund_amount
from user_reason_with_total ur
left join (
    select distinct qici, employee_email_name, xuebu, dazu, leader_employee_email_name, jingli
    from temp_table.dingxi01_qing_team_jg
) jg
  on ur.qici = jg.qici
 and ur.name = jg.employee_email_name
)
select qici, name, channel_1, channel_map_2, dept_2, dazu, xiaozu, jingli, grade_list, uid,
       refund_head_key, refund_reason, refund_amount, refund_order_count, user_total_refund_amount
