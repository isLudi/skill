...
select distinct 
f.*
,concat(date_format(date_trunc('week',date_parse(replace(concat(group_period_year,group_period_term),'期',''),'%Y%m%d') - interval '1' day) + interval '4' day,'%Y%m%d'),'期') qici
...
when f.rule_name like '%赠失-郭艺%' then 'IP郭艺' 
when f.rule_name like '%赠失-亚飞%' then 'IP亚飞' 
...
from data
left join jg_base jg on data.employee_email_name = jg.employee_email_name and data.qici = jg.qici
...
select
final_marked.*
,final_marked.employee_email_name as daiban_guwen_name
,concat(final_marked.qici, '|', final_marked.department, '|', final_marked.dept_2, '|', final_marked.xiaozu, '|', final_marked.employee_email_name) as daiban_guwen_key
,case
    when final_marked.daiban_rn = 1 then 1
    else 0
end as daiban_guwen_cnt
from (
select
final_base.*
,row_number() over (
    partition by final_base.qici, final_base.department, final_base.dept_2, final_base.xiaozu, final_base.employee_email_name
    order by final_base.v_lead desc, final_base.assign_day, final_base.rule_name, final_base.channel_map_1, final_base.channel_map_2, final_base.grade_1
) as daiban_rn
from final_base
) final_marked
)
select
final_with_daiban.qici
,final_with_daiban.rule_name
,final_with_daiban.assign_day
,final_with_daiban.channel_map_1
,final_with_daiban.channel_map_2
,final_with_daiban.grade_1
,final_with_daiban.department
,final_with_daiban.dept_2
,final_with_daiban.xiaozu
,final_with_daiban.employee_email_name
,final_with_daiban.daiban_guwen_name
,final_with_daiban.daiban_guwen_key
,final_with_daiban.daiban_guwen_cnt
,final_with_daiban.v_lead
,final_with_daiban.is_friend_lead
,final_with_daiban.first_call_time_diff_hour
,final_with_daiban.first_call_in_24h
,final_with_daiban.first_call_in_48h
,final_with_daiban.first_call_cnt
,final_with_daiban.first_call_connected_in_24h
,final_with_daiban.first_call_connected_in_48h
,final_with_daiban.first_call_connected_cnt
,final_with_daiban.call_duration
,final_with_daiban.is_long_call
,final_with_daiban.zong_call_ci
,final_with_daiban.call_status
,final_with_daiban.is_app_denglu
,final_with_daiban.daoke1
,final_with_daiban.valid_daoke_1
