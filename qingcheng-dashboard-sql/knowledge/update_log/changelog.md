...
## 2026-09-17 契约来源 Hash 与生成索引维护

- 核对退款原因、TMK 转移与期次过程导出三份既有源文档，更新 13 个契约的 `source_sha256`；未改变业务口径、契约状态或自动编译权限。
- 重建本域 manifest、contract index 及共享物理 catalog，补齐完整性检查要求的示例目录入口；未执行青橙生产查询或修改远端资源。
