...
渠道维度：线索侧 `bb.channel_map_1` 从 20260722 期起将 `私域本地化`、`河南本地化` 归为一级渠道 `本地化`；最终输出的 `channel_1` 同步按 `qici >= '20260722期' and channel_map_2 in ('私域本地化','河南本地化')` 归为 `本地化`。历史期次不回刷，仍沿用原 `私域` 归类。
