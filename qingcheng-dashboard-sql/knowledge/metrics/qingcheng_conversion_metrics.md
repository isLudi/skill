...
`qici` 当前按业务日历优先、`trade_timestamp` 周五逻辑兜底生成。业务日历覆盖：
...
- `20260808期`：`2026-08-07` 至 `2026-08-11`。
- `20260815期`：`2026-08-12` 至 `2026-08-18`。
- `20260821期`：`2026-08-19` 至 `2026-08-23`。
- 周二到周日：归到当周周五期次。
- 周一：回拨到上一周周五期次。
- 线索侧 `bb.qici` 对 `group_period_year + group_period_term` 使用同一份业务日历。
...
渠道维度：`lead_map` 与 `normal_bb` 共用图片标准渠道命名。`私域本地化`、`河南本地化`、`青橙本地化` 输出一级 `本地化`；`%青橙IP-招生退费-春春%`、`%青橙IP-招生退费-朱博士%`、`%青橙IP-招生退费-郭艺%` 优先输出一级 `IP退费`，二级分别为 `春春`、`朱博士`、`郭艺`。精确分支必须位于宽泛 `%青橙IP%` 和 `%招生退费%` 之前。
