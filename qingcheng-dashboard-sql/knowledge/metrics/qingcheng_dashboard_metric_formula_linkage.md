...
- Current snapshot date: 2026-07-29.
...
| `2677 Team completion monthly` | [qingcheng_team_completion_month_raw_20260522.sql](../../resources/raw_sql/qingcheng_team_completion_month_raw_20260522.sql) | `emye_c`, `goal`, `income`, `refund`, `promit`, `promit_4`, `H_promit_4`, `n_H_promit_4`, `refund_4`, `class_refund_4`, `podan_4` | Completion formulas use adjusted Qingcheng net receipts. SQL outputs raw non-H net fields and the frontend applies the 50% non-H discount. Class-course refund formulas should use `sum(class_refund_4)`. | Keep finance order-change links on the main transaction layer, use `order_attr.original_paid_time` for org attribution, and do not rely only on refund detail links. |
| `2680 Team completion period` | [qingcheng_team_completion_period_raw_20260522.sql](../../resources/raw_sql/qingcheng_team_completion_period_raw_20260522.sql) | `emye_c`, `goal`, `income`, `refund`, `promit`, `promit_4`, `H_promit_4`, `n_H_promit_4`, `refund_4`, `class_refund_4`, `podan_4` | Period completion uses the same metric family as monthly completion with period grain. SQL outputs raw non-H net fields and the frontend applies the 50% non-H discount. Class-course refund formulas should use `sum(class_refund_4)`. | Must cover order-change `biz_type in (2,7)` paths, preserve transfer/change-chain amount, and join team org by `employee_email_name + qici`. |
...
- Class-course refund: use `sum(class_refund_4)`. Historical formula `sum(refund) - sum(Y_refund_4)` includes post-threshold class refunds and should not be used for completion refund display.
