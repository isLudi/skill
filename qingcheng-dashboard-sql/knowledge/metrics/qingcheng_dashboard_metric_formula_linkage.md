...
- Current snapshot date: 2026-08-05.
...
| `2460 Conversion data` | [data_center_qingcheng_2460.sql](../../resources/raw_sql/data_center_qingcheng_2460.sql) | `v_lead`, `pay_user`, `p_pay_user`, `pay_sub`, `p_pay_sub`, `income`, `refund`, `promit`, `p_income`, `refund_user`, `podan`, `sc`, `cost_lead` | Conversion rate, order rate, ROI, manpower efficiency, customer price, and current/past-period revenue are frontend aggregations over these SQL outputs. | This dataset is lead-attribution oriented and can differ from finance-performance completion datasets. Current retained logic uses the six-row summer business calendar before `trade_timestamp` Friday-period fallback, service main-detail revenue with internal transfer-chain exclusion, protected course-transfer supplementation, exact IP-refund channel branches, discounted `podan`, and team-org backfill on `employee_email_name + qici`. |
...
| `2677 Team completion monthly` | [data_center_qingcheng_2677.sql](../../resources/raw_sql/data_center_qingcheng_2677.sql) | `emye_c`, `goal`, `income_all`, `refund_all`, `income`, `refund`, `promit`, `promit_4`, `H_promit_4`, `n_H_promit_4`, `refund_4`, `class_refund_4`, `podan_4` | Current team revenue/refund display uses `sum(income_all)` / `sum(refund_all)` and net uses their difference. Adjusted output remains `sum(n_H_promit_4) * 0.5 + sum(H_promit_4)`. | Service is the main amount fact; finance only supplements service-missing course-transfer paths and supplies rule/linkage fields. Finance supplements are deduped before join. |
| `2680 Team completion period` | [data_center_qingcheng_2680.sql](../../resources/raw_sql/data_center_qingcheng_2680.sql) | `emye_c`, `goal`, `income_all`, `refund_all`, `income`, `refund`, `promit`, `promit_4`, `H_promit_4`, `n_H_promit_4`, `refund_4`, `class_refund_4`, `podan_4` | Period completion uses the same amount family as monthly completion with period grain. Current revenue/refund display uses `sum(income_all)` / `sum(refund_all)`; adjusted output keeps the退4/点睛2 rule. | Must cover order-change `biz_type in (2,7)` paths, preserve transfer/change-chain amount, and join team org by `employee_email_name + qici`. |
| `2740 Dousi conversion` | [data_center_qingcheng_2740.sql](../../resources/raw_sql/data_center_qingcheng_2740.sql) | `gmv_7`, `gmv_14`, `gmv_30`, `gmv_n30`, `gmv_7_h`, `gmv_total`, `refund_7`, `refund_14`, `refund_30`, `refund_n30`, `refund_7_p`, `refund_total` | Frontend formulas split the standard-order net revenue and refunds by the rule-period Friday versus trade-period Friday difference. | Reuses the exact `2460` `service_gmv + course_transfer_gmv` standard order set, then adds only timing buckets. Team org joins on `employee_email_name + qici`; `gmv_total=sum(promit_amount)` and refund outputs are negative display values. Validate both bucket closure equations before interpreting ratios. |
| `2769 Personal conversion` | [data_center_qingcheng_2769.sql](../../resources/raw_sql/data_center_qingcheng_2769.sql) | `income_all`, `refund_all`, `income`, `refund`, `promit`, `H_promit_4`, `n_H_promit_4`, `Y_promit_4`, `Y_income_4`, `Y_refund_4`, `class_refund_4`, `qici_goal`, `moth_goal` | Current class-course revenue/refund/net = `sum(income_all)` / `sum(refund_all)` / difference. Adjusted output remains `sum(n_H_promit_4) * 0.5 + (sum(H_promit_4) - sum(Y_promit_4))`; cumulative net revenue adds back `sum(Y_promit_4)`. | This is service-main completion attribution with finance course-transfer/rule supplementation. Org attribution must prefer `order_attr.original_paid_time`; normal orders on an order-change chain must remain included. |
...
- Class-course revenue: `sum(income_all)`.
- Class-course refund: `sum(refund_all)`.
- Class-course net receipt: `sum(income_all) - sum(refund_all)`.
- `class_refund_4` remains the class-only refund under the退4/点睛退2 rule and is not the current all-refund display field.
...
- Revenue: `sum(income_all)`.
- Refund: `sum(refund_all)`.
- Net amount: `sum(income_all) - sum(refund_all)`.
...
5. For completion metrics, re-check service-main amount fields, course-change chain preservation, trial-order filtering, post-course refund exclusion, H-stage 50% discount, and `biz_type in (2,7)` coverage before changing formulas.
...
- Personal and team completion metrics use service `income_amount/refund_amount` as the main all-income/all-refund fact. `dim_finance_order_change_df` and finance are retained for internal-transfer identification and service-missing course-transfer supplementation; joining them only inside refund detail logic must not replace the service amount fact.
