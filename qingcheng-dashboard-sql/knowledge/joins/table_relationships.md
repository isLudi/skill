...
| `app_prelead_source` | `finance_dw.dim_finance_employee_df employee_org` | `app_prelead_source.employee_email_prefix = employee_org.email_prefix` | TMK 顾问直属上级补充 | 员工维表取 T-1 `dt`、H 业务线主岗，按邮箱前缀去重后 left join；`leader_employee_email_name` 输出为 `xiaozu`，未匹配不过滤原线索 | `data_center_qingcheng_3180.sql`；query `1522332746`, `1522351329`, `1522366854` |
...
| `gmv_order` | `finance_dw.dwd_finance_order_refund_df r` | `gmv_order.order_number = r.order_number` | 退费原因补充 | 先按青橙结果期次得到去重订单集合，再读取原因表；同一订单多原因必须先按订单+原因聚合，原因金额只作订单退款分摊权重 | `qingcheng_refund_reason_analysis_20260718.sql`, `qingcheng_refund_reason_analysis_20260710_20260728.sql`；退款类型 `in (1,2)` 已确认 |
| `share_order` | `temp_table.dingxi01_qing_team_jg jg` | `share_order.qici = jg.qici and share_order.name = jg.employee_email_name` | 退费结构组织补充 | 同一正向退费订单集合一次性生成 `grade/product/subject` 三类维度；结构占比分母按期次、组织和渠道切片求和 | `qingcheng_refund_structure_share_analysis_20260710_20260728.sql` |
| `order_reason_alloc` | `refund_reason_by_order` / `refund_reason_order_total` | `order_number` | 退费原因分摊 | 先按订单+原因聚合财务源金额，再将订单退款金额按权重或等额分配；分配后必须回到订单退款金额 | `qingcheng_refund_reason_analysis_20260710_20260728.sql`；分摊规则已确认 |
| `reason_detail_with_org` | `temp_table.dingxi01_qing_team_jg jg` | `qici + name = qici + employee_email_name` | 原因组织补充 | 退费原因人头按 `qici + 渠道 + 年级 + 顾问 + uid` 归属切片生成 `refund_head_key`；不使用最新架构覆盖历史期次 | `qingcheng_refund_reason_analysis_20260710_20260728.sql` |
...
| `course_transfer_finance` | `service_dw.dwd_crm_assign_private_detail_hf p` | `target_user_number = p.user_number and employee_email_name = p.employee_email_name`，并要求 `assign_time <= trade_time < close_time/fall_sea_time` | B 用户交易时保护期归因 | 一用户可有多条私海记录；按订单+科目以 `assign_time desc, private_sea_update_time desc, private_sea_id desc` 取 1 条，仅 2026-07-20 起使用 | `data_center_qingcheng_2460.sql` |
| `course_transfer_protected_finance` | `lead_map` | `lead_id + employee_email_name` | 课程转移补数渠道归因 | 复用 model 2460 当前渠道 CASE、年级和主管；不得另建渠道映射，避免覆盖抖音正价退费等并发修复 | `data_center_qingcheng_2460.sql` |
| `service_gmv` | `course_transfer_gmv` | 相同字段顺序 `union all` | 订单结果合并 | 原 service 分支保持不变；保护期课程转移在 gmv 层隔离追加，避免进入原 `prc/re_ke/order_change` 联表；2740 复用同一标准订单集后再计算时间分层 | `data_center_qingcheng_2460.sql`, `data_center_qingcheng_2740.sql` |
...
| `bucketed b` | `temp_table.dingxi01_qing_team_jg team_org` | `b.name = team_org.employee_email_name and b.qici = team_org.qici` | 抖私转化团队架构补充 | 2740 必须按员工和结果期次共同关联；只按员工关联会把一人的多期架构行复制到每个历史期次 | `data_center_qingcheng_2740.sql` |
