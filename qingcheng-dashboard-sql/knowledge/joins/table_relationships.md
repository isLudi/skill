...
| `service_gmv` | `course_transfer_gmv` | 相同字段顺序 `union all` | 订单结果合并 | 原 service 分支保持不变；保护期课程转移在 gmv 层隔离追加，避免进入原 `prc/re_ke/order_change` 联表 | `data_center_qingcheng_2460.sql` |
