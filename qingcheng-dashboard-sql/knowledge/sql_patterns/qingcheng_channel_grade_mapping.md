...
当前转化数据数据中心快照：`resources/raw_sql/data_center_qingcheng_2460.sql`
...
| 一级渠道 | 二级渠道 | 业务侧 | 线索类型 |
|---|---|---|---|
| `私域` | `私域表单`、`私域品效`、`私域图书`、`私域会话` | 顾问 | 线索 |
| `公域` | `公域学霸` | SEC、顾问 | 潜客 |
| `图书` | `武汉图书`、`西安图书` | SEC | 潜客 |
| `主讲IP` | `亚飞IP` | 顾问 | 线索 |
| `公海` | `顾问未加好友` | 顾问 | 线索 |
| `订单复用` | `SEC未加好友`、`SEC首期掉海`、`SEC招生退费` | SEC | 潜客 |
| `抖音私信` | `抖音私信` | 顾问 | 线索 |
| `进校9元` | `进校9元` | 顾问 | 线索 |
| `IP赠课失败` | `星义IP`、`朱博士IP`、`春春IP`、`郭艺IP`、`亚飞IP` | SEC、顾问 | 线索 |
| `IP退费` | `春春`、`朱博士`、`郭艺` | 顾问 | 线索 |
| `抖音复用` | `抖音正价退费` | SEC | 线索 |
| `本地化` | `私域本地化`、`河南本地化` | 顾问 | 线索 |

图片备注：自 `0728期` 起，SEC 所有渠道线索均从潜客线索进入。SQL 命名层必须保持上表标准，入口来源和组织限域仍由对应 CTE 控制。

### 1.3 订单复用 / SEC 未加好友双重限域

- `record_source='tmk_prelead'` 的潜客过程行先在 `tmk_data.tmk_channel_detail` 标准化二级渠道，再反推图片标准一级渠道：
  - `公域学霸` -> `公域`
  - `武汉图书`、`西安图书` -> `图书`
  - `SEC未加好友`、`SEC首期掉海`、`SEC招生退费` -> `订单复用`
- `规划系统高一`、`规划系统高二`、`规划系统高三` 输出二级渠道 `SEC未加好友`。
...
- 仅对 `record_source='normal_lead'` 生效；TMK 潜客继续由 `tmk_channel_detail` 映射，不进入 `IP退费`。
...
| 匹配规则 | 输出 |
|---|---|
| `%青橙IP-招生退费-春春%`、`%青橙IP-招生退费-朱博士%`、`%青橙IP-招生退费-郭艺%` | `IP退费` |
| `%抖音正价退费%` | `抖音复用` |
| `%赠失-星义%`、`%赠失-朱博士%`、`%赠失-春春%`、`%赠失-郭艺%`、`%赠失-亚飞%`、`%青橙IP%` | `IP赠课失败` |
| `%私域本地化%`、`%河南本地化%`、`%青橙本地化%` | `本地化` |
| `%私域会话%`、`%私域表单%`、`%私域品效%`、`%私域图书%` | `私域` |
| `%公域学霸%`、`%青橙公域%` | `公域` |
| `%武汉图书%`、`%西安图书%` | `图书` |
| `%亚飞IP%` | `主讲IP` |
| `%顾问未加好友%`、`%青橙公海%` | `公海` |
| `%SEC未加好友%`、`%SEC首期掉海%`、`%SEC招生退费%`、`%招生退费%` | `订单复用` |
| `%抖音私信%` | `抖音私信` |
| `%进校9元%`、`%进校%` | `进校9元` |
...
- `IP退费` 三个精确分支必须先于 `%青橙IP%` 和 `%招生退费%`；否则会分别被错误归入 `IP赠课失败 / 亚飞IP` 或 `订单复用 / SEC招生退费`。
- `%抖音正价退费%` 是成对渠道契约：一级输出 `抖音复用`，二级输出 `抖音正价退费`。
- `私域本地化`、`河南本地化`、`青橙本地化` 统一归属一级 `本地化`；该分支必须放在私域宽泛分支之前。
- `青橙IP` 经业务确认归属一级 `IP赠课失败`；二级兜底为 `亚飞IP`，明确的 `%赠失-*%` 先细分到 `星义IP`、`朱博士IP`、`春春IP`、`郭艺IP`、`亚飞IP`。
- 平台会拒绝 SQL 文本里直接出现 `赠课失败` 字面值，生产 SQL 用 `concat('IP', chr(36192), chr(35838), chr(22833), chr(36133))` 构造一级渠道，实际输出仍为 `IP赠课失败`。
- `SEC未加好友`、`SEC首期掉海`、`SEC招生退费` 必须放在 `%公海%` 宽泛分支之前，否则会被错误打到一级 `公海`。
- 转化数据中 `亚飞IP` 同时可能属于 `主讲IP` 和 `IP赠课失败`，因此 `2460` 必须贯穿一级渠道并用 `channel_1 + channel_map_2` 共同 join，不得再只按二级渠道反推一级。
...
转化 SQL 中存在两套渠道映射，但输出命名必须保持一致：

1. `lead_map` 中输出一级渠道 `channel_map_1`、二级渠道 `rule_name0` 和年级 `grade_0`。
2. `bb` 中为线索量生成 `channel_map_1`、`channel_map_2` 和 `grade_1`。
3. `mm` 合并订单与线索时按 `channel_map_1 + channel_map_2` 对齐，并最终输出 `channel_1`。
...
最终不再按 `channel_map_2` 反推，而是使用 `coalesce(bb1.channel_map_1, ud.channel_map_1, '未知')`。这样可区分 `主讲IP / 亚飞IP` 与 `IP赠课失败 / 亚飞IP`。
