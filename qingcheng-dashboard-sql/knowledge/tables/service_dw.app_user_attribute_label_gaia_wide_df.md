...
本 Skill 仅复用该公共表的结构说明；如与青橙业务数据联用，范围限定必须落在主事实表或员工/订单侧。
...
|---|---|---|---|---|---|
| 无 | - | - | 否 | 未识别 department_name 字段；若查询涉及青橙业务范围，需在主事实表或关联表侧补充范围限定 | 字段目录 |

说明：
- 该表本身不提供稳定的部门范围字段。
- 如果与青橙主事实表、订单表、员工维表联用，应由那些表承担青橙范围限定。
...
- 历史字段类型注意事项：旧 PDF 图片 OCR 记录仅保留为来源说明，不是现行 schema 权威；字段必须以 Data Map 同步结果为准。
- 常见报错：字段不存在、库名前缀不正确、小时表遗漏 hour、group by 不完整。
- 待人工确认问题：
- 历史 PDF 第 1-4 页曾用于人工录入部分字段；这些记录不得继续扩充或刷新字段目录，完整字段应通过 `sync-datamap-fields --target-skill qingcheng` 复核。
