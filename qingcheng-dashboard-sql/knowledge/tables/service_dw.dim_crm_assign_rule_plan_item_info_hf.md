...
| 字段名 | 类型 | 中文含义 | 备注 |
|---|---|---|---|
| `plan_item_id` | bigint | 分配计划Item主键ID | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `account_id` | bigint | 顾问ID | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `employee_email_prefix` | string | 顾问邮箱前缀 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `employee_state_desc` | string | 顾问状态描述 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `relation_count` | int | U群关联次数 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `is_del_desc` | string | 删除状态描述 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `plan_item_create_time` | timestamp | Item创建时间 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `plan_item_update_time` | timestamp | Item更新时间 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `employee_id` | int | 课程顾问employeeId | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `employee_is_enable_desc` | string | 顾问启用状态描述 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `has_group_state` | int | 有群状态（0有群可分 1无群可分） | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `has_group_state_desc` | string | 有群状态描述 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
| `assign_priority` | int | 人员分配优先级 | 数据地图补充，业务口径需结合青橙 SQL 使用场景确认 |
