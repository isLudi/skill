...
| 写青橙退费率、退费结构或退费原因分析 | `knowledge/dashboards/qingcheng_refund_analysis_20260710_20260728.md` | `knowledge/metrics/qingcheng_refund_analysis_metrics.md`、`knowledge/sql_patterns/qingcheng_refund_analysis.md`、`knowledge/tables/finance_dw.dwd_finance_order_refund_df.md`、`knowledge/joins/table_relationships.md` | 三份 SQL 复用 2460 的订单归属；比例先聚合分子/分母再计算；退款类型、原因金额分摊和 500 元人头阈值已确认；金额和人头必须与转化看板核对 |
| 写青橙 TMK 转移线索、过程渠道、成交线索数或单效 | `knowledge/dashboards/qingcheng_tmk_transfer_order_trace_template.md` | `knowledge/metrics/qingcheng_tmk_transfer_order_trace_metrics.md`、`knowledge/joins/common_join_keys.md`、过程渠道维度契约；延迟排查再读观测台账 | 潜客 ID 与转移后正常 lead_id 不可混用；`deal_lead_count` 一条线索最多计 1；析出率/单效按分子分母聚合后重算；首次承接需按私海历史去重 |
...
| 写青橙营收、团队完成度、个人转化 SQL | 对应 `knowledge/dashboards/qingcheng_*_raw_*.md` 和 `resources/raw_sql/data_center_qingcheng_2677.sql`、`resources/raw_sql/data_center_qingcheng_2680.sql`、`resources/raw_sql/data_center_qingcheng_2769.sql` | 对应 `knowledge/metrics/*.md`、`knowledge/temp_tables/*.md`、`knowledge/sql_patterns/qingcheng_completion_sql_repair_checklist.md` | 先区分目标表驱动、架构表驱动和 service 主金额事实；完成度三份 SQL 必须联动维护，金额展示使用 `income_all/refund_all`，折算仍使用原退4/点睛2字段 |
| 排查个人/团队完成度与订单流水不一致 | `knowledge/sql_patterns/qingcheng_template_dashboard_amount_reconciliation_20260805.md`、`knowledge/sql_patterns/qingcheng_completion_sql_repair_checklist.md` | `knowledge/sql_patterns/qingcheng_personal_completion_discounted_output_risks.md`、`knowledge/dashboards/qingcheng_personal_conversion_raw_20260522.md`、`knowledge/metrics/qingcheng_personal_conversion_metrics.md`、`knowledge/tables/finance_dw.app_finance_performance_extend_details_hf.md` | 先统一员工范围、期次、试听过滤和金额字段；再查 service transfer、finance 补充去重和渠道 ld 唯一性，最后看前端公式 |
...
| Apply 青橙看板变更 | `DashboardChangePlan` 与 operator 当前 allowlist | `usql-web-query-operator` 的 `apply-dashboard-change` | 仅允许字段/局筛标签/组件标题/公筛标题/Tab 标签/布局/公式/动态默认/根背景九类 stable-ID 窄操作；任一 blocked operation 使整次 Apply 零写入；本 Skill 不掌握登录态或写接口 |
