...
青橙团队架构表。当前在 `data_center_qingcheng_2460.sql` 和 `data_center_qingcheng_2740.sql` 最终层按员工与期次补充团队架构；在 `qingcheng_team_completion_month_raw_20260522.sql` 和 `qingcheng_team_completion_period_raw_20260522.sql` 中补充直属主管，用于和团队目标表合并；在 `qingcheng_personal_conversion_raw_20260522.sql` 中作为个人期次输出骨架。
...
| 刷新方式 | 待人工确认 |
| 刷新频率 | 待人工确认 |
| 有效期 | 表内覆盖多个 `qici`；当前 canonical 转化 raw、抖私转化 raw、个人转化和团队完成度月/期都按结果期次/交易期次 join |
...
2026-07-29 快照验证：155 名员工共有 1,729 行，152 名员工跨期重复，单人最多 14 行；`employee_email_name + qici` 为 1,729 个唯一键且无重复。该结论需在临时表刷新后继续回归。

## 4. 字段清单

| 字段名 | 类型 | 中文含义 | 备注 |
|---|---|---|---|
| `qici` | 待人工确认 | 期次 | 当前 canonical 转化 raw、抖私转化 raw、个人转化和团队完成度月/期都按 `qici` 回连 |
...
- `qingcheng_dousi_conversion_raw_20260729`
...
- 默认只作为青橙转化、青橙抖私转化、青橙团队完成度【月/期】和青橙个人转化看板的团队架构表。
- 当前 canonical 转化 raw 已按 `mm.qici = jg.qici` 使用期次架构；如果后续改回最新架构，会重新引入历史期次漂移风险。
- 当前 canonical 抖私转化 raw 已按 `b.qici = jg.qici` 使用期次架构；禁止退回仅按员工 Join。
...
- 临时表刷新后继续验证 `employee_email_name + qici` 唯一性；若不唯一会放大转化和个人业绩。
