...
- 历史修复规则（2026-07-03 至 2026-08-06）：团队月完成度曾同步个人/团队期次完成度，在 `order_attr` 聚合 service `transfer_in_amount/transfer_out_amount` 并传递到 `t4`；该版本已由 2026-08-07 的明细行规则替代。
...
## 16. 2026-08-07 调课调班明细粒度与 finance 归因修复

- 团队月 SQL 与个人 SQL 同步修复：`order_attr` 不再按订单级 `max` 聚合 transfer；service 当前明细行直接带出 transfer 金额；finance raw 层删除基于不完整复合键的 `row_number()=1`，保留独立明细后按真实输出粒度聚合。
...
- 修复保护了同订单正常支付行，继续保持 `income_all/refund_all` 的 service 全量金额含义，并保留 `ord/re_ke` 的退 4/点睛退 2 规则。
- 团队月基线/候选全量回归均为 88 行、键集合完全一致；6 个团队键修正，移除旧 finance 误补 `4,573.3206` 元。`income_all=26,962,143.26`、`refund_all=3,507,394.74` 不变，`income` 和 `promit` 分别为 `26,956,364.18`、`23,806,802.94`。
- 团队月 202608 受影响金额为 `100.2816` 元；生产 Preview `1534872315`、新抽数 run `163252066`，状态 `SUCCESS`。
