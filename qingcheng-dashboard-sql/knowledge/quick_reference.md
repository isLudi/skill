...
| 一级渠道/二级渠道/`channel_map_1`/`channel_map_2` | 读取 `semantic/contracts/dimension_contracts.json`、`knowledge/sql_patterns/qingcheng_channel_grade_mapping.md` 与 2064 权威 SQL | 使用青橙过程渠道派生维度；不要退回原始 `channel_name_2`；抖音正价退费必须在两级成对映射；`私域本地化` 从 20260722 期起才归 `本地化`；普通线索 `%青橙IP-招生退费-(春春|朱博士|郭艺)%` 必须优先归 `IP退费` 及对应人名 |
...
| 青橙转化、青橙订单、净营收、破单、暑期期次、B 用户课程转移保护期 | `knowledge/dashboards/qingcheng_conversion_raw_20260626.md` | `knowledge/metrics/qingcheng_conversion_metrics.md`、`knowledge/sql_patterns/qingcheng_protected_course_transfer_conversion.md`、`knowledge/sql_patterns/qingcheng_channel_grade_mapping.md`、`knowledge/sql_patterns/qingcheng_summer_qici_corrections.md` | 业务日历优先于固定周五；service 内部转单继续剔除；20260722 期起仅将交易时处于同一青橙顾问保护期的 B 用户课程转移正向支付隔离补回 |
| 青-抖私-转化、7/14/30 天分层 | `knowledge/dashboards/qingcheng_dousi_conversion_raw_20260729.md` | `knowledge/metrics/qingcheng_dousi_conversion_metrics.md`、2460 canonical SQL、`knowledge/joins/table_relationships.md` | 2740 必须复用 2460 标准订单集；团队表按 `employee_email_name + qici` Join；总额与各时间桶必须闭合 |
...
| 青橙退费率、年级/产品/科目退费结构、退费原因与人头 | `knowledge/dashboards/qingcheng_refund_analysis_20260710_20260728.md` | `knowledge/metrics/qingcheng_refund_analysis_metrics.md`、`knowledge/sql_patterns/qingcheng_refund_analysis.md`、`knowledge/joins/table_relationships.md` | 三份本期 SQL 已保留完整分子/分母字段；比例必须聚合后重算；退款原因分摊、退款类型和 500 元人头阈值已确认 |
| 青橙 TMK 潜客转正常线索、过程一二级渠道、成交线索数与单效 | `knowledge/dashboards/qingcheng_tmk_transfer_order_trace_template.md` | `knowledge/metrics/qingcheng_tmk_transfer_order_trace_metrics.md`、两个已确认转移 Join 契约、过程渠道维度契约；延迟排查看观测台账 | 订单必须关联转移后的正常 lead_id；`deal_lead_count` 一条线索最多计 1；析出率与单效必须按聚合后的分子/分母重算，未匹配订单金额保持空 |
| 青橙指定期次/渠道过程数据导出 | `knowledge/sql_patterns/qingcheng_period_channel_process_export_template.md` | 当前 `resources/raw_sql/data_center_qingcheng_2064.sql`、`knowledge/metrics/qingcheng_process_data_metrics.md` | 不复制 runtime 全量 SQL；保留分子分母，按部门/年级/主管聚合后重算比率 |
...
| 青橙团队完成度月/期 | `knowledge/dashboards/qingcheng_team_completion_month_raw_20260522.md` 或 `knowledge/dashboards/qingcheng_team_completion_period_raw_20260522.md` | 当前 SQL：`resources/raw_sql/data_center_qingcheng_2677.sql` / `data_center_qingcheng_2680.sql`；对应 metrics、`knowledge/sql_patterns/qingcheng_completion_sql_repair_checklist.md` | 金额展示 `income_all/refund_all`；`income/refund`、`refund_4` 只服务 legacy/规则；`order_attr.original_paid_time`、service transfer、finance 独立明细真实粒度聚合、同订单同顾问补充抑制、`team_hist`、`qtg.qici = wa.qici`、只剔除调课调班流水本身 |
| 青橙个人转化、个人完成度、折算后产出 | `knowledge/dashboards/qingcheng_personal_conversion_raw_20260522.md` | 当前 SQL：`resources/raw_sql/data_center_qingcheng_2769.sql`；对应 metrics、`knowledge/sql_patterns/qingcheng_personal_completion_discounted_output_risks.md`、`knowledge/sql_patterns/qingcheng_completion_sql_repair_checklist.md` | 金额展示 `income_all/refund_all`；折算仍使用退 4/点睛退 2 和 H/非 H 规则；课程部门兜底、调课调班明细真实粒度、service transfer、finance 同订单同顾问补缺抑制、原始支付时间归属、非 H 全量 50% 折算 |
...
| 追踪 TMK 潜客转正常线索的小时产出、字段覆盖和延迟 | `knowledge/sql_patterns/qingcheng_tmk_transfer_latency_observation_log.md` | `knowledge/tables/data_lake_fuwu.dwd_crm_leads_rt.md`、`knowledge/tables/service_dw.dwd_crm_assign_private_detail_hf.md`、`knowledge/tables/bdg_ba.app_crm_prelead_cost_gmv_full_link_data_hf.md`、`knowledge/tables/service_dw.dm_crm_lead_stats_detail_hf.md`、`knowledge/tables/service_dw.dws_crm_order_lead_attribute_income_refund_stats_detail_hf.md` | 必须区分物理分区存在、目标线索进入和字段可回补；以 DWD/私海固定转移集合为主，不能先用潜客小时宽表过滤 |
| 个人/团队完成度与订单流水不一致 | `knowledge/sql_patterns/qingcheng_template_dashboard_amount_reconciliation_20260805.md`、`knowledge/sql_patterns/qingcheng_completion_sql_repair_checklist.md` | `knowledge/sql_patterns/qingcheng_personal_completion_discounted_output_risks.md`、`knowledge/dashboards/qingcheng_personal_conversion_raw_20260522.md`、`knowledge/metrics/qingcheng_personal_conversion_metrics.md`、`knowledge/tables/finance_dw.app_finance_performance_extend_details_hf.md` | 不要只看前端公式；先统一员工/团队范围、期次、试听过滤和 `income_all/refund_all`，再查 service transfer、finance 独立明细真实粒度与同订单同顾问补缺抑制、渠道 ld 唯一性和期次架构 join |
...
| `service_dw.dws_crm_order_lead_attribute_income_refund_stats_detail_hf` | 青橙完成度 `income_all/refund_all` 主金额事实、订单属性和 transfer 信号 | 金额分转元；当前完成度排除试听；内部调课仅影响 legacy/rule 字段 |
| `finance_dw.app_finance_performance_extend_details_hf` | 课程转移缺失补充、调课链路、退 4/点睛退 2 规则 | 当前完成度 SQL 保留独立明细并按真实输出粒度聚合；用 `order_number + employee_email_name` 抑制 service 已覆盖链路；不直接替代 service 的 `income_all/refund_all` |
