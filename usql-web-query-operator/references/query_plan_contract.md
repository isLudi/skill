...
- `edit-public-filters` 是 legacy dry-run-only 命令，任何 `--apply`、`--publish` 或 `--confirm-publish` 都会在浏览器启动前拒绝；
- 新草稿变更必须走 stable-ID `DashboardChangePlan`，再由独立 `apply-dashboard-change` 消费精确 Hash；
- 发布必须由独立 `publish-dashboard-change` 消费成功 ApplyReceipt 并显式 `--confirm-publish`，不得与 Apply 合并。
