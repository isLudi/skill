...
Registry 1.1.0 共 20 项：14 项 `verified/allowlisted`、5 项 `blocked`、1 项 `separate_confirmation`。P4C 的 `create_dashboard`、`create_formula`、四类组件创建、`create_public_filter` 和 `assemble_new_dashboard` 均为 `transaction_class=creation_saga`、`recovery_policy=creation_saga_no_auto_delete`，生产 adapter 为 `taitan_dashboard_build_v1`。仍 blocked 的五项是既有组件 `bind_dataset`、`update_component_filter`、模板克隆、移动文件夹和权限修改。详见 `references/dashboard_build_workflow.md`。
...
2026-07-18 的生产晋级证据保存在 `references/dashboard_build_evidence_20260718.json`，聚合证据 SHA-256 为 `7eaf0c07395f8271c30da9dd04d3b1de78c2714583a4a2725e4d34f4bd422e3c`。离线验证覆盖八项 operation，最终真实草稿还证明了 dashboard-scoped subject、四种 `value/unit` response shape、局部筛选器、全局筛选器组件目标和明确值断言。Registry 的代码、测试、证据和自身 Hash 仍由 `inspect-write-capabilities` 每次执行时重新校验。
