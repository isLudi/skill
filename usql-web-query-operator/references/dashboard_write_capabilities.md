...
Registry 1.1.0 共 25 项：19 项 `verified/allowlisted`、5 项 `blocked`、1 项 `separate_confirmation`。P4C 的 `create_dashboard`、`create_formula`、四类组件创建、`create_public_filter`、`assemble_new_dashboard`、`rename_new_component_metrics`、`style_new_components`、`create_tab_container`、`assemble_tab_slots` 和 `create_text_component` 共十三项 operation，均为 `transaction_class=creation_saga`、`recovery_policy=creation_saga_no_auto_delete`，生产 adapter 为 `taitan_dashboard_build_v1`。多个局部筛选器只作为新 unit 创建请求的一部分，不是既有组件筛选修改。仍 blocked 的五项是既有组件 `bind_dataset`、`update_component_filter`、模板克隆、移动文件夹和权限修改。详见 `references/dashboard_build_workflow.md`。
