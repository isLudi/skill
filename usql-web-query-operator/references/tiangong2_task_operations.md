...
每个旧片段必须在当前源码中恰好命中一次；单个片段、总大小和替换数量都有上限。补丁文本不能包含疑似密钥或改变凭据绑定和值，不能修改 `query_sql`、公司默认参数块、资源绑定，也不能使投影源码语法失效。密钥命名默认拦截；唯一窄例外是 `new=""` 删除一个完整函数或调用语句，且敏感名称仅作为无默认值、无注解的形参或只读变量引用。凭据赋值、属性/关键字/字面值、装饰器、动态字典键、注释中的敏感内容及不完整片段均不在例外内。投影时还须证明命中的是完整 AST 节点而非字符串内文本，并逐字保留凭据绑定/值区域；不能拆分敏感词规避检查。计划只保存补丁文件 Hash、旧/新片段 Hash、长度、匹配次数和投影源码 Hash；含凭据完整源码仅在进程内读取、投影和提交，不落盘、不打印。
