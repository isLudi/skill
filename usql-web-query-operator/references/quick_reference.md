...
| 看板配置画像 / 实时取值健康检查 | `SKILL.md` 中“看板文件夹扫描”“脚本能力” + `references/platform_profile.md` | 默认 config-only：`profile-dashboard` / `profile-folder` / `profile-all`；知识维护另加 `--write-knowledge --confirm-skill-maintenance`；独立健康检查：`check-dashboard-values` | `profile-all` 默认只写 runtime，知识目标固定且任一画像失败时整批不写；不要让 value/unit 超时阻塞知识同步 |
