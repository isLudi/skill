...
|---|---|---|---|
| SQL 页面执行、小结果预览、<=1000 行下载 | `references/sql_query_execution.md` | `scripts\usql_web_query.py run` | 失败时再读 `references/query_error_handling.md`；若怀疑 UI 变化再读 `references/platform_profile.md` |
...
| 永久参数化模板创建、原地更新、发布和回读 | `references/template_query.md` | 新建：`plan-template-creation` → `apply-template-creation` → `publish-template`；更新：`plan-template-update` → `apply-template-update` → `publish-template` | 更新必须绑定精确模板 ID/名称/基线 Hash，保留既有模板 ID 与申请权限；Plan 只读，Apply/Publish 分开确认 |
...
| Text2SQL 看板组件/布局/公式/筛选器设计与变更 | `references/dashboard_change_workflow.md` | `profile-edit-dashboard` → `design-dashboard` → `plan-dashboard-change` → `apply-dashboard-change` → `publish-dashboard-change` | P3A 可画像和 Diff 全部类型；P4B 只 Apply Registry 中九类窄修改，Apply 与 Publish 必须独立 |
| P4A/P4B 看板写能力与受控 Apply | `references/dashboard_write_capabilities.md` + `references/dashboard_change_workflow.md` | 离线 `inspect-write-capabilities`；人工抓证据用 `capture-write-evidence`；可逆沙箱事务验证用 `verify-sandbox-write-adapters`；生产草稿变更用 `apply-dashboard-change` | 只有 Registry `verified/allowlisted` 的九类窄操作可 Apply；`sandbox_verified` 仍不能自动晋级，发布必须独立确认 |
| P4C 从零创建新看板 | `references/dashboard_build_workflow.md` + `references/dashboard_write_capabilities.md` | `plan-dashboard-build` → 必要的数据中心独立 Plan/Apply/SUCCESS → 重新 Plan → `apply-dashboard-build` → `publish-dashboard-build` | 十三项创建 capability 已 `verified/allowlisted`，覆盖四类数据组件、计算列、多个局部/全局筛选器、全部指标别名、双插槽透视容器、富文本和证据限定样式；生产适配器为 `taitan_dashboard_build_v1`，仍须 exact Hash、Apply/Publish 分权、完整回读和 `creation_saga_no_auto_delete` |
| 透视表组件复制重建 / unit 重新绑定 | `references/dashboard_change_workflow.md` 的“复制重建透视表组件” | 仅沙箱：`rebind-pivot-fields-sandbox --confirm-sandbox-write` | `rebuild_pivot_unit_by_copy` 固定为 `sandbox_verified/sandbox_only`；生产 CLI 已移除，生产参数会在浏览器前返回 `blocked_unsupported`；沙箱 receipt 不授权生产 |
...
| 手工临时表上传 | `references/manual_temp_table_registry.md` | `scripts\usql_web_query.py check-manual-table` / `upload-temp-table` | 只有需要确认具体登记项或表名映射时，才打开 `manual_temp_table_registry.json` |
| 数据地图字段同步 | `references/data_knowledge_sync.md` 和 `references/domain_adapters.md` | `scripts\usql_web_query.py sync-datamap-fields` | 两业务 Skill 的物理字段权威入口；只有同步结果异常或需要改写逻辑时，才看相关实现 |
...
- 需要生成、修复、解释市场顾问 SQL：先用 `market-consultant-dashboard-sql`，再由本 skill 执行。
