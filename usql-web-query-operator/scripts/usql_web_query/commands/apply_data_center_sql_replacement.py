"""Apply one reviewed Data Center SQL replacement in production."""
...
def cmd_apply_data_center_sql_replacement(args: argparse.Namespace) -> int:
    """Run the production write chain and persist a success/failure receipt."""

    plan = load_replacement_plan(args.plan_file)
    validate_apply_request(args, plan)
    replacement_sql = load_replacement_sql(Path(plan.sql_file))
    if sql_sha256(replacement_sql) != plan.replacement_sql_sha256:
        raise UsageError("replacement SQL file changed after the reviewed plan was created")
    ...
    receipt: dict[str, Any] = {
        ...
        "operation": "apply_data_center_dataset_sql_replacement",
        "ok": False,
        "status": "running",
    ...
    executor: DataCenterReplacementExecutor | None = None
    browser = None
    context = None
    page = None
    try:
        with data_center_replacement_lock(str(plan.dataset.get("id") or "")):
            with sync_playwright() as playwright:
                ...
                print(json.dumps({**receipt, "receipt_path": str(receipt_path.resolve())}, ensure_ascii=False, indent=2))
                return 0
    except Exception as exc:
        progress = dict(executor.progress) if executor is not None else {"phase": "startup"}
        remote_write = bool(
            progress.get("remote_sql_saved") or progress.get("refresh_triggered")
        )
...
def validate_apply_request(args: argparse.Namespace, plan: Any) -> None:
    if not args.confirm_production_write:
        raise UsageError(
            "apply-data-center-sql-replacement requires --confirm-production-write"
        )
    if not args.expected_plan_sha256:
        raise UsageError(
            "apply-data-center-sql-replacement requires --expected-plan-sha256"
        )
    if args.expected_plan_sha256 != plan.plan_sha256:
        raise UsageError(
            "Data Center replacement plan hash mismatch: "
            f"expected={args.expected_plan_sha256}, actual={plan.plan_sha256}"
        )
    if plan.status != "ready":
        raise UsageError("Data Center replacement plan is blocked")


def _default_receipt_path(artifacts_dir: Path, plan_sha256: str) -> Path:
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return artifacts_dir / f"data_center_replacement_receipt_{stamp}_{plan_sha256[:12]}.json"
