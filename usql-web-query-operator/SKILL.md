...
description: 通过 Playwright 自动化执行受控的 USQL 网页取数、模板下载、临时表上传、数据平台同步和 BI 看板读取流程，可校验上游 QueryPlan，并按独立 profile→design→diff/dry-run→apply draft→publish confirmation 链路治理 Taitan 看板变更。使用本 skill 运行或下载公司 SQL、读取模板 SQL、扫描/画像看板、设计组件/布局/公式/筛选器变更，或显式应用已验证的 stable-ID 公共筛选器动态默认项；不要用它生成业务 SQL、推断指标口径或调用未经验证的看板写接口。
---
...
3. 只有在任务属于对应分支时，才继续下钻：
   ...
   - Text2SQL 看板设计、Diff、草稿 Apply 或发布：读 `references/dashboard_change_workflow.md`；不要从 legacy `edit-public-filters` 推导新流程权限。
...
- P3B 当前只允许 `relation_id + filter_id + field_id` 稳定定位的公共筛选器动态默认项更新；组件、布局和公式仅支持画像、设计和 Diff，写入统一 `blocked_unsupported`。
...
- `apply-dashboard-change`：消费精确 ChangePlan Hash，只允许 stable-ID 公共筛选器动态默认项写入 draft，并在回读后生成 ApplyReceipt；不提供发布参数。
