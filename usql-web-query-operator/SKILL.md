...
description: 通过 Playwright 自动化执行受控的 USQL 网页取数、模板下载、临时表上传、数据平台同步和 BI 看板读取流程，可在 run 前校验上游 QueryPlan 执行契约，并在用户明确授权时修改 Taitan 公共筛选器。使用本 skill 运行或下载公司 SQL、读取模板 SQL、同步数据地图/数据中心、扫描或画像看板，以及先 dry-run 再显式 apply/publish 公共筛选器；不要用它生成业务 SQL、推断指标口径或扩大看板写权限。
---
