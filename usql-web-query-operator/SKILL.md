...
description: 通过 Playwright 自动化执行受控的 USQL 网页取数、模板下载、临时表上传、数据平台同步和 BI 看板读取流程，并在用户明确授权时修改 Taitan 公共筛选器。使用本 skill 运行或下载公司 SQL、读取模板 SQL、同步数据地图/数据中心、扫描或画像看板，以及先 dry-run 再显式 apply/publish 公共筛选器；不要用它生成业务 SQL 或推断指标口径。
---
...
3. 只有在任务属于对应分支时，才继续下钻：
   - SQL 页面执行 / 小结果下载：读“标准流程”；失败时再读 `references/query_error_handling.md`。
...
`profile-edit-dashboard` 会读取 `draft` 版本的看板配置、透视表单元详情、模型字段详情、自定义指标公式和可识别的数据集字段树，并把 JSON 写入 runtime artifact 目录。该命令只调用读取类接口，不调用保存、发布、删除、新建或更新接口；它的边界是“了解指标含义和公式”，不是修改看板。若只想读取当前透视表实际使用字段而不抓取完整字段树，可加 `--skip-dataset-fields`。
...
- `template-download`：通过临时模板创建、查询、下载和默认清理流程处理明确授权的大结果下载。
...
- `edit-public-filters`：打开 Taitan 编辑页，规划全局筛选器第 1/第 2 项的动态筛选设置。默认仅 dry-run；`--apply` 才写入草稿，`--apply --publish --confirm-publish` 才允许全量发布。生产发布示例：
```powershell
D:\anaconda3\python.exe scripts\read_dashboard.py edit-public-filters `
  --target-set qingcheng-required `
  --apply `
  --publish `
  --confirm-publish `
  --version-description "auto update public filters"
```
