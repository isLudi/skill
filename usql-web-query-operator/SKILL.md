...
- 账号密码从环境变量、`E:\1900_work\GAOTU\19002_市场顾问部看板维护表格\usql_api.env` 或交互式输入读取，不要硬编码到脚本、文档、shell history 或已提交文件中。
...
- `edit-public-filters`：打开 Taitan 编辑页，修改全局筛选器第 1/第 2 项的动态筛选设置，默认保存并全量发布；生产使用示例：
```powershell
...
$env:MINERU_TOKEN = (Get-Content "E:\1900_work\GAOTU\19002_市场顾问部看板维护表格\usql_api.env" | Select-String '^MINERU_TOKEN=(.+)$').Matches.Groups[1].Value
