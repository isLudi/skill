...
- P4C 创建使用独立 `DashboardBuildSpec → DashboardBuildPlan → DashboardBuildReceipt → DashboardBuildPublishReceipt` Saga。失败不自动删除新 dashboard、formula、unit、relation 或数据集；Receipt 必须列出孤立资源和人工清理要求。当前 P4C 创建 capability 尚无完整真实证据和生产 adapter，保持 `observed/blocked`，`apply-dashboard-build` 必须在浏览器前停止。
...
从零创建唯一命名新看板时，按 `references/dashboard_build_workflow.md` 使用独立 P4C 链路。`plan-dashboard-build` 重新读取并校验 QueryPlan、DashboardDatasetSpec、业务 contract source Hash、字段树和可选 Data Center Receipt；`apply-dashboard-build` 只允许 Registry 已验证的 creation-saga adapter 且永不发布；`publish-dashboard-build` 必须消费成功 BuildReceipt 并独立确认。未收齐真实沙箱证据时，正确行为是保持 blocked。
