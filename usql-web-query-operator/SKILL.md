...
从零创建唯一命名新看板时，按 `references/dashboard_build_workflow.md` 使用独立 P4C 链路。`plan-dashboard-build` 重新读取并校验 QueryPlan、DashboardDatasetSpec、业务 contract source Hash、抽取字段树和可选 Data Center Receipt；subject 级 `customized_*` 列不进入抽取 schema Hash。对于平台会分配看板级 subject 的数据集，Plan 必须声明 `subject_identity_policy=dashboard_scoped_clone`，Saga 先创建无公式组件取得真实 subject，再创建计算列和依赖它的组件。`apply-dashboard-build` 只允许 Registry 已验证的 creation-saga adapter 且永不发布；`publish-dashboard-build` 必须消费成功 BuildReceipt 并独立确认。
...
- `profile-edit-dashboard`：打开 Taitan 编辑页，只读采集 `card/u_pivot/u_bar/u_pie` 的字段、数据集、局部筛选器、指标含义、自定义指标公式和文本口径说明，不保存、不发布、不修改看板。
...
- `apply-dashboard-build`：消费 exact BuildPlan Hash，仅在全部 P4C capability 已生产 allowlist 时执行创建 Saga；失败不自动删除，输出含孤立资源的 BuildReceipt；绝不发布。
