...
description: 通过 Playwright 自动化执行受控的 USQL 网页取数、模板下载、临时表上传、数据中心 SQL 读取/本地同步、显式生产替换刷新与新建数据集抽数，以及 BI 看板读取流程；可校验上游 QueryPlan，按独立 profile→design→diff/dry-run→apply draft→publish confirmation 链路治理既有 Taitan 看板变更，并以 P4C 创建 Saga 规划、取证和门禁从零新建看板。使用本 skill 运行或下载公司 SQL、读取模板 SQL、计划或执行经 Hash 审阅的数据中心数据集替换/创建与刷新、扫描/画像看板、设计看板变更或检查 P4A/P4B/P4C capability registry；不要用它生成业务 SQL、推断指标口径或调用未经验证的写接口。
---
...
3. 只有在任务属于对应分支时，才继续下钻：
   ...
   - Text2SQL 既有看板设计、Diff、草稿 Apply 或发布：读 `references/dashboard_change_workflow.md`；P4A 写接口探测再读 `references/dashboard_write_capabilities.md`，不要从 legacy `edit-public-filters` 推导新流程权限。
   - 从零创建看板、编排数据集、P4C 取证或独立发布：读 `references/dashboard_build_workflow.md` 和 `references/dashboard_write_capabilities.md`。创建链路不得复用 `DashboardChangePlan`。
...
- P4B draft Apply 仅允许 Registry 中九类 `verified/allowlisted` 操作：既有稳定字段显示名、既有组件局部筛选器显示标签、既有组件标题、稳定三元组公共筛选器标题、既有稳定 Tab 标签、同容器/Tab 既有节点 `x/y/w/h`、依赖不变的单组件非共享既有公式表达式、稳定三元组公共筛选器动态默认项、根背景色。每类都只能改一个已验证显示/配置槽位；其余组件、布局、公式、筛选器、主题以及所有新建/删除/重绑仍为 `blocked_unsupported`。
- P4A capability registry 默认阻断所有未经验证的写操作。沙箱 `DashboardWriteProbe` 只收集脱敏请求结构和前后 Profile，不自动晋级 registry、Apply allowlist 或发布权限。
- `sandbox_verified/sandbox_only` 仍不等于生产权限；只有同时经过共享 ChangePlan 策略、生产补偿事务、完整测试、不可变证据 Hash，并在 Registry 明确标记 `verified/allowlisted` 的操作才能进入 Apply。`verify-sandbox-write-adapters` 会额外执行九操作连续写入、逆序恢复和完整 Profile Hash 复原。
- P4C 创建使用独立 `DashboardBuildSpec → DashboardBuildPlan → DashboardBuildReceipt → DashboardBuildPublishReceipt` Saga。失败不自动删除新 dashboard、formula、unit、relation 或数据集；Receipt 必须列出孤立资源和人工清理要求。Registry 中 dashboard shell、subject 级计算列、`card/u_pivot/u_bar/u_pie`、公共筛选器、dashboardHtml 组装、全部新指标显示名、证据限定样式、双插槽 Tab 容器/插槽组装和富文本框共十三项创建能力已由真实沙箱证据和 `taitan_dashboard_build_v1` 适配器晋级为 `verified/allowlisted`；生产 Apply 仍须 exact BuildPlan Hash、显式确认、即时漂移检查、完整画像和取值回读。
- 看板构建与数据中心创建保留独立 Plan Hash、Apply 确认和 Receipt；DashboardBuild Artifact 永不授权数据中心 Apply。模板克隆和预建空板不是生产降级路径。
...
1. 先按业务域解析选择 SQL Skill：`market_consultant` 使用 `sql-query-writer-for-dashboard`，`qingcheng` 使用 `qingcheng-dashboard-sql`；域未解析时不得生成或执行生产 SQL。
2. 确认 SQL 适合网页执行：
   - 使用 Presto 语法。
   - 使用完整表名。
   - 补齐必要的 `dt` 和 `hour` 分区。
...
`profile-edit-dashboard` 会读取 `draft` 版本的看板配置、`card/u_pivot/u_bar/u_pie` 数据组件详情、模型字段详情、自定义指标公式和可识别的数据集字段树，并把 JSON 写入 runtime artifact 目录。该命令只调用读取类接口，不调用保存、发布、删除、新建或更新接口；它的边界是“了解字段、指标含义和公式”，不是修改看板。规范画像通过 `data_units[]` 绑定四类数据组件的字段、公式 ID、局部筛选器和模型/数据集 identity，并保留 `pivot_units[]` 兼容旧调用方；每个已配置数据组件都必须通过 unit/component/dataset 闭环、字段和公式/筛选器引用、dataset 反向引用，抓取完整字段树时还会核对已选字段与公式依赖存在于对应 subject。空白容器和文本等非数据组件不参与该门禁。任一抓取或绑定失败时仍保存含 `binding_validation` 的诊断 artifact，但标记 `completeness.status=incomplete`、返回非零，并禁止进入 Design/Apply/Publish。`--skip-dataset-fields` 只跳过完整字段树，不跳过 unit/model identity 和引用闭环检查。
...
从零创建唯一命名新看板时，按 `references/dashboard_build_workflow.md` 使用独立 P4C 链路。`plan-dashboard-build` 重新读取并校验 QueryPlan、DashboardDatasetSpec、业务 contract source Hash、抽取字段树和可选 Data Center Receipt；subject 级 `customized_*` 列不进入抽取 schema Hash。对于平台会分配看板级 subject 的数据集，Plan 必须声明 `subject_identity_policy=dashboard_scoped_clone`，Saga 先创建无公式组件取得真实 subject，再创建计算列和依赖它的组件。每个 measure 可声明 `display_name`，但同一 Build 必须对全部指标实例完整声明；新根组件可在创建时携带多个局部筛选器，也可创建多个全局筛选器并分别绑定逻辑组件。首版容器限定为一个或多个双插槽 `SingleTabs`，每个插槽恰有一个新 `u_pivot`，插槽内透视表不再嵌套局部筛选器。样式只接受取证支持的页面背景、`arco_blue` 透视表、`wide` Tab 和受限富文本/CSS，不开放任意 raw style。`apply-dashboard-build` 只允许 Registry 已验证的 creation-saga adapter 且永不发布；`publish-dashboard-build` 必须消费成功 BuildReceipt 并独立确认。

如果要把当前市场顾问和青橙看板文件夹完整同步到各自隔离的 SQL skill 知识库，运行：

```powershell
D:\anaconda3\python.exe scripts\read_dashboard.py profile-all `
  --dashboard-wait-ms 15000 `
  --write-knowledge `
  --confirm-skill-maintenance
```

不带 `--write-knowledge --confirm-skill-maintenance` 时，`profile-all` 只把画像写入 runtime，不修改任何业务 Skill。知识目标不可由命令行改写，始终按固定文件夹路由保持市场顾问与青橙隔离；任一画像失败时整次知识写入被阻断。
...
- `run`：可选先用 `--query-plan` 校验上游只读执行契约；通过后打开 `SQL取数`，创建或复用查询 tab，写入 SQL 前切换引擎，将 SQL 写入 CodeMirror，优先用 `Ctrl+E` 提交，再用运行按钮回退，等待查询历史/结果 tab 状态，平台失败时捕获 `error_details`，可用时提取小范围可见结果预览，并在 QueryPlan 与本地行数门禁均允许时下载并校验结果制品。直接下载若识别为 XML 伪 CSV、表头空或列不完整 Excel，会失败关闭，并明确声明未执行 Template Query 写入；只有用户另行授权并显式运行 `template-download` 才会创建临时模板。
...
- `profile-edit-dashboard`：打开 Taitan 编辑页，只读采集 `card/u_pivot/u_bar/u_pie` 的字段、数据集、局部筛选器、指标显示名、自定义指标公式，以及 `u_text/SingleTabs` 的内容、插槽和安全样式投影，不保存、不发布、不修改看板。
...
- `apply-dashboard-change`：消费精确 ChangePlan Hash，只执行九类窄范围 `verified/allowlisted` 草稿操作；逐目标双读、写后回读、最终完整画像。任一步失败会逆序恢复已完成操作，并在 ApplyReceipt 中记录恢复状态；不提供发布参数。
...
- `plan-dashboard-build`：纯本地验证声明式 P4C Spec、上游 QueryPlan/DashboardDatasetSpec、Data Center Receipt 与平台字段 resolution，输出 exact-hash BuildPlan；不启动浏览器、不授权写入。
- `apply-dashboard-build`：消费 exact BuildPlan Hash，仅在全部 P4C capability 已生产 allowlist 时执行创建 Saga；可创建多级局部/全局筛选、全量指标别名、双插槽透视容器、富文本和证据限定样式，随后做完整 Profile、七类实例/shape 与 `publicFilterList` 断言；失败不自动删除，输出含孤立资源的 BuildReceipt；绝不发布。
- `publish-dashboard-build`：独立消费成功 BuildReceipt，双读 draft/profile/payload Hash 后显式发布；无线上版本读 API 时保持 `fully_verified=false`。
- `capture-dashboard-build-evidence`：在精确沙箱看板或测试文件夹中捕获单个 P4C 创建动作的脱敏请求形态与前后 menu/profile Hash；禁止发布、删除、权限和认证写。
- `verify-sandbox-dashboard-build`：离线验证 P4C evidence manifest 覆盖和不可变 Hash，不执行写入、不自动晋级 Registry。
...
- `rebind-pivot-fields-sandbox`：仅在名称明确包含沙箱/测试标记的看板中复制重建既有透视表 unit，要求 `--confirm-sandbox-write`，先保存完整配置备份，再用 `config/copy/unit → config/update/unit → config/save/dashboardHtml` 把目标组件指向新 unit；不发布。`rebuild_pivot_unit_by_copy` 在 Registry 中固定为 `sandbox_verified/sandbox_only`；生产 CLI 已移除，内部收到生产确认参数也会在浏览器启动前返回 `blocked_unsupported`。
...
- `profile-all`：默认以 config-only 扫描固定文件夹并只写 runtime。只有同时带 `--write-knowledge --confirm-skill-maintenance` 才进入知识维护；市场顾问 markdown 固定路由到 `sql-query-writer-for-dashboard/knowledge/dashboard_web_profiles/`，青橙 markdown 固定路由到 `qingcheng-dashboard-sql/knowledge/dashboard_web_profiles/`，目标不可覆盖，且任一画像失败时不写任何业务 Skill。
...
- 清理是强制行为，没有跳过或保留临时模板的参数；只要已创建模板，成功或失败都会尝试执行 `offline -> delete`，清理失败则整条命令失败。
...
3. **usql-web-query-operator** 或已解析业务域对应的 SQL Skill：用截图证据诊断、修复或验证；不得把青橙问题默认交给市场顾问 Skill。
